...
#define OPCODE_WR_REG 1         /*!< 指令：写入外围寄存器（RTC_CNTL/RTC_IO/SARADC）*/

#define OPCODE_RD_REG 2         /*!< 指令：读取外围寄存器（RTC_CNTL/RTC_IO/SARADC）*/
...
#define OPCODE_I2C 3            /*!< 指令：读/写I2C*/
#define SUB_OPCODE_I2C_RD 0     /*!< I2C读取*/
#define SUB_OPCODE_I2C_WR 1     /*!< I2C写入*/

#define OPCODE_DELAY 4          /*!< 指令：给定周期数的延迟（nop）*/

#define OPCODE_ADC 5            /*!< 说明：SAR ADC测量*/
...
#define SUB_OPCODE_ALU_CNT 2    /*!< 算术指令、阶段计数器和立即数*/
...
#define ALU_SEL_SINC  0         /*!< 增加阶段计数器*/
#define ALU_SEL_SDEC  1         /*!< 减小阶段计数器*/
#define ALU_SEL_SRST  2         /*!< 重置阶段计数器*/

#define OPCODE_BRANCH 8         /*!< 分支机构指令*/
#define SUB_OPCODE_BX  0        /*!< 分支到绝对PC（直接或登记）*/
#define SUB_OPCODE_BR  1        /*!< 分支到相对PC，以R0为条件*/
#define SUB_OPCODE_BS  2        /*!< 分支到相对PC，取决于阶段计数器*/
...
#define JUMPS_LT 0              /*!< 如果阶段计数器<*/
#define JUMPS_GE 1              /*!< 如果阶段计数器>=*/
#define JUMPS_LE 2              /*!< 如果阶段计数器<=*/

#define OPCODE_END 9            /*!< 停止执行程序*/
#define SUB_OPCODE_END 0        /*!< 停止执行程序并可选地唤醒芯片*/
#define SUB_OPCODE_SLEEP 1      /*!< 停止执行程序并在选定的间隔后再次运行*/

#define OPCODE_TSENS 10         /*!< 说明：温度传感器测量*/
...
#define SUB_OPCODE_MACRO_LABELPC 2  /*!< 标签指针宏*/
/**@}*/
...
union ulp_insn {
    ...
    struct {
        ...
        uint32_t opcode : 4;        /*!< 操作码（Opcode_BRANCH）*/
    } b;                            /*!< BRANCH指令的格式（相对地址，以R0为条件）*/

    struct {
        uint32_t imm : 8;           /*!< 要比较的即时值*/
        uint32_t unused : 7;        /*!< 未使用的*/
        uint32_t cmp : 2;           /*!< 要执行的比较：JUMPS_LT、JUMPS_GE或JUMPS_LE*/
        uint32_t offset : 7;        /*!< 相对于当前PC的目标PC偏移的绝对值，用文字表示*/
        uint32_t sign : 1;          /*!< 目标PC偏移量的符号：0：正，1：负*/
        uint32_t sub_opcode : 3;    /*!< 子操作码（Sub_opcode_BS）*/
        uint32_t opcode : 4;        /*!< 操作码（Opcode_BRANCH）*/
    } bs;                           /*!< BRANCH指令的格式（相对地址，取决于阶段计数器）*/
    ...
    struct {
        uint32_t unused1 : 4;       /*!< 未使用的*/
        uint32_t imm : 8;           /*!< 操作数的立即值*/
        uint32_t unused2 : 9;       /*!< 未使用的*/
        uint32_t sel : 4;           /*!< 要执行的操作，ALU_SEL_Sxxx之一*/
        uint32_t sub_opcode : 3;    /*!< 子操作码（Sub_opcode_ALU_CNT）*/
        uint32_t opcode : 4;        /*!< 操作码（Opcode_ALU）*/
    } alu_reg_s;                    /*!< ALU指令的格式（阶段计数器和立即数）*/
    ...
    struct {
        ...
        uint32_t data : 8;          /*!< 8位数据用于写入操作*/
        uint32_t low_bits : 3;      /*!< 写入操作范围的低位（低位被屏蔽）*/
        uint32_t high_bits : 3;     /*!< 写入操作范围的高位（高位被屏蔽）*/
        uint32_t i2c_sel : 4;       /*!< 从地址寄存器索引[7:0]*/
        uint32_t unused : 1;        /*!< 未使用的*/
    ...
    struct {
        uint32_t dreg : 2;          /*!< 目标寄存器（用于SUB_OPCODE_MRO_LABELPC）>*/
        uint32_t label : 16;        /*!< 标签编号*/
        uint32_t unused : 6;        /*!< 未使用的*/
        uint32_t sub_opcode : 4;    /*!< SUB_OPCODE_MRO_LABEL或SUB_OPCONE_MRO_BRANCH或SUB_OPCDE_MRO_MABELPC*/
        uint32_t opcode: 4;         /*!< 操作码（OPCODEMACRO）*/
    } macro;                        /*!< MACRO使用的令牌格式*/

    uint32_t instruction;           /*!< ULP协处理器的编码指令*/
...
/**
 * 宏：将给定标签处的程序计数器移动到寄存器中。然后，该地址可以与I_BXR、I_BXZR、I_BXFR等一起使用。
 *
...
/**
 * 宏：如果阶段计数器等于立即数，则分支到标签。使用两条JUMPS指令执行：JUMPS next、imm_value、LT JUMPS label_num、imm_vvalue、LE
 *
 * 此宏生成三个由逗号分隔的ulp_insn_t值，在定义ulp_inst_t数组的内容时应使用这些值。第二个值不是真正的指令；它是一个由ulp_process_macros_and_load函数删除的令牌。
 */
...
/**
 * 宏：如果阶段计数器大于立即数，则分支到标签。使用两条指令执行：JUMPS next、imm_value、LE JUMPS label_num、imm_vvalue、GE
 *
 * 此宏生成三个由逗号分隔的ulp_insn_t值，在定义ulp_inst_t数组的内容时应使用这些值。第二个值不是真正的指令；它是一个由ulp_process_macros_and_load函数删除的令牌。
 */
...
/**
 *  分支相对如果（阶段计数器[comp_type][imm_value]）的计算结果为true。
 *
 *  pc_offset以文字表示，可以从-127到127 imm_value是一个8位值，用于将阶段计数器与comp_type进行比较。comp_type是要执行的比较类型：JUMPS_LT（<）、JUMPS_GE（>=）或JUMPS_LE（<=）
 */
...
/**
 *  与从属设备执行I2C事务。提供I_I2C_READ和I_I2C_WRITE是为了方便，而不是直接使用。
 *
 *  从属地址（7位格式）必须预先设置到SENS_I2C_Slave_ADDRx寄存器字段中，其中x==Slave_sel。对于读取操作，8位读取结果存储在R0寄存器中。对于写入操作，val将写入[high_bit:low_bit]处的sub_addr。超出此范围的位被屏蔽。
 */
...
/**
 * 从I2C从设备的子地址读取一个字节，并将结果存储在R0中。
 *
 * 从属地址（7位格式）必须预先设置到SENS_I2C_Slave_ADDRx寄存器字段中，其中x==Slave_sel。
 */
