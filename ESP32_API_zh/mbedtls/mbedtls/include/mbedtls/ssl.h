...
/**所有常见的密码套件都不可用（例如，没有合适的证书，请参阅调试消息）。*/
 ...
 *
 * \return如果已接收到数据，则为接收到的字节数。\如果连接已关闭，则返回\c 0。\return如果执行非阻塞I/O，则必须在操作阻塞时返回\c MBEDTLS_ERROR_SSL_WANT_READ。\返回其他类型失败的另一个负错误代码。
 *
 ...
 *
 * 注意：更改此定义时，我们需要检查并更新：
 *  -在tests/suites/test_suite_ssl中。函数：sslpopulate_session（）和ssl_serialize_session_save_load（）
 *  -在库/ssl.tls中。c： mbedtls_ssl_session_init（）和mbedtls_ssl_session_free（）mbedtls-ssl_sessions_save（）和ssl_session_load（）ssl_session_copy（）
 */
...
{
    /* 按大小对项目进行分组并重新排序，以最大限度地利用即时偏移访问。*/
    ...
    /*
     * 标志（可以是保存RAM的位字段，但在具有直接字节访问指令的架构上，单独的字节会使代码变小）。
     */

    uint8_t endpoint /*布尔*/;      /*!< 0:客户端，1:服务器*/
    uint8_t transport /*布尔*/;     /*!< 流（TLS）或数据报（DTLS）*/
    uint8_t authmode /*2位*/;    /*!< MBEDTLS_SSL_VERIFY_XXX             */
    /* 即使LEGACY_BREAK_HANDSHAKE禁用了反悔，也需要*/
    uint8_t allow_legacy_renegotiation /*2位*/; /*!< MBEDTLS_LEGACY_XXX  */
#if defined(MBEDTLS_ARC4_C)
    uint8_t arc4_disabled /*布尔*/; /*!< RC4密码套件黑名单？*/
#endif
#if defined(MBEDTLS_SSL_MAX_FRAGMENT_LENGTH)
    uint8_t mfl_code /*3位*/;    /*!< 期望碎片长度*/
#endif
#if defined(MBEDTLS_SSL_ENCRYPT_THEN_MAC)
    uint8_t encrypt_then_mac /*布尔*/;  /*!< 协商加密然后mac？*/
#endif
#if defined(MBEDTLS_SSL_EXTENDED_MASTER_SECRET)
    uint8_t extended_ms /*布尔*/;   /*!< 协商扩展的主秘密？*/
#endif
#if defined(MBEDTLS_SSL_DTLS_ANTI_REPLAY)
    uint8_t anti_replay /*布尔*/;   /*!< 检测并防止重放？*/
#endif
#if defined(MBEDTLS_SSL_CBC_RECORD_SPLITTING)
    uint8_t cbc_record_splitting /*布尔*/;  /*!< 进行cbc记录拆分*/
#endif
#if defined(MBEDTLS_SSL_RENEGOTIATION)
    uint8_t disable_renegotiation /*布尔*/; /*!< 禁用重新协商？*/
#endif
#if defined(MBEDTLS_SSL_TRUNCATED_HMAC)
    uint8_t trunc_hmac /*布尔*/;    /*!< 协商截断的hmac？*/
#endif
#if defined(MBEDTLS_SSL_SESSION_TICKETS)
    uint8_t session_tickets /*布尔*/;   /*!< 使用会话票证？*/
#endif
#if defined(MBEDTLS_SSL_FALLBACK_SCSV) && defined(MBEDTLS_SSL_CLI_C)
    uint8_t fallback /*布尔*/;      /*!< 这是退路吗？*/
#endif
#if defined(MBEDTLS_SSL_SRV_C)
    uint8_t cert_req_ca_list /*布尔*/;  /*!< 是否启用在证书请求消息中发送CA列表？*/
#endif
#if defined(MBEDTLS_SSL_DTLS_CONNECTION_ID)
    uint8_t ignore_unexpected_cid /*布尔*/; /*!< 确定具有意外CID*的DTLS记录是否应导致失败。*/
#endif /* MBEDTLS_SSL_DTLS_CONNECTION_ID */
#if defined(MBEDTLS_SSL_DTLS_SRTP)
    uint8_t dtls_srtp_mki_support /*布尔*/; /*!< 是否支持在use_srtp扩展中使用mki_value？*/
#endif
 ...
 *
 *                 警告：会话。连接关闭时，SSL/TLS层会清除peer_cert，因此不要缓存指针！将其设置为NULL或制作证书的完整副本。
 *
...
#if defined(MBEDTLS_ECP_C)
/**
 * \brief按优先顺序设置允许的曲线。（默认值：所有定义的曲线按大小递减的顺序排列，蒙哥马利曲线排在最后。此顺序在未来版本中可能会更改。）
 *
...
#if defined(MBEDTLS_KEY_EXCHANGE_WITH_CERT_ENABLED)
/**
 * \brief为握手期间的签名设置允许的哈希值。（默认值：所有SHA-2哈希值，首先是最大值。如果启用了编译时选项`MBEDTLS_TLS_Default_ALLOW_SHA1_IN_KEY_EXCHANGE`，则也是SHA-1。）
 *
...
/**
 * \brief返回当前最大传出记录负载（字节）。这将考虑config.h设置\c MBEDTLS_SL_OUT_CONTENT_LEN、配置和协商的最大片段长度扩展（如果使用），以及DTLS的路径MTU配置和当前记录扩展。
 *
