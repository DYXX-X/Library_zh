...
#if defined(MBEDTLS_THREADING_C)
    /* 不变量：当且仅当md_ctx->md_info！=无效的。这意味着互斥体在mbedtls_hmac_drbg_seed（）或mbedtls_hmac_drbg_seed_buf（）中的初始种子设定期间初始化，并在mbedtls _ctr_drbg_free（）中释放。
     *
...
#if defined(MBEDTLS_THREADING_C)
/**
 * \注意：当使用线程支持构建Mbed TLS时，在该函数成功返回后，可以从多个线程调用mbedtls_hmac_drbg_random（）。其他操作，包括重新种子，不是线程安全的。
 */
