 ...
 * 需要在编译器中支持asm（）。
 *
 * 用于：library/aria。c库/定时。c包括/mbedtls/bn_mul.h
 *
 ...
 *
 * 用于：include/mbedtls/bignum。h库/bignum.c
 *
 ...
 *
 * 系统有时间。h、 time（）和mbedtls_platform_gmtime_r（）的实现（见下文）。时间需要正确（不一定非常准确，但至少日期应该正确）。这用于验证X.509证书的有效期。
 *
 * 如果您的系统没有正确的时钟，请进行注释。
 *
 * \注意mbedtls_platform_gmtimer（）是platform_util中的一个抽象。h，其行为类似于C标准中的gmtime_r（）函数。有关详细信息，请参阅mbedtls_platform_gmtime_r（）的文档。
 *
 ...
 *
 * 标记已弃用的函数和功能，以便在使用时生成警告。在一个版本中弃用的功能通常在下一个版本中将被删除。通过确保代码未使用此功能，您可以启用此功能来帮助您准备向新的主要版本的过渡。
 *
 * 这只适用于GCC和Clang。对于其他编译器，您可能需要使用MBEDTLS_DEPRECATED_REMOVED
 *
 * 取消注释以获取有关使用已弃用的函数和功能的警告。
 */
 ...
 *
 * 删除已弃用的函数和功能，以便在使用时生成错误。在一个版本中弃用的功能通常在下一个版本中将被删除。通过确保代码未使用此功能，您可以启用此功能来帮助您准备向新的主要版本的过渡。
 *
 * 取消注释以获取有关使用已弃用的函数和功能的错误。
 */
 ...
 * -缓冲区的内容始终有效。
 *
 * 定义此标志时，如果库函数接收到无效的参数：1.该函数将调用宏MBEDTLS_PARAM_FAILED（）。2.如果MBEDTLS_PARAM_FAILED（）未终止程序，则函数将立即返回。如果函数返回Mbed TLS错误代码，则本例中的错误代码为MBEDTLS_error_xxx_BAD_INPUT_DATA。
 *
 * 定义此标志时，还需要为MBEDTLS_PARAM_FAILED（）安排定义。您可以通过以下任一方法执行此操作：
 * -默认情况下，库定义MBEDTLS_PARAM_FAILED（）来调用函数MBEDTLS_PARAM_FAILED（），但库没有定义此函数。如果不进行任何其他安排，则必须在应用程序中提供函数mbedtls_param_failed（）。请参见`platform_util。h’表示其原型。
 * -如果启用宏#MBEDTLS_CHECK_PARAMS_ASSERT，则库将MBEDTLS_PARAM_FAILED（\c cond）定义为“ASSERT（cond）”。您仍然可以提供MBEDTLS_PARAM_FAILED（）的替代定义，它可以调用“assert”。
 * -如果在包含`config。h`或取消注释`config中MBEDTLS_PARAM_FAILED（）的定义。h`，库将调用您定义的宏，并且不会提供自己的版本。注意，如果MBEDTLS_PARAM_FAILED（）调用“assert”，则需要启用#MBEDTLS_CHECK_PARAMS_assert，以便库源文件包含“<assert.h>”。
 *
 * 取消注释以启用应用程序控制参数的验证。
 */
//#定义MBEDTLS_CHECK_PARAMS
...
/**
 * \定义MBEDTLS_CHECK_PARAMS_ASSERT
 *
 * 允许MBEDTLS_PARAM_FAILED（）调用“断言”，并将其设置为默认值“断言”。仅当定义了#MBEDTLS_CHECK_PARAMS时，才使用此宏。
 *
 * 如果未定义此宏，则MBEDTLS_PARAM_FAILED（）默认为调用函数MBEDTLS_PARAM_FAILED（）。有关详细信息，请参阅#MBEDTLS_CHECK_PARAMS的文档。
 *
 * 取消注释以允许MBEDTLS_PARAM_FAILED（）调用“assert”。
 */
//#定义MBEDTLS_CHECK_PARAMS_ASSERT
 ...
 *
 * \注意：如果使用AES_xxx_ALT宏，则建议还设置MBEDTLS_AES_ROM_TABLES，以帮助链接器垃圾收集AES表。
 *
 * 取消注释宏以启用相应函数的替代实现。
 *
 * \警告MD2、MD4、MD5、DES和SHA-1被认为是脆弱的，它们的使用构成了安全风险。如果可能的话，我们建议避免对它们的依赖，而是考虑更强的消息摘要和密码。
 *
 * \警告如果MBEDTLS_ECDSA_SIGN_ALT和MBEDTLS_ECDSA_DETERMINISTIC都已启用，则确定性ECDH签名函数将静态HMAC-DRBG作为RNG传递给MBEDTLS_ECDSA_SIGN（）。因此，替代实现应该只使用RNG来生成临时密钥，而不使用其他。如果这不可能，则应禁用MBEDTLS_ECDSA_DETERMINISTIC，并为MBEDTLS_ECDSA_sign_det_ext（）提供替代实现（如果需要向后兼容，也为MBEDTLS _ECDSA_sign_det（）提供）。
 *
 */
//#定义MBEDTLS_MD2_PROCESS_ALT
 ...
 *
 * 此外，还可以通过设置MBEDTLS_ECP_NO_FALLBACK选项删除原始实现，在这种情况下，为其定义了相应MBEDTLS_ECP_function_NAME__ALT宏的任何函数都将无法回退到替代实现不支持的曲线。
 *
 ...
 *
 * 示例：如果设置了MBEDTLS_ECP_INTERNAL_ALT和MBEDTLS_ECP_OUBLE_JAC_ALT，则mbed TLS仍将提供ECP_DOUBLE_JAC（）函数，但如果您的实现支持该操作的组（即，MBEDTLS_INTERNAL_ECP_grp_capable（）函数为该组返回1），则将使用MBEDTLS.INTERNAL_eccp_DOUBLE_6jac（。如果您的实现不支持该组，则使用ecp_double_jac（）的原始mbed TLS实现，除非通过设置MBEDTLS_ecp_NO_fallback禁用此回退行为（在这种情况下，ecp_double _jac）将返回MBEDTLS_ERROR_ecp_FEATURE_NAVAILABLE）。
 *
 * 函数原型以及mbedtls_ecp_group和mbedtls-ecp_point的定义不会因mbedtls_ecp_INTERNAL_ALT而改变，因此mbedtls_INTERNAL_ecp_function_name__的实现必须与其定义兼容。
 *
 * 取消注释宏以启用相应函数的替代实现。
 */
/* 本节中的所有功能都需要*/
...
/**
 * \定义MBEDTLS_CHECK_RETURN_WARNING
 *
 * 如果定义了此宏，如果应用程序代码调用函数而不检查其返回值，则会发出编译时警告，但通常应在可移植应用程序中检查返回值。
 *
 * 这仅在实现#MBEDTLS_CHECK_RERETURN的平台上受支持。否则，此选项无效。
 *
 * 取消注释以获取有关使用易出错函数而不检查其返回值的警告。
 *
 * \注意：此功能正在开发中。警告将在将来添加到更多功能中。
 *
 * \注意：一些函数被认为是关键的，即使未定义此宏，忽略这些函数的返回值也会触发警告。要完全禁用返回值检查警告，请使用空扩展定义#MBEDTLS_check_RERETURN。
 */
//#定义MBEDTLS_CHECK_RETURN_WARNING
...
/** \定义MBEDTLS_CTR_DRBG_USE_128_BIT_KEY
 *
 * 取消注释此宏以在CTR_DRBG模块中使用128位密钥。默认情况下，CTR_DRBG使用256位密钥。
 */
//#定义MBEDTLS_CTR_DRBG_USE_128_BIT_KEY
...
/**
 * \定义MBEDTLS_REMOVE_DES_CIPHERSUITES
 *
 * 默认情况下，在SSL/TLS中删除3DES密码套件。此标志从mbedtls_ssl_list_ciphersuites（）返回的默认列表中删除基于3DES的密码套件。但是，通过显式包含它们，仍然可以使用mbedtls_ssl_conf_cciphersuites（）启用（其中一些）。
 *
 * 浏览器中的人攻击者可以使用基于3DES的密码套件恢复通过TLS连接发送的身份验证令牌（参见Karthikeyan Bhargavan和Gaëtan Leulent的“64位块密码的实用（内部）安全性”，参见https://sweet32.info/SWEET32_CCS16.pdf). 如果此攻击属于您的威胁模型或您不确定，则应保持启用此选项以删除基于3DES的密码套件。
 *
 * 注释此宏以将3DES保留在默认密码套件列表中。
 */
...
/**
 * \定义MBEDTLS_ECP_NO_INTERNAL_RNG
 *
 * 禁用此选项后，当使用NULL \c f_RNG参数调用时，mbedtls_ecp_mul（）将使用内部RNG，以防止某些侧信道攻击。
 *
 * 该保护引入了ECP模块对其中一个DRBG模块的依赖性。对于不需要这种保护的非常受限的实现（例如，因为您只进行签名验证，所以不操纵任何秘密，或者因为本地/物理侧信道攻击在您的威胁模型之外），可能需要消除这种依赖性。
 *
 * \警告启用此选项会使ECP的某些使用容易受到某些侧信道攻击。只有当您知道这对您的用例来说不是问题时才启用它。
 *
 * 取消注释此宏以禁用ECP中的某些对策。
 */
//#定义MBEDTLS_ECP_NO_INTERNAL_RNG
 ...
 * 使用向后兼容的ECDH上下文。
 *
 * Mbed TLS支持ECDH上下文的两种格式（在“ECDH.h”中定义的#mbedtls_ECDH_context）。对于大多数应用程序，格式的选择没有什么区别，因为所有库函数都可以使用任一格式，除了新格式与MBEDTLS_ECP_RESTTARTABLE不兼容。
 ...
 *
 * 密钥的所有者由类型为：：mbedtls_key_owner_id_t的值标识，当前硬编码为int32_t。
 *
 * 请注意，此选项仅供内部使用，可在不另行通知的情况下删除。它与MBEDTLS_USE_PSA_CRYPTO不兼容。
 */
...
/** \定义MBEDTLS_PSA_CRYPTO_BUILTIN_KEYS
 *
 * 启用对平台内置密钥的支持。如果启用此功能，则必须实现函数mbedtls_psa_platform_get_builtin_key（）。有关详细信息，请参阅该函数的文档。
 *
 * 内置密钥通常源自硬件唯一密钥或存储在安全元素中。
 *
 * 需要：MBEDTLS_PSA_CRYPTO_C。
 *
 * \警告此界面是实验性的，可能会更改或删除，恕不另行通知。
 */
//#定义MBEDTLS_PSA_CRYPTO_BUILTIN_KEYS

/** \定义MBEDTLS_PSA_CRYPTO_CLIENT
 *
 * 启用对PSA加密客户端的支持。
 *
 * \注意：当PSA加密实现未包含在库中时（MBEDTLS_PSA_crypto_C已禁用），此选项允许包含PSA加密客户端所需的代码。包含的代码是设置和获取PSA键属性的代码。PSA驱动程序的开发部分依赖于库来弥补硬件差距，这是该选项的另一种可能用法。
 *
 * \警告此界面是实验性的，可能会更改或删除，恕不另行通知。
 */
//#定义MBEDTLS_PSA_CRYPTO_CLIENT

/** \定义MBEDTLS_PSA_CRYPTO_DRIVER
 *
 * 启用对实验性PSA加密驱动程序接口的支持。
 *
 * 需要：MBEDTLS_PSA_CRYPTO_C
 *
 * \警告此界面是实验性的，可能会更改或删除，恕不另行通知。
 */
//#定义MBEDTLS_PSA_CRYPTO_DRIVERS

/** \定义MBEDTLS_PSA_CRYPTO_EXTERNAL_RNG
 *
 * 使PSA Crypto模块使用由驱动程序提供的外部随机生成器，而不是Mbed TLS的熵和DRBG模块。
 *
 ...
 *
 * 如果启用此选项，则必须在psa/crypto_platform中配置类型：：mbedtls_psa_external_random_context_t。h并使用以下原型定义一个名为mbedtls_psa_external_get_random（）的函数：``psa_status_t mbedtls_prsa_exteernal_get_rand（mbedtls_sa_external_random_context_t context，uint8_t output，size_t output_size，size_toutput_length）；）；```在第一次调用之前，\c上下文值被初始化为0。函数必须用\p output_size字节的随机数据填充\c输出缓冲区，并将\c output_length设置为\p outut_size。
 *
 * 需要：MBEDTLS_PSA_CRYPTO_C
 *
 * \警告如果启用此选项，则使用PSA加密接口的代码将不会使用为熵模块设置的任何熵源，也不会使用MBEDTLS_entropy_NV_seed启用的NV种子。
 *
 * \注意：此选项是实验性的，可能会被删除，恕不另行通知。
 */
//#定义MBEDTLS_PSA_CRYPTO_EXTERNAL_RNG
...
/**
 * \定义MBEDTLS_SHA512_SMALLER
 *
 * 启用具有较低ROM占用空间但性能较低的SHA-512实现。
 *
 * 取消注释以实现SHA512的较小实现。
 */
//#定义MBEDTLS_SHA512_SMALLER
...
/**
 * \定义MBEDTLS_SHA512_NO_SHA384
 *
 * 禁用SHA-512模块的SHA-384选项。使用此选项可以在不使用SHA-384的设备上节省一些代码大小。
 *
 * 需要：MBEDTLS_HA512_C
 *
 * 取消注释以禁用SHA-384
...
/**
 * \定义MBEDTLS_SL_RECORD_CHECKING
 *
 * 启用函数mbedtls_ssl_check_record（），该函数可用于检查传入记录的有效性和真实性，以验证以前从未见过该记录。这些检查在不修改SSL上下文的外部可见状态的情况下执行。
 *
 * 有关详细信息，请参见mbedtls_ssl_check_record（）。
 *
 * 取消注释以启用对记录检查的支持。
 */
...
/**
 * \定义MBEDTLS_SL_PROTO_TLS1_3_实验
 *
 * 此宏用于选择性地启用代码的实验部分，这些部分有助于原型TLS 1.3和DTLS 1.3实现的持续开发，并且不提供其他用途。
 *
 * \警告TLS 1.3和DTLS 1.3在Mbed TLS中尚不受支持，并且通过该宏公开的任何功能都不是公共API的一部分。特别是，此宏控制下的特性是实验性的，不具有任何稳定性保证。
 *
 * 取消注释此宏以启用TLS 1.3特有的实验和部分功能。
 */
//#定义MBEDTLS_SL_PROTO_TLS1_3_EXPERIAL
 ...
 *
 * 回退到旧的（2.7之前的版本），截断HMAC扩展的不一致实现，也截断HMAC密钥。请注意，此选项仅适用于临时升级期间，并将在库的未来版本中删除。
 *
...
/**
 * \定义MBEDTLS_TEST_CONSTANT_FLOW_MEMSAN
 *
 * 使用clang的MemoryManitizer可以测试某些敏感函数的恒定流特性。这导致一些现有测试也测试被测试代码的非功能属性。
 *
 * 此设置要求使用clang-fsanitize=内存进行编译。然后可以正常运行测试套件。
 *
 * \警告此宏仅用于扩展测试；它不被视为库API的一部分，因此它可能随时更改或消失。
 *
 * 取消注释以允许测试所选代码的恒定流特性。
 */
//#定义MBEDTLS_TEST_CONSTANT_FLOW_MEMSAN
...
/**
 * \定义MBEDTLS_TEST_CONSTANT_FLOW_VALGRIND
 *
 * 使用valgrind的memcheck工具测试某些敏感功能的恒定流量特性。这导致一些现有测试也测试被测试代码的非功能属性。
 *
 * 此设置需要valgrind标头进行构建，并且仅当测试套件使用valgrind的memcheck运行时才对测试有用。可以使用“valgrind./test_suite_xxx”对单个测试套件执行此操作，或者使用CMake时，可以使用“make memcheck”对所有测试套件执行该操作。
 *
 * \警告此宏仅用于扩展测试；它不被视为库API的一部分，因此它可能随时更改或消失。
 *
 * 取消注释以允许测试所选代码的恒定流特性。
...
/**
 * \定义MBEDTLS_TEST_HOOKS
 *
 * 启用侵入性测试的功能，如自检功能和故障注入挂钩。这将启用额外的单元测试。
 *
 * 仅启用此功能不应改变产品的行为。它只添加新的代码和新的分支点，其中默认行为与禁用此功能时相同。然而，这一特性增加了攻击面：增加了漏洞的风险，增加了更多的小工具，使漏洞利用更加容易。因此，决不能在生产中启用此功能。
 *
 * 有关详细信息，请参阅`docs/architecture/testing/mbed crypto invasive testing.md`。
 *
 * 取消注释以启用侵入性测试。
 */
//#定义MBEDTLS_TEST_HOOKS
 ...
 *
 * 使X.509和TLS库使用PSA进行加密操作，并启用新的API来使用PSA Crypto处理的密钥。
 *
 * \注意：此选项的开发目前正在进行中，Mbed TLS的X.509和TLS模块的部分尚未移植到PSA。然而，这些部分仍将照常工作，因此启用此选项不会破坏向后兼容性。
 *
 * \注意：请参阅docs/use-psa-crypto.md，了解该选项当前的功能以及迄今为止未受其影响的部分的完整描述。
 *
 * \警告此选项启用新的Mbed TLS API，这些API目前被认为是实验性的，可能随时以不兼容的方式更改。也就是说，该选项启用的API不包含在通常的API稳定性承诺中。
 *
 * 需要：MBEDTLS_PSA_CRYPTO_C。
 *
 * 取消注释，以启用PSA Crypto和新的相关API的内部使用。
 */
//#定义MBEDTLS_USE_PSA_CRYPTO

/**
 * \定义MBEDTLS_PSA_CRYPTO_CONFIG
 *
 * 此设置允许通过PSA API对加密机制的支持与通过mbedtls API的支持分开配置。
 *
 * 取消注释以启用PSA Crypto配置设置，该设置可以在include/PSA/Crypto_config.h中找到。
 *
 * 如果启用此选项并编写自己的配置文件，则必须包含mbedtls/config_psa。h在配置文件中。默认提供mbedtls/config。h包含必要的包含。
 *
 * 此功能仍处于试验阶段，尚未准备就绪，因为尚未完成。
 */
//#定义MBEDTLS_PSA_CRYPTO_CONFIG
 ...
 * 启用Camellia分组密码。
 *
 * 模块：library/chileal.c调用方：library/Ccipher.c
 *
 ...
 * 启用通用密码层。
 *
 * 模块：library/cypher.c调用方：library/ssl_tls.c
 *
 * 取消注释以启用通用密码包装器。
 */
 ...
 *
 * 启用基于CTR_DRBG AES的随机生成器。默认情况下，CTR_DRBG生成器使用AES-256。要改用AES-128，请启用上面的\c MBEDTLS_CTR_DRBG_use_128_BIT_KEY。
 *
 * \注意：要使用CTR_DRBG实现256位安全强度，必须使用AES-256并使用足够的熵。有关详细信息，请参见ctr_drbg.h。
 *
 * 模块：library/ctr_drbg.c调用方：
 *
 * 需要：MBEDTLS_AES_C
 *
 ...
 * 启用调试功能。
 *
 * 模块：library/debug.c调用方：library/ssl_cli.c library/sssl_srv.c library/ssl_tls.c
 *
 * 此模块提供调试功能。
 */
 ...
 * 启用DES分组密码。
 *
 * 模块：library/des.c调用方：library/pem.c library/cypher.c
 *
 ...
 *
 * 需要：MBEDTLS_CTR_DRBG_C和MBEDTLS_ENTROPY_C，或MBEDTLS_MAC_DRBG_ C和MBEDTLS_ENTROPY_C，或者MBEDTLC_PSA_CRYPTO_EXTERNAL_RNG。
 *
 */
#define MBEDTLS_PSA_CRYPTO_C

/**
 * \定义MBEDTLS_PSA_CRYPTO_SE_C
 *
 * 在平台安全体系结构加密API中启用安全元素支持。
 *
 * \警告此功能尚不适用于生产。它仅用于API评估和测试目的。
 *
 * 模块：library/psa_crypto_se.c
 *
 * 需要：MBEDTLS_PSA_CRYPTO_C、MBEDTLS_PSA_CRYPTO_STORAGE_C
 *
 */
//#定义MBEDTLS_PSA_CRYPTO_SE_C
...
/* ECP选项*/
//#define MBEDTLS_ECP_MAX_BITS             521 /**<组的最大位大小。通常根据配置曲线自动确定。*/
...
/**
 * \brief当检测到仅使用#MBEDTLS_CHECK_PARAMS检查的无效参数时，库将调用此宏（有关上下文，请参阅该选项的文档）。
 *
 *              当您在此处未定义此项时，库将提供默认定义。如果定义了宏#MBEDTLS_CHECK_PARAMS_ASSERT，则默认定义为“ASSERT（cond）”，否则默认定义调用函数MBEDTLS_param_failed（）。此函数在`platform_util中声明。h`的好处，但您需要在应用程序中定义。
 *
 *              当您在此处定义它时，它将替换platform_util中的默认定义。h（不再声明函数mbedtls_param_failed（）），您有责任确保此宏扩展到合适的位置（特别是，所有必要的声明都在库中可见-您可以通过在宏定义旁边的文件中提供它们来确保）。如果将此宏定义为调用“assert”，请同时定义#MBEDTLS_CHECK_PARAMS_assert，以便库源文件包含“<assert.h>”。
 *
...
/** \定义MBEDTLS_CHECK_RETURN
 *
 * 此宏用于函数声明的开头，以指示应检查其返回值。如果调用函数而不检查其返回值，它应该指示编译器发出警告或错误。
 *
 * platform_util.h中有流行编译器的默认实现。您可以通过在此处定义自己的实现来覆盖默认实现。
 *
 * 如果这里的实现是空的，这将有效地禁用对函数返回值的检查。
 */
//#定义MBEDTLS_CHECK_RETURN __attribute__（（__warn_unused_result__））

/** \定义MBEDTLS_IGNORE_RETURN
 *
 * 此宏需要一个参数，该参数应为C函数调用。如果该函数调用将导致#MBEDTLS_CHECK_RERETURN警告，则会抑制此警告。
 */
//#定义MBEDTLS_IGNORE_RETURN（result）（（void）！（结果））

/* PSA选项*/
/**
 * 将HMAC_DRBG与PSA加密子系统的HMAC_DRB的指定哈希算法一起使用。
 *
 * 如果未设置此选项：
 * -如果CTR_DRBG可用，PSA子系统将使用它而不是HMAC_DRBG。
 * -否则，PSA子系统将HMAC_DRBG与#MBEDTLS_MD_SHA512或#MBEDTLS_MD_SHA256一起使用，具体取决于可用性和未指定的启发式。
 */
//#定义MBEDTLS_PSA_HMAC_DRBG_MD_TYPE MBEDTLS_MD_SHA256

/** \def MBEDTLS_PSA_KEY_SLOT_COUNT限制PSA库支持最大数量的同时加载的密钥。加载的密钥是PSA Crypto核心存储的作为易失性密钥的密钥，或库临时加载的作为飞行中加密操作的一部分的持久密钥。
 *
 * 如果未设置此选项，库将恢复为默认值32个键。
 */
//#定义MBEDTLS_PSA_KEY_SLOT_COUNT 32
 ...
 *
 */
//#定义MBEDTLS_SL_CID_IN_LEN_MAX 32
 ...
 *
 */
//#定义MBEDTLS_SL_CID_OUT_LEN_MAX 32
 ...
 *
 */
//#定义MBEDTLS_SL_CID_PADDING_GRANULARITY 16

/** \定义MBEDTLS_SL_TLS1_3_PADDING_GRANULARITY
 *
 * 此选项控制TLS 1.3中记录明文填充的使用。
 *
 * 将始终选择填充，以便填充的明文的长度是此选项值的倍数。
 *
 * 注意：值\c 1表示传出记录不使用填充。
 ...
 * 注：在缺少除法指令的系统上，应首选二次幂。
 */
//#定义MBEDTLS_SL_TLS1_3_PADDING_GRANULARITY 1
 ...
 *
 * 这至少应为9/8*MBEDTLS_SL_IN_CONTENT_LEN，以说明最大大小的重新组合握手消息及其重新组合位图。
 *
...
/** \定义MBEDTLS_TLS_EXT_CID
 *
 * 在编写时，CID扩展名尚未分配其最终值。设置此配置选项以使Mbed TLS使用不同的值。
 *
 * 未来对Mbed TLS的一次小修订可能会更改此选项的默认值，以匹配不断发展的标准和用法。
 */
//#定义MBEDTLS_TLS_EXT_CID 254
 ...
 *
 */
//#定义MBEDTLS_TLS_DEFAULT_ALLOW_SHA1_IN_KEY_EXCHANGE
...
/**
 * 启用来自Project Everest（当前仅Curve25519）的ECDH原语的验证实现。该特性改变了ECDH上下文的布局，因此对于直接访问mbedtls_ECDH_context结构的字段的应用程序来说，这是一个兼容性问题。另请参见include/MBEDTLS/ECDH.h中的MBEDTLS_ECDH_LEGACY_CONTEXT。
 */
//#定义MBEDTLS_ECDH_VARIANT_EVEREST_ENABLED
