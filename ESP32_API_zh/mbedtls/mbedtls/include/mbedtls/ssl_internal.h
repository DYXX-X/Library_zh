...
struct mbedtls_ssl_handshake_params
{
    ...
    uint8_t max_major_ver;              /*!< 最大主版本客户端*/
    uint8_t max_minor_ver;              /*!< 最大次要版本客户端*/
    uint8_t resume;                     /*!<  会话恢复指示器*/
    uint8_t cli_exts;                   /*!< 客户端扩展存在*/

#if defined(MBEDTLS_X509_CRT_PARSE_C) &&        \
    defined(MBEDTLS_SSL_SERVER_NAME_INDICATION)
    uint8_t sni_authmode;               /*!< SNI回调的authmode*/
#endif

#if defined(MBEDTLS_SSL_SESSION_TICKETS)
    uint8_t new_session_ticket;         /*!< 使用NewSessionTicket？*/
#endif /* MBEDTLS_SSL_SESSION_TICKETS */

#if defined(MBEDTLS_SSL_EXTENDED_MASTER_SECRET)
    uint8_t extended_ms;                /*!< 使用扩展主密钥？*/
#endif

#if defined(MBEDTLS_SSL_ASYNC_PRIVATE)
    uint8_t async_in_progress;          /*!< 正在进行异步操作*/
