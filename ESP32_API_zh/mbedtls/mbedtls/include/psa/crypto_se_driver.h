 ...
 *
 * 如果记录在\p属性中的密钥类型\c类型是非对称的（#PSA_key_type_is_asymmetric（\c类型）=1），则驱动程序可以在生成时以PSA_export_public_key（）记录的格式导出公钥，方法是将其写入\p公钥缓冲区。这是可选的，用于在生成时输出公钥并且以后无法导出公钥的安全元素。不需要此功能的驱动程序应将\p pubkey_length设置为0，并应实现psa_drv_key_management_t:：p_export_public函数。某些实现不支持此功能，在这种情况下，\p pubkey为\c NULL，\p publickey_size为0。
 *
 ...
 *
 * 由于许多密钥派生算法需要多个参数，因此对于同一操作，预期可以多次调用此函数，每个函数都具有不同的算法特定的`CDROD_id`
 *
 ...
 *
 * \return#PSA_SUCCESS驱动程序已成功注册。应用程序现在可以使用\p位置通过传递给此函数的方法访问密钥。\return#PSA_ERROR_BAD_STATE此函数是在加密模块初始化后调用的，此实现在此阶段不支持驱动程序注册。\return#PSA_ERROR_ALREADY_EXISTS此位置值已注册驱动程序。\return#PSA_ERROR_INVALID_ARGUMENT\p位置是保留值。\return#PSA_ERROR_NOT_SUPPORTED`methods->hal_version`不支持此实现。\return#PSA_ERROR_INSUFFIE_MEMORY\return#PSE_ERROR_NOT_PERMITTED\return#PSA_ERROR_STORAGE_FAILURE\return#PSA错误_DATA_CORRUPT
 */
