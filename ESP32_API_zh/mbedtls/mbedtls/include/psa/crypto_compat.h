...
/*
 * 重命名的AEAD标记长度宏（PSA Crypto API<=1.0 beta3）
 */
...
/*
 * 不推荐的PSA AEAD输出大小宏（PSA Crypto API<=1.0 beta3）
 */
 ...
 *
 * \return指定算法的标记大小。如果AEAD算法没有可与其他密文区分的已识别标记，则返回0。如果AEAD无法识别，则返回1。
 */
 ...
 *
 * \return指定算法的AEAD密文大小。如果无法识别AEAD算法，则返回0。
 */
 ...
 *
 * \返回指定算法的足够输出缓冲区大小。如果无法识别AEAD算法，则返回0。
 */
 ...
 * \param alg AEAD算法（\c PSA_alg_XXX值，使得#PSA_alg_IS_AEAD（\p alg）为真）。
 *
 * \返回指定算法的足够密文缓冲区大小。如果无法识别AEAD算法，则返回0。
 */
 ...
 * \param alg AEAD算法（\c PSA_alg_XXX值，使得#PSA_alg_IS_AEAD（\p alg）为真）。
 *
 * \返回指定算法的足够明文缓冲区大小。如果无法识别AEAD算法，则返回0。
 */
 ...
 *
 * \retval#PSA_SUCCESS成功。应用程序现在可以使用“handle”值来访问密钥。\retval#PSA_ERROR_INSUFFIE_MEMORY实现没有足够的资源来打开密钥。这可能是由于达到了打开键的数量、打开键句柄的数量或可用内存的实现限制。\retval#PSA_ERROR_DOES_NOT_EXIST没有具有密钥标识符\p密钥的持久密钥。\retval#PSA_ERROR_INVALID_ARGUMENT\p密钥不是有效的持久密钥标识符。\retval#PSA_ERROR_NOT_PERMITTED指定的密钥存在，但应用程序无权访问它。请注意，本规范没有定义创建此类密钥的任何方式，但可以通过特定于实现的方式实现。\retval#PSA_ERROR_COMMUNICATION_FAILURE\retval#PSA_ERROR_CORRUPTION_DETECTED\retval#PSA_ERROR_STORAGE_FAILURE \retval#PSA _ERROR_DATA_INVALID\retval#PSA_RROR_DATA _CORRUPT\retval＃PSA_ERROR_BAD_STATE该库以前未由PSA_crypto_init（）初始化。初始化失败是否导致此错误代码取决于实现。
 */
