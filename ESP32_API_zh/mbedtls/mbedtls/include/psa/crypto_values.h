...
/** 存储的数据已损坏。
 *
 * 此错误表示某些永久性存储已损坏。它不指示具有特定错误代码的以下情况：
 *
 * -易失性内存损坏-请使用#PSA_ERROR_corruption_DETECTED。
 * -密码处理器与其外部存储器之间的通信错误-请使用#PSA_error_communication_FAILURE。
 * -当存储处于有效状态但已满时，请使用#PSA_ERROR_INSUFFIE_storage。
 * -当存储因其他原因失败时，请使用#PSA_ERROR_storage_FAILURE。
 * -当存储的数据无效时，使用#PSA_ERROR_data_INVALID。
 *
 * \注意：存储损坏并不表示以前读取的任何数据都无效。然而，以前读取的数据可能不再可从存储器读取。
 *
 * 当发生存储故障时，不再可能确保密钥库的全局完整性。
 */
#define PSA_ERROR_DATA_CORRUPT          ((psa_status_t)-152)

/** 从存储中读取的数据对实现无效。
 *
 * 此错误表示从存储中读取的某些数据的格式无效。它不指示具有特定错误代码的以下情况：
 *
 * -当存储或存储数据损坏时，请使用#PSA_ERROR_data_CORRUPT
 * -由于其他原因导致存储失败时，请使用#PSA_ERROR_storage_FAILURE
 * -API的参数无效-使用#PSA_ERROR_invalid_argument
 *
 * 此错误通常是由于明文存储后端上的存储损坏，或者试图读取由不兼容版本的库写入的数据。
 */
 ...
 *
 * HMAC密钥通常应具有与基础哈希相同的大小。此大小可以使用#PSA_HAH_LENGTH（\c alg）计算，其中\c alh是HMAC算法或基础哈希算法。*/
...
/**基于ARIA分组密码的密码、AEAD或MAC算法的密钥。*/
#define PSA_KEY_TYPE_ARIA                           ((psa_key_type_t)0x2406)

/** 基于DES或3DES（三重DES）的密码或MAC算法的密钥。
 *
 * 密钥的大小可以是64位（单个DES）、128位（2密钥3DES）或192位（3密钥3DES。
 *
...
/** 椭圆曲线密钥对。
 *
 * 椭圆曲线密钥的大小是与曲线相关联的比特大小，即字段F<sub>q</sub>上的曲线的q的比特大小。有关详细信息，请参见“PSA_ECC_FAMILY_xx”曲线族的文档。
 *
...
/** 椭圆曲线公钥。
 *
 * 椭圆曲线公钥的大小与对应的私钥相同（请参见#PSA_key_TYPE_ECC_key_PAIR和“PSA_ECC_FAMILY_xx”曲线族的文档）。
 *
 * \param curve类型为：：psa_ecc_family_t的值，用于标识要使用的ecc曲线。
 */
...
/** 扭曲的Edwards曲线Ed25519和Ed448。
 *
 * 这些曲线适用于EdDSA（两条曲线均为#PSA_ALG_PURE_EdDSA，255位曲线为#PS_ALG_ED25519PH，448位曲线为#PSA_ALG_ED448PH）。
 *
 * 该系列包括以下扭曲的Edwards曲线：
 * -255位：Edwards25519，扭曲的Edwards曲线在理论上等同于Curve25519。Bernstein等人，_twisted Edwards曲线_，Africacrypt 2008。
 * -448位：Edwards448，扭曲的Edwards曲线在理论上等同于Curve448。汉堡，_Ed448-Goldlocks，一种新的椭圆曲线，NIST ECC研讨会，2015年。
 */
...
/** SHAKE256输出的前512位（64字节）。
 *
 * 这是Ed448ph的预灰化（参见#PSA_ALG_Ed448ph）。对于需要基于SHA3/SHAKE的哈希函数的其他场景，SHA3-512具有相同的输出大小和（理论上）更高的安全强度。
 */
...
/* 在MAC算法的编码中，与#PSA_ALG_MAC_AT_LEAST_THIS_LENGTH_FLAG相对应的位编码该算法是通配符算法的事实。具有作为允许算法策略的通配符算法的密钥可以与对应于相同基类并且具有大于或等于#PSA_ALG_MAC_TRUNCATION_MASK中编码的MAC长度的（可能被截断的）MAC长度的任何算法一起使用。*/
 ...
 *
 * \param mac_alg mac算法标识符（#psa_algorithm_t类型的值，使得#psa_alg_IS_mac（\p mac_a1g）为真）。这可以是截断或未截断的MAC算法。\param mac_length截断mac的期望长度（字节）。这最多必须是MAC的完整长度，并且必须至少是指定的最小实现长度。执行规定的最小值不得为零。
 *
 * \return指定长度的对应MAC算法。\如果\p mac_alg不是受支持的mac算法，或者\p mac_length对于指定的mac算法来说太小或太大，则返回未指定。
 */
 ...
 *
 * \param mac_alg mac算法标识符（#psa_algorithm_t类型的值，使得#psa_alg_IS_mac（\p mac_a1g）为真）。这可以是截断或未截断的MAC算法。
 *
 * \return对应的基本MAC算法。\return如果\p mac_alg不是受支持的mac算法，则未指定。
 */
...
/** MAC算法被截断的长度。
 *
 * \param mac_alg mac算法标识符（#psa_algorithm_t类型的值，使得#psa_alg_IS_mac（\p mac_a1g）为真）。
 *
 * \return截断MAC的长度（字节）。\如果\p mac_alg是非截断mac算法，则返回0。\return如果\p mac_alg不是受支持的mac算法，则未指定。
 */
...
/** 宏以构建MAC最小MAC长度通配符算法。
 *
 * 最小MAC长度MAC通配符算法允许所有MAC算法共享相同的基本算法，并且其中特定算法的（可能被截断的）MAC长度等于或大于通配符算法的最小MAC长度。
 *
 * \注意：当将最小所需MAC长度设置为小于基本算法允许的最小MAC长度时，这实际上成为该基本算法的“允许的任何MAC长度”策略。
 *
 * \param mac_alg mac算法标识符（#psa_algorithm_t类型的值，使得#psa_alg_IS_mac（\p mac_a1g）为真）。\param min_mac_length所需的消息身份验证代码的最小长度（字节）。这最多必须是MAC的未压缩长度，并且必须至少为1。
 *
 * \return指定最小长度的对应MAC通配符算法。\如果\p mac_alg不是受支持的mac算法，或者如果\p min_mac_length小于1或对于指定的mac算法太大，则返回未指定。
 */
...
/* 在AEAD算法的编码中，对应于#PSA_ALG_EAD_AT_EAST_THIS_LENGTH_FLAG的位编码算法是通配符算法的事实。具有作为允许算法策略的通配符算法的密钥可以与对应于相同基类并且具有大于或等于#PSA_ALG_EAD_tag_length_MASK中编码的标签长度的任何算法一起使用。*/
 ...
 *
 * \param aead_alg aead算法标识符（#psa_algorithm_t类型的值，使得#psa_alg_IS_aead（\p aead_alg）为真）。\param tag_length身份验证标签的期望长度（字节）。
 *
 * \return指定长度的相应AEAD算法。\如果\p aead_alg不是受支持的aead算法，或者如果\p tag_length对于指定的aead运算法则无效，则返回未指定。
 */
...
/** 检索指定AEAD算法的标记长度
 *
 * \param aead_alg aead算法标识符（#psa_algorithm_t类型的值，使得#psa_alg_IS_aead（\p aead_alg）为真）。
 *
 * \return输入算法指定的标记长度。\return如果\p aead_alg不是受支持的aead算法，则未指定。
 */
...
/** 使用默认标签长度计算相应的AEAD算法。
 *
 * \param aead_alg aead算法（\c PSA_alg_XXX值，使得#PSA_alg_IS_aead（\p aead_alg）为真）。
 *
...
/** 宏以构建AEAD最小标记长度通配符算法。
 *
 * 最小标签长度AEAD通配符算法允许所有AEAD算法共享相同的基本算法，并且其中特定算法的标签长度等于或大于通配符算法指定的最小标签长度。
 *
 * \注意：当将最小所需标签长度设置为小于基本算法允许的最小标签长度时，这实际上成为该基本算法的“允许的任何标签长度”策略。
 *
 * \param aead_alg aead算法标识符（#psa_algorithm_t类型的值，使得#psa_alg_IS_aead（\p aead_alg）为真）。\param min_tag_length身份验证标签的期望最小长度（字节）。这必须至少为1，并且最多为算法允许的最大标记长度。
 *
 * \return指定最小长度的对应AEAD通配符算法。\如果\p aead_alg不是受支持的aead算法，或者如果\p min_tag_length小于1或对于指定的aead运算法则太大，则返回未指定。
 */
...
/** RSA PSS签名，带有哈希和宽松验证。
 *
 * 此算法在签名时具有与#PSA_AG_RSA_PSS相同的行为，但在验证签名时允许任意盐长度（包括\c 0）。
 *
 * \param hash_alg哈希算法（\c PSA_alg_XXX值，使得#PSA_alg_IS_hash（\p hash_alk）为真）。当在使用策略中指定算法时，这包括#PSA_ALG_ANY_HASH。
 *
 * \return对应的RSA PSS签名算法。\return如果\p hash_alg不是受支持的哈希算法，则未指定。
...
/** 指定的算法是否为带标准盐的RSA PSS。
 *
 * \param alg算法值或算法策略通配符。
 *
 * \如果\p alg的形式为#PSA_AG_RSA_PSS（\c hash_alg），则返回1，其中\c hash_alg是哈希算法或#PSA_alg_ANY_hash。否则为0。如果alg不是受支持的算法标识符或策略，则此宏可能返回0或1。
 */
#define PSA_ALG_IS_RSA_PSS_STANDARD_SALT(alg)                   \
    (((alg) & ~PSA_ALG_HASH_MASK) == PSA_ALG_RSA_PSS_BASE)

/** 指定的算法是否为带任何盐的RSA PSS。
 *
 * \param alg算法值或算法策略通配符。
 *
 * \如果\p alg的形式为#PSA_AG_RSA_PSS_ANY_SALT_BASE（\c hash_alg），则返回1，其中\c hash_alg是哈希算法或#PSA_alg_ANY_hash。否则为0。如果alg不是受支持的算法标识符或策略，则此宏可能返回0或1。
 */
...
/** 指定的算法是否为RSA PSS。
 *
 * 这包括任何RSA PSS算法变体，而不考虑盐长度的限制。
 *
 * \param alg算法值或算法策略通配符。
 *
 * \如果\p alg的形式为#PSA_LG_RSA_PSS（\c hash_alg）或#PSA_AG_RSA_PSS_ANY_SALT_BASE（\c hash_alg），则返回1，其中\c hash_alg是哈希算法或#PS_alg_ANY_hash。否则为0。如果alg不是受支持的算法标识符或策略，则此宏可能返回0或1。
 */
...
/** Edwards曲线数字签名算法，无需预灰化（PureEdDSA），使用标准参数。
 *
 * 此规范的当前版本不支持上下文，因为没有合适的签名接口可以将上下文作为参数。本规范的未来版本可以添加适当的函数，并扩展该算法以支持上下文。
 *
 * PureDDSA需要扭曲的Edwards曲线上的椭圆曲线关键点。本规范支持以下曲线：
 * -#PSA_ECC_FAMILY_TWITED_EDWARDS，255位：RFC 8032中指定的Ed25519。曲线为Edwards25519。内部使用的哈希函数为SHA-512。
 * -#PSA_ECC_FAMILY_TWITED_EDWARDS，448位：如RFC 8032中规定的Ed448。曲线为Edwards448。内部使用的哈希函数是SHAKE256输出的前114个字节。
 *
 * 此算法可以与psa_sign_message（）和psa_verify_message）一起使用。由于没有预散列，因此不能与psa_sign_hash（）或psa_verify_hash）一起使用。
 *
 * 签名格式是RFC 8032§5.1.6和§5.2.6定义的R和S的级联（Ed25519为64字节字符串，Ed448为114字节字符串）。
 */
...
/** Edwards曲线数字签名算法（HashEdDSA），使用SHA-512和Edwards25519曲线。
 *
 * 有关上下文支持和签名格式，请参阅#PSA_ALG_PURE_EDDSA。
 *
 * 该算法是RFC 8032中指定的Ed25519。曲线是Edwards25519。预散列是SHA-512。内部使用的散列函数是SHA-51。
 *
 * 这是一个哈希和签名算法：要计算签名，您可以：
 * -对消息调用psa_sign_message（）；
 * -或使用哈希算法#psa_ALG_SHA_512，使用psa_hash_compute（）或以psa_hhash_setup（）开始的多部分哈希操作计算消息的SHA-512哈希，然后使用psa\usign_hash（）对计算出的哈希进行签名。验证签名类似，使用psa_verify_message（）或psa_vverify_hash（）代替签名函数。
 */
#define PSA_ALG_ED25519PH                               \
    (PSA_ALG_HASH_EDDSA_BASE | (PSA_ALG_SHA_512 & PSA_ALG_HASH_MASK))

/** Edwards曲线数字签名算法（HashEdDSA），使用SHAKE256和Edwards448曲线。
 *
 * 有关上下文支持和签名格式，请参阅#PSA_ALG_PURE_EDDSA。
 *
 * 该算法是RFC 8032中规定的Ed448。曲线是Edwards448。预散列是SHAKE256输出的前64个字节。内部使用的哈希函数是SHAKE256输出的前114个字节。
 *
 * 这是一个哈希和签名算法：要计算签名，您可以：
 * -对消息调用psa_sign_message（）；
 * -或使用哈希算法#psa_ALG_SHAK256_512，使用psa_hash_compute（）或以psa_hhash_setup（）开始的多部分哈希操作计算消息SHAKE256输出的前64个字节，然后使用psa\usign_hash（）对计算出的哈希进行签名。验证签名类似，使用psa_verify_message（）或psa_vverify_hash（）代替签名函数。
 */
#define PSA_ALG_ED448PH                                 \
    (PSA_ALG_HASH_EDDSA_BASE | (PSA_ALG_SHAKE256_512 & PSA_ALG_HASH_MASK))

/* 默认定义，如果库扩展了更多的哈希和签名算法，而我们希望将其排除在头文件之外，则将被重写。*/
#define PSA_ALG_IS_VENDOR_HASH_AND_SIGN(alg) 0

/** 指定的算法是否是可以与psa_sign_hash（）和psa_verify_hash）一起使用的签名算法。
 *
 * 这包括PSA_ALG_IS_hash_and_sign（）分类的所有严格的哈希和符号算法，以及更宽松地遵循范式的算法：
 * -#PSA_LG_RSA_PKCS1V15_SIGN_RAW（期望其输入为编码哈希）
 * -#PSA_LG_ECDSA_ANY（不指定输入的哈希类型）
 *
 * \param alg算法标识符（类型为psa_algorithm_t的值）。
 *
 * \如果alg是可用于签名哈希的签名算法，则返回1。0，如果alg是只能用于签名消息的签名算法。0，如果alg不是签名算法。如果alg不是受支持的算法标识符，则此宏可以返回0或1。
 */
...
/** 指定的算法是否是可以与psa_sign_message（）和psa_verify_message）一起使用的签名算法。
 *
 * \param alg算法标识符（#psa_algorithm_t类型的值）。
 *
 * \如果alg是可用于签名消息的签名算法，则返回1。0，如果\p alg是只能用于对已计算的哈希进行签名的签名算法。如果\p alg不是签名算法，则为0。如果\p alg不是受支持的算法标识符，则此宏可以返回0或1。
 */
...
/** 指定的算法是否为哈希和符号算法。
 *
 * 哈希和签名算法是由两部分组成的非对称（公钥）签名算法：首先以不依赖于密钥的方式计算哈希，然后根据哈希值和密钥计算签名。哈希和符号算法对用于哈希步骤的哈希进行编码，您可以调用#PSA_ALG_SIG_GET_Hash来提取此算法。
 *
 * 因此，对于哈希和符号算法，“psa_sign_message（key，alg，input，…）”等同于“”psa_hash_compute（psa_alg_SIG_GET_hash（alg），input、…，hash，……）；psa_sign_hash（密钥、代数、哈希、…、签名、…）；```最有用的是，将哈希与签名分离允许使用psa_hash_setup（）、psa_hhash_update（）和psa_has_finish（）分多个步骤计算哈希。同样，psa_verify_message（）相当于计算哈希，然后调用psa_vverify_hash（）。
 *
 * \param alg算法标识符（#psa_algorithm_t类型的值）。
 *
...
/** 持久密钥的默认生存期。
 *
 * 持久密钥保留在存储中，直到它被明确销毁或相应的存储区域被擦除。本规范未定义任何擦除存储区域的机制，但集成可以提供自己的机制（例如，执行工厂重置、准备设备翻新或卸载应用程序）。
 *
 * 此生存期值是调用应用程序的默认存储区域。Mbed TLS的集成可能支持其他持久生存期。有关详细信息，请参见：：psa_key_lifetime_t。
 */
...
/** 密钥生存期是否指示密钥是只读的。
 *
 * 无法通过PSA Crypto API创建或销毁只读密钥。它们必须通过绕过API的特定于平台的方法来创建。
 *
 * 某些平台可能提供销毁只读密钥的方法。例如，考虑一个具有多个权限级别的平台，在该平台中，低权限应用程序可以使用密钥，但不允许销毁密钥，并且该平台以只读生存期向应用程序公开密钥。即使应用程序将密钥视为只读，高权限代码也可以销毁密钥。
 *
 * \param lifetime要查询的生存期值（类型为：psa_key_lifetime_t的值）。
 *
 * \如果密钥是只读的，则返回\c 1，否则返回\c 0。
 */
...
/** 密钥是否可用于签署消息。
 *
 * 如果密钥的类型和策略允许，则此标志允许密钥用于MAC计算操作或非对称消息签名操作。
 *
 * 对于密钥对，这涉及私钥。
 */
...
/** 密钥是否可用于验证消息。
 *
 * 此标志允许密钥用于MAC验证操作或非对称消息签名验证操作（如果密钥类型和策略允许）。
 *
 * 对于密钥对，这涉及公钥。
 */
...
/** \defgroup helper_macros助手宏@{
 */

/* 助手宏*/

/** 检查两个AEAD算法标识符是否引用相同的AEAD算法，无论它们编码的标签长度如何。
 *
 * \param aead_alg_1 aead算法标识符。\param aead_alg_2 aead算法标识符。
 *
 * \如果两个标识符都引用相同的AEAD算法，则返回1，否则返回0。如果\n aead_alg_1和\n aead_alg_2都不是受支持的aead算法，则未指定。
 */
