...
/**************************************************************************************
  ...
  ************************************************************************************* RTC内存和存储寄存器使用
  *************************************************************************************
  *     rtc内存地址类型大小用法0x3ff61000（0x50000000）慢size_CP协处理器代码/重置项0x3ff6100+size_C慢8192-size_CP
  *
  *     0x3ff80000（0x400c0000）快速8192深度睡眠进入代码
  *
  ************************************************************************************* RTC存储寄存器用法RTC_CNTL_STORE0_REG保留RTC_CNTL_STORE1_REG RTC_SLOW_CLK校准值RTC_CNTL-STORE2_REG引导时间、低字RTC_CNTI_STORE3_REG引导时间和高字RTC_CNTL_STORE4_REG外部XTAL频率。频率必须均匀，否则将与低位发生冲突，低位用于禁用ROM代码中的日志。RTC_CNTL_STORE5_REG APB总线频率
  *************************************************************************************
  */
...
typedef enum {
    ...
    SW_RESET               =  3,    /**<3，软件重置数字核心*/
    OWDT_RESET             =  4,    /**<4，Legacy看门狗重置数字核心*/
    DEEPSLEEP_RESET        =  5,    /**<3，深度睡眠重置数字核心*/
    SDIO_RESET             =  6,    /**<6，通过SLC模块重置，重置数字核心*/
    ...
    TGWDT_CPU_RESET        = 11,    /**<11，时间组重置CPU*/
    SW_CPU_RESET           = 12,    /**<12，软件重置CPU*/
    RTCWDT_CPU_RESET       = 13,    /**<13，RTC看门狗重置CPU*/
    EXT_CPU_RESET          = 14,    /**<14，对于APP CPU，由PRO CPU重新设定*/
