...
/**
  * @brief  将flash中的图像解压缩为iram和dram，不使用缓存。
  *
  * @param  uint32_t pos：闪存物理地址。
  *
  * @param  uint32_t *entry_addr：可以存储条目代码地址的变量的指针。
  *
  * @param  bool 跳转：是否跳转到函数中的代码。
  *
  * @param  bool config：在解压缩映像时配置闪存，配置只应执行一次。
  *
  * @return ETS_OK：解包成功
  * @return ETS_FAILED:解包失败
  */
ETS_STATUS ets_unpack_flash_code_legacy(uint32_t pos, uint32_t *entry_addr, bool jump, bool config);

/**
  * @brief  将flash中的图像解压缩到iram和dram，使用缓存，可能进行解密。
  *
  * @param  uint32_t pos：闪存物理地址。
  *
  * @param  uint32_t *entry_addr：可以存储条目代码地址的变量的指针。
  ...
  * @param  bool 跳转：是否跳转到函数中的代码。
  *
  * @param  bool sb_need_check：是否进行安全引导检查。
  *
  * @param  bool config：在解压缩映像时配置闪存，配置只应执行一次。
  *
  * @return ETS_OK：解包成功
...
/**
  * @brief  获取rtc模块中校准的xtal_freq/analy_8M*256值。
  *
  * @param  None
  *
...
/**
  ...
  * @param  None
  *
  * @return uint32_t：如果rtc存储值（rtc_STORE5高16位和低16位具有相同值），则从rtc寄存器读取。时钟=（REG_READ（RTC_STORE5）&0xffff）<<12；否则，如果efuse clock中的analog_8M=ets_get_xtal_scale（）15625 ets_efuse_get_8M_clock（）/40；否则时钟=26M。
  */
