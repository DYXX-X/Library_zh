...
/*
ESP32-S2 ROM代码包含一些C库函数的实现。每当ROM中的函数需要使用系统调用时，它都会调用指向以下结构中定义的相应系统调用实现的指针。

默认情况下，表本身不在RAM中分配。有一个指针“syscall_table_ptr_pro”，可以设置为指向CPU 0（也称为pro CPU）的系统调用表的位置。该指针在ROM代码的.bss段中的位置在链接器脚本中定义。
