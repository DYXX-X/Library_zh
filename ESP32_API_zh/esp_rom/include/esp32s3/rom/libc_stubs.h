...
/**
 * @brief ESP32-S3 ROM代码包含一些C库函数的实现。每当ROM中的函数需要使用系统调用时，它都会调用指向以下结构中定义的相应系统调用实现的指针。
 *
 ...
 *
 * 所以，在使用任何C库函数（纯函数和memcpy/memset函数除外）之前，应用程序必须为所使用的每个CPU分配系统调用表结构，并用指向相应系统调用的实际实现的指针填充它。
 *
 */
