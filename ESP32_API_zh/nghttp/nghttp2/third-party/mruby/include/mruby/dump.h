/*
** mruby/dump。h-mruby二进制转储程序（mrbc二进制格式）
**
** 参见mruby.h中的版权声明
*/
...
/* 转储/加载错误代码
 *
 * 注意：MRB_DUMP_GENERAL_FAILURE是由诸如malloc failed之类的未指定问题引起的。
 */
...
struct rite_binary_header {
  uint8_t binary_ident[4];    /* 二进制标识符*/
  uint8_t binary_version[4];  /* 二进制格式版本*/
  uint8_t binary_crc[2];      /* 二进制CRC*/
  uint8_t binary_size[4];     /* 二进制大小*/
  uint8_t compiler_name[4];   /* 编译器名称*/
...
struct rite_section_irep_header {
  ...
  uint8_t rite_version[4];    /* Rite指令规范版本*/
};
