...
#define XCHAL_CA_WRITETHRU		1	/* 缓存启用（写入）模式*/
...
#define XCHAL_CA_WRITETHRU_RW		0	/* 缓存启用（写入）模式（无执行）（FALLACK）*/
 ...
 *	- 必须具有所有可写的VPN和PPN字段；
 *	- 在任何给定时间只能使用一个页面大小（例如，启动时设置），由相应的ITLBCFG或DTLBCFG特殊寄存器选择，其位n4+3..n4索引路径n的页面大小列表（XCHAL_xTLB_SETm_PAGESZ_LOG2_list对应于路径n的集合m）；该列表对于自动重新填充方式可能是稀疏的，因为自动重新填充方法具有与PTE条目共享公共编码的受支持页面大小的独立列表；编码是该列表的索引；列表中给定方式的不支持大小为零；选择不受支持的大小会导致不确定的硬件行为；
