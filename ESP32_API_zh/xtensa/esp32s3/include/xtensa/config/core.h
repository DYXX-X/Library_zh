...
/*  对齐保存区域的一部分，如有必要，将指针置于范围内。用于保存区域加载/存储序列。通常不直接调用。允许任意组合多个（子）序列。保存区域的ptr指针（可能已关闭，请参见.Lxchal_pofs_）minofs，从当前ptr到下一个加载/存储位置的最大偏移范围；minofs<=0<=maxofs（0必须始终是有效的偏移量）范围必须在+/-30kB左右。minofs的对齐粒度…maxofs，（功率为2）（从ptr到下一个加载/存储位置的偏移量限制）
 */
...
.endm
/*
 *  如果可能的话，我们可以优化addi扩展到仅addmi而不是“addmi；addi”。下面是一个部分示例：.set.Lmaxmask，-（\ofalign）&-（\totasign）.if（（（\maxofs）+~.Lmaxmask+1）&0xFFFFFF00）&&（（.Lxchal_ofs_&~.Lmaxmask）==0）.set.Ligmask，0xFFFFFF00.elif…同上，用于负ofs范围….set.Ligmask，0xFFFFF00.set…按偏移调整…else.set.Ligmask，0xFFFFF.endif
 */
...
/*
 *  宏，扩展到存储所选协处理器状态（寄存器等）的函数体。条目：a2=保存区域的指针，保存cp状态a3=协处理器编号退出：任何寄存器a2-a15（？）可能已被清除。
 */
...
/*
 *  宏，扩展到加载所选协处理器状态（寄存器等）的函数体。条目：a2=保存要从中恢复cp状态的区域的ptr a3=协处理器编号退出：任何寄存器a2-a15（？）可能已被清除。
 */
