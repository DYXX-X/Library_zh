 ...
 * 住址h——网络地址的表示
 *
 * 版权所有（C）2010-20112015-2016 Olaf Bergmann<bergmann@tzi.org>
 *
...
/**
 * Returns the port from @p 主机字节顺序中的addr。
 */
...
/**
 * Sets the port field of @p addr到@p端口（按主机字节顺序）。
 */
...
/**Contiki的_coap_address_isany_impl（）的@todo实现*/
...
/**
 * Set the port field of @p addr到@p端口（按主机字节顺序）。
 */
void coap_address_set_port(coap_address_t *addr, uint16_t port);

/**
 * Compares given address objects @p a和@p b.如果
 * addresses are equal, @c 否则为0。参数@p a和@p b不能
 * @c NULL;
 */
...
/**
 * Resets the given coap_address_t object @p addr设置为其默认值。特别是，成员大小必须初始化为存储地址的可用大小。
 *
 * @param addr 要初始化的coap_address_t对象。
 */
...
/**
 * Checks if given address object @p a表示通配符地址。这
 * function returns @c 如果是这种情况，则为1，否则为@c 0。参数@p a不能为@c NULL；
 */
COAP_STATIC_INLINE int
...
/**
 * Checks if given address @p a表示多播地址。此函数
 * returns @c 如果@p a是多播，则为1，否则为@c 0。
 */
