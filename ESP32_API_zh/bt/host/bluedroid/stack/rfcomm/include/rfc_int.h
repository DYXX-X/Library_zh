...
#define LINE_STATUS_OVERRUN        0x02  /* 接收超限错误*/
#define LINE_STATUS_RXPARITY       0x04  /* 接收奇偶校验错误*/
#define LINE_STATUS_FRAME          0x08  /* 接收帧错误*/
#define LINE_STATUS_FAILED         0x10  /* 连接失败*/
...
// btla特定++
#define RFC_T1_TIMEOUT                  20   /* 等待轮询位回复的秒数*/
#define RFC_PORT_T1_TIMEOUT             60   /* 等待具有除MX以外的轮询位的答复的秒数*/
#define RFC_T2_TIMEOUT                  20   /* 等待Mx UIH超时*/
// btla特定--
#define RFC_DISC_TIMEOUT                3    /* 如果出现问题，我们发送DISC，我们不应等待分钟*/
#define RFC_CLOSE_TIMEOUT               10
#define RFCOMM_CONN_TIMEOUT            120   /* 在Mx上建立第一个连接*/
...
typedef struct {
    ...
    tRFC_MCB *p_rfc_lcid_mcb[MAX_L2CAP_CHANNELS];     /* 基于L2CAP的lcid的MCB*/
    BOOLEAN   peer_rx_disabled;              /* 如果为TRUE，对等方发送FCOFF*/
    UINT8     last_mux;                      /* 上次分配的多路复用器*/
    UINT8     last_port;                     /* 上次分配的端口*/
} tRFCOMM_CB;

/* RFCOMM层的主控制块（PORT和RFC）*/
typedef struct {
    tRFCOMM_CB  rfc;
    tPORT_CB    port;
    UINT8       trace_level;
...
/* 当没有DLCI连接打开时，在多路复用器信道上运行计时器*/
#define RFC_MCB_INIT_INACT_TIMER    60  /* 以秒为单位*/

/* 上次DLCI释放后，多路复用器通道上运行的计时器*/
#define RFC_MCB_RELEASE_INACT_TIMER 2   /* 以秒为单位*/
