...
typedef struct per_device_ctb {
    BOOLEAN        in_use;
    BD_ADDR        addr;  /* 主机设备的BD地址*/
    UINT16         attr_mask; /* 0x01—虚拟电缆；0x02—正常可连接；0x03-重新配置初始化；0x04-sdp_disable；*/
    UINT8          state;  /* 处于HOST-KNOWN模式时的设备状态*/
    UINT8          conn_substate;
    UINT8          conn_tries; /* 连接时记住连接尝试次数*/

    tHID_CONN      conn; /* L2CAP信道信息*/
} tHID_HOST_DEV_CTB;

typedef struct host_ctb {
    tHID_HOST_DEV_CTB       devices[HID_HOST_MAX_DEVICES];
    tHID_HOST_DEV_CALLBACK  *callback;             /* 应用程序回调*/
