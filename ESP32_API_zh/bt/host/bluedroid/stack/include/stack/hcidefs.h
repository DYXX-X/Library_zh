/******************************************************************************
 * 版权所有（C）1999-2014 Broadcom Corporation
 *
...
#define HCI_PROTO_VERSION     0x01      /* BT规范1.1版本*/
#define HCI_PROTO_VERSION_1_2 0x02      /* BT规范1.2版本*/
#define HCI_PROTO_VERSION_2_0 0x03      /* BT规范2.0版本*/
#define HCI_PROTO_VERSION_2_1 0x04      /* BT规范2.1版本[里斯本]*/
#define HCI_PROTO_VERSION_3_0 0x05      /* BT规范3.0版本*/
#define HCI_PROTO_VERSION_4_0 0x06      /* BT规范4.0版本*/
#define HCI_PROTO_VERSION_4_1 0x07      /* BT规范4.1版本*/
#define HCI_PROTO_VERSION_4_2 0x08      /* BT规范4.2版本*/
#define HCI_PROTO_REVISION    0x000C    /* 当前实施版本*/
/*
**  HCI组的定义
*/
...
#define HCI_GRP_INFORMATIONAL_PARAMS    (0x04 << 10)            /* 0x1000像素*/
#define HCI_GRP_STATUS_PARAMS           (0x05 << 10)            /* 0x1400像素*/
#define HCI_GRP_TESTING_CMDS            (0x06 << 10)            /* 0x1800像素*/

#define HCI_GRP_VENDOR_SPECIFIC         (0x3F << 10)            /* 0xFC00*/

/* 组占用HCI命令的高6位，其余为操作码本身*/
...
/* HCI_GRP_HOST_CONT_BASEBAND_CMDS的命令*/
...
#define HCI_SHORT_RANGE_MODE                  (0x006B | HCI_GRP_HOST_CONT_BASEBAND_CMDS) /* 仅802.11*/
...
/* HCI_GRP_INFORMATIONAL_PARAMS组的命令*/
...
/*
* |OGF |供应商组|组子代码||1 1 1 1 1 1|0 0 0|X X X X X|已存在|1 1 1 1| 0 0 1|X X X X X| ESP供应商通用HCI命令|1 1 1 11 1|0 1 0|X X X X X|ESP供应商BLE HCI命令|11 1 1 11 1 |1|0 1 1|X X X X|ESP厂商BT HCI命令|1 1 1 2 1 |1|10 0 |X X XX X|保留供将来使用|1 1 1 |1 0 1 | X X X X X X X |保留供将来使用|1 1 1 1 1 | 1 1 0 | X X X X xx X |保留供未来使用|1 1 11 1 |1 | X X X X |保留用于将来使用
*/
...
/* 多adv VSE子码*/
#define HCI_VSE_SUBCODE_BLE_MULTI_ADV_ST_CHG       0x55 /* 多adv实例状态更改*/
...
/* 批处理扫描VSE子代码*/
#define HCI_VSE_SUBCODE_BLE_THRESHOLD_SUB_EVT      0x54 /* 阈值事件*/

/* 跟踪子事件*/
#define HCI_VSE_SUBCODE_BLE_TRACKING_SUB_EVT       0x56 /* 跟踪事件*/
...
/* LE支持的状态*/
/* 支持不可连接的高级状态。0x0000000000000001*/
...
/*支持Scannable Connectable Adv状态。0x0000000000000002*/
...
/* 支持Connectable Adv状态。0x0000000000000004*/
...
/* 支持高占空比定向提前状态。0x0000000000000008*/
...
/* 支持启动状态。0x0000000000000040（或也支持主角色中的连接状态）*/
...
/*还支持从属角色中的连接状态。0x0000000000000080*/
...
/* 支持不可连接的高级状态和被动扫描状态组合。0x0000000000000100*/
...
/*支持可扫描高级状态和被动扫描状态组合。0x0000000000000200*/
...
/*支持可连接的高级状态和被动扫描状态组合。0x0000000000000400*/
...
/*支持高负载Cycl定向ADv和被动扫描状态组合。0x0000000000000800*/
...
/*支持不可连接的高级状态和被动扫描状态组合。0x0000000000001000*/
...
/*支持Scannable Adv状态和Active Scanning state组合。0x0000000000002000*/
...
/*支持可连接的高级状态和活动扫描状态组合。0x0000000000004000*/
...
/*支持High Duty Cycl Directed ADv和ACive Scanning State组合。0x0000000000008000*/
...
/*支持不可连接的高级状态和初始状态组合。0x0000000000010000*/
...
/* 支持可扫描的高级状态和启动状态组合。0x0000000000020000*/
...
/* 支持不可连接的高级状态和主角色组合。0x0000000000040000*/
...
/*支持可扫描的高级状态和主角色组合。0x0000000000040000*/
...
/* 支持不可连接的高级和从属角色组合。0x000000000100000*/
...
/*支持可扫描的高级和从属角色组合。0x000000000200000*/
...
/*支持被动扫描和启动状态组合。0x000000000400000*/
...
/*支持活动扫描和启动状态组合。0x000000000800000*/
...
/*链接层拓扑添加状态组合框*/
/*支持启动状态和主角色组合。还支持主角色和主角色组合。00000010000000美元*/
...
/*低占空比定向广告状态和被动扫描组合。00000040000000美元*/
...
/*低占空比定向广告状态和活动扫描组合。0x0000000080000000*/
...
/* 支持可连接的广告状态和启动状态组合。0x0000000100000000*/
...
/*#define HCI_GENERIC_AMP_LINK_KEY_NOTIF_EVT 0x3E从规范中删除*/
...
#define HCI_VENDOR_SPECIFIC_EVT         0xFF  /* 供应商特定事件*/
#define HCI_NAP_TRACE_EVT               0xFF  /* 定义为0xFE、0xFD，更改为0xFF，因为与TCI_EVT冲突且符合规范*/
...
/*  0x00001FFFFFFFFFFF默认值-无Lisbon事件0x00000800 00000000同步连接完成事件0x00001000 00000000同步链接更改事件0x00002000 00000000嗅探子速率事件0x00004000 00000000扩展查询结果事件0x00008000 00000000加密密钥刷新完成事件0x00010000 00000000 IO功能请求事件0x00020000 00000000 IO能力响应事件0x00040000 00000000用户确认请求事件0x0008000 00000000用户密钥请求事件0x00100000 00000000远程OOB数据请求事件0x00200000 00000000简单配对完成事件0x00400000 00000000通用AMP链路密钥通知事件0x00800000 00000000链路监控超时更改事件0x01000000 00000000增强型刷新完成事件0x0400000000000000用户密钥通知事件0x08000000 00000000按键通知事件0x10000000 00000000远程主机支持的功能通知事件0x20000000 00000000 LE元事件
 */

#define HCI_LE_ONLY_EVENT_MASK              "\x20\x00\x80\x00\x02\x00\x88\x90"
/*  0x0000000000000010断开连接完成事件0x0000000000000080加密更改事件0x0000000000000800读取远程版本信息完成事件0x000000000008000硬件错误事件0x0000000002000000数据缓冲区溢出事件0x0000800000000000加密密钥刷新完成事件0x200000000000000 LE元事件
*/

/* AMP控制器的事件掩码*/
#define HCI_AMP_EVENT_MASK_3_0               "\x00\x00\x00\x00\x00\x00\x3F\xFF"

/*  0x0000000000000000未指定事件（默认值）0x0000000000000001物理链路完成事件0x0000000000000002通道选择事件0x00000000000000004断开物理链路事件0x000000000000000 8物理链路丢失预警事件0x0000000000000010物理链路恢复事件0x00000000000000 20逻辑链路完成事件00000000000000000040断开逻辑链路完成事件0x0000000000000080流规范修改完成事件0x0000000000000100已完成数据块数量事件0x0000000000000200 AMP开始测试事件0x0000000000000400 AMP测试结束事件0x000000000000800 AMP接收器报告事件0x0000000000001000短程模式更改完成事件0x00000000002000 AMP状态更改事件
*/

/* BR/EDR控制器的事件掩码页2（CLB+CSA4）*/
#define HCI_PAGE_2_EVENT_MASK                  "\x00\x00\x00\x00\x00\x7F\xC0\x00"
/*  0x0000000000004000触发的时钟捕获事件0x0000000000008000同步序列完成事件0x0000000000010000同步序列接收事件0x0000000000020000无连接广播接收事件0x00000000040000无连接的广播超时事件0x0000000000080000截断页面完成事件0x00000000100000 Salve页面响应超时事件0x00000000200000无连接广播频道映射更改事件0x0000000000400000查询响应通知事件
*/
...
#define HCI_SCAN_TYPE_INTERLACED        0x01       /* 1.2设备或更高版本*/
...
#define HCI_DO_AUTO_ACCEPT_CONNECT      2   /* 角色切换已禁用*/
#define HCI_DO_AUTO_ACCEPT_CONNECT_RS   3   /* 启用角色切换（1.1勘误表1115）*/
...
#define SLOTS_PER_10MS                  16      /* 10毫秒周期内0.625毫秒时隙*/

/* 最大连接接受超时（0.625msec）*/
#define HCI_MAX_CONN_ACCEPT_TOUT        0xB540  /* 29秒*/
#define HCI_DEF_CONN_ACCEPT_TOUT        0x1F40  /* 5秒*/

/* 页面超时仅在LC中使用，LC正在对不使用操作系统的插槽进行计数*/
#define HCI_DEFAULT_PAGE_TOUT           0x2000  /* 5.12秒（插槽中）*/
...
/* 0.625毫秒内的页面扫描计时器定义*/
#define HCI_MIN_PAGESCAN_INTERVAL       0x12    /* 11.25毫秒*/
#define HCI_MAX_PAGESCAN_INTERVAL       0x1000  /* 2.56秒*/
#define HCI_DEF_PAGESCAN_INTERVAL       0x0800  /* 1.28秒*/

/* 页面扫描窗口的参数传递给LC并保存在插槽中*/
#define HCI_MIN_PAGESCAN_WINDOW         0x11    /* 10.625毫秒*/
#define HCI_MAX_PAGESCAN_WINDOW         0x1000  /* 2.56秒*/
#define HCI_DEF_PAGESCAN_WINDOW         0x12    /* 11.25毫秒*/

/* 0.625毫秒内的查询扫描计时器定义*/
#define HCI_MIN_INQUIRYSCAN_INTERVAL    0x12    /* 11.25毫秒*/
#define HCI_MAX_INQUIRYSCAN_INTERVAL    0x1000  /* 2.56秒*/
#define HCI_DEF_INQUIRYSCAN_INTERVAL    0x1000  /* 2.56秒*/

/* 查询扫描窗口的参数传递给LC并保存在插槽中*/
#define HCI_MIN_INQUIRYSCAN_WINDOW      0x11    /* 10.625毫秒*/
#define HCI_MAX_INQUIRYSCAN_WINDOW      0x1000  /* 2.56秒*/
#define HCI_DEF_INQUIRYSCAN_WINDOW      0x12    /* 11.25毫秒*/
...
#define HCI_DEFAULT_AUTO_FLUSH_TOUT     0       /* 无自动冲洗*/
...
/* 默认链接监控超时*/
#define HCI_DEFAULT_INACT_TOUT          0x7D00  /* BR/EDR（20秒）*/
#define HCI_DEFAULT_AMP_INACT_TOUT      0x3E80  /* AMP（10秒）*/
...
/* 定义所有HCI命令的前导码长度。操作码为2字节，长度为1字节
*/
#define HCIC_PREAMBLE_SIZE      3

/* 定义所有HCI事件的前导码长度。操作码为1字节，长度为1字节
*/
...
/* HCI_BRCM_SET_ACL_PRIORITY的参数信息*/
...
// TODO（zachoverflow）：一旦删除了broadcom特定的黑客，请删除此项
#define LMP_COMPID_BROADCOM             15

/*
** 在数据包头中定义数据包类型，以及一些额外的
*/
...
#define PKT_TYPE_ID     0x10        /* 内部使用的数据包类型*/
...
/* 功能页*/
#define HCI_EXT_FEATURES_PAGE_0     0       /* 扩展功能页0（常规功能）*/
#define HCI_EXT_FEATURES_PAGE_1     1       /* 扩展功能页1*/
#define HCI_EXT_FEATURES_PAGE_2     2       /* 扩展功能页2*/
...
/* 当使用某些av/hfp耳机进行呼叫切换时，此功能会导致频繁的链路中断*/
#define HCI_NON_FLUSHABLE_PB_SUPPORTED(x)      (0)//（（x）[HCI_FFEATURE_NON_LUSHABLE_PB_OFF]和HCI_FEATURE_NON_LUSSABLE_PB_MASK）
