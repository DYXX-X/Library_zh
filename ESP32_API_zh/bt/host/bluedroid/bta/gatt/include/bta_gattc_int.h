...
#else
    #define     BTA_GATTC_CL_MAX    3 // 原点值为10
#endif
#endif
...
#else
    #define     BTA_GATTC_KNOWN_SR_MAX    3 // 原点值为10
#endif
#endif
...
enum {
    BTA_GATTC_IDLE_ST = 0,      /* 闲置的*/
    BTA_GATTC_W4_CONN_ST,       /* 等待连接-（可选）*/
    BTA_GATTC_CONN_ST,          /* 连接状态*/
    BTA_GATTC_DISCOVER_ST       /* 发现正在进行中*/
};
...
#define BTA_GATTC_SERV_DISC_ACT 4
    ...
    list_t              *p_srvc_cache;  /* tBTA_GATTC_SERVICE列表*/
    UINT8               update_count;   /* 收到的指示*/
    UINT8               num_clcb;       /* 相关CLCB的数量*/
    ...
    UINT8               total_char;
    UINT16              total_attr;
    UINT8               srvc_hdl_chg;   /* 服务句柄更改指示待定*/
    UINT16              attr_index;     /* cahce NV保存/加载属性索引*/
...
typedef struct {
    tBTA_GATTC_CBACK        *p_cback;
    BOOLEAN                 in_use;
    tBTA_GATTC_IF           client_if;      /* 具有此应用程序的BTE堆栈的客户端接口*/
    UINT8                   num_clcb;       /* 相关CLCB的数量*/
...
/* 客户端通道是BTA客户端（cl_id）和远程BD地址之间的映射*/
typedef struct {
    UINT16              bta_conn_id;    /* 客户端通道ID，clcb唯一*/
    BD_ADDR             bda;
    tBTA_TRANSPORT      transport;      /* 航道运输*/
    tBTA_GATTC_RCB      *p_rcb;         /* 指向注册CB的指针*/
    tBTA_GATTC_SERV     *p_srcb;    /* 服务器缓存CB*/
    tBTA_GATTC_DATA     *p_q_cmd;   /* 队列中等待执行的命令*/
    list_t              *p_cmd_list; /* 存储要发送的命令的列表*/
    BOOLEAN             is_full;     /* gattc命令队列是否已满*/
#define BTA_GATTC_NO_SCHEDULE       0
#define BTA_GATTC_DISC_WAITING      0x01
#define BTA_GATTC_REQ_WAITING       0x10

    UINT8               auto_update; /* 自动更新正在等待*/
    BOOLEAN             disc_active;
    BOOLEAN             in_use;
