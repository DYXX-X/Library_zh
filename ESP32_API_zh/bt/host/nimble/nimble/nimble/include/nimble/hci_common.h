...
/*
 * LE命令数。注意：这实际上只是用来调整包含LE命令长度的数组的大小。
 */
#define BLE_HCI_NUM_LE_CMDS                 (79)

/* 链路控制命令的OCF列表（OGF=0x01）*/
...
/* 命令特定定义*/
/* ---将控制器设置为主机流量控制（OGF 0x03，OCF 0x0031）---*/
 ...
 * 确定广告客户如何筛选扫描和连接请求。
 *
 *  NONE：无过滤（默认值）。未使用白名单。SCAN：处理所有连接请求，但仅从白名单中扫描。CONN：处理白名单中的所有扫描请求，但仅处理连接请求BOTH：忽略所有扫描和连接请求，除非在白名单中。
 */
...
/* 广告时间间隔*/
#define BLE_HCI_ADV_ITVL                    (625)           /* 用途*/
#define BLE_HCI_ADV_ITVL_MIN                (32)            /* 单位*/
#define BLE_HCI_ADV_ITVL_MAX                (16384)         /* 单位*/
#define BLE_HCI_ADV_ITVL_NONCONN_MIN        (160)           /* 单位*/

#define BLE_HCI_ADV_ITVL_DEF                (0x800)         /* 1.28秒*/
#define BLE_HCI_ADV_CHANMASK_DEF            (0x7)           /* 所有频道*/
...
/* 扫描间隔和扫描窗口计时*/
#define BLE_HCI_SCAN_ITVL                   (625)           /* 用途*/
#define BLE_HCI_SCAN_ITVL_MIN               (4)             /* 单位*/
#define BLE_HCI_SCAN_ITVL_MAX               (16384)         /* 单位*/
#define BLE_HCI_SCAN_ITVL_DEF               (16)            /* 单位*/
#define BLE_HCI_SCAN_WINDOW_MIN             (4)             /* 单位*/
#define BLE_HCI_SCAN_WINDOW_MAX             (16384)         /* 单位*/
#define BLE_HCI_SCAN_WINDOW_DEF             (16)            /* 单位*/

/*
 * 扫描筛选器策略NO_WL:扫描程序处理所有广告数据包（未使用白名单），但未发送到扫描程序的定向可连接广告数据包除外。USE_WL：扫描仪仅处理白名单中的广告。除非包含扫描仪地址，否则将忽略可连接的定向广告。NO_WL_INITA：扫描仪处理所有广告数据包（未使用白名单）。如果InitA是可解析的专用地址，则不应忽略可连接的定向广告。USE_WL_INITA：扫描仪仅处理白名单中的广告。如果InitA是可解析的专用地址，则不应忽略可连接的定向广告。
 */
...
#define BLE_HCI_CONN_ITVL                   (1250)  /* 用途*/
...
#define BLE_HCI_CONN_SPVN_TMO_UNITS         (10)    /* 毫秒*/
