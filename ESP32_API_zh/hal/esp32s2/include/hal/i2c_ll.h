...
#define I2C_LL_INTR_MASK          (0x1ffff) /*!< I2C全中断位图*/
...
// I2C源时钟
#define I2C_LL_CLK_SRC_FREQ(src_clk)  (((src_clk) == I2C_SCLK_REF_TICK) ? 1000*1000 : 80*1000*1000); // 另一个时钟是APB时钟
// I2C最大超时值
 ...
 *
 * @param  hw 外围寄存器的起始地址
 * @param  hight_period I2C SCL高期（APB周期，高期>2）
 * @param  low_period I2C SCL低周期（在APB周期中，low_period>1）
 *
 * @return 没有一个
 */
...
/**
 * @brief 重置I2C主FSM。当主FSM卡住时，调用此函数重置FSM
 *
 * @param  hw 外围寄存器的起始地址
 *
 * @return 没有一个
...
{
    // src_clk：（1）用于APB_clk，（0）用于REF_clk
...
/**
 * @brief  启用I2C内部开漏模式如果I2C模块的内部开漏被禁用，则应将scl和sda-gpio配置为开漏模式。否则不需要。
 *
 * @param  hw 外围寄存器的起始地址
 * @param  internal_od_ena 将true设置为enble内部开放漏极，否则设置为false。
 *
 * @return 没有一个
 */
