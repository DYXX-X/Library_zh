 ...
 * 设置数字控制器的adc转换模式。
 *
 * @param mode 转换模式选择。TODO IDF-3610
 */
...
/**
 * 设置数字控制器的模式表。定义每个SAR ADC的转换规则的模式表。每个表有16个项目，其中存储了信道选择、分辨率和衰减。当转换开始时，控制器从模式表中逐个读取转换规则。对于每个控制器，扫描序列在重复之前具有最多16个不同的规则。
 *
 * @param adc_n ADC单元。
...
static inline void adc_ll_digi_set_pattern_table(adc_ll_num_t adc_n, uint32_t pattern_index, adc_digi_pattern_config_t table)
{
    ...
    if (table.unit == ADC_NUM_1) {
        tab = APB_SARADC.sar1_patt_tab[index];  // 读取旧寄存器值
        tab &= (~(0xFF000000 >> offset));       // 清除旧数据
        tab |= ((uint32_t)pattern.val << 24) >> offset; // 填写新数据
        APB_SARADC.sar1_patt_tab[index] = tab;  // 回写
    } else { // adc_n==adc_NUM_2
        tab = APB_SARADC.sar2_patt_tab[index];  // 读取旧寄存器值
        tab &= (~(0xFF000000 >> offset));       // 清除旧数据
        tab |= ((uint32_t)pattern.val << 24) >> offset; // 填写新数据
        APB_SARADC.sar2_patt_tab[index] = tab;  // 回写
    }
}
 ...
 *
 * @note 触发间隔应大于SAR ADC的采样时间。
 * @param cycle 测量的时钟周期（触发间隔）。范围：40~4095。
 */
...
/**
 * 设置ADC数字控制器时钟分频因子。从“APLL”或“APB”时钟分频的时钟。表达式：controller_clk=（`APLL`或`APB`）/（div_num+div_a/div_b+1）。
 *
...
{
    SENS.sar_meas2_mux.sar2_rtc_force = 0;  // 在唤醒模式下启用仲裁器
...
static inline void adc_ll_vref_output(adc_ll_num_t adc, adc_channel_t channel, bool en)
{
    ...
    if (en) {
        ...
        //设置sar2 en力
        SENS.sar_meas2_ctrl2.sar2_en_pad_force = 1;      //由软件控制的填充位图
        //为ADC2通道设置en_pad（位0x380）
        ...
        //设置sar2 en力
        SENS.sar_meas2_ctrl2.sar2_en_pad_force = 0;      //由软件控制的填充位图
        //为ADC2通道设置en_pad（位0x380）
