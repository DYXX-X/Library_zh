...
{
    hw->command_reg.val = 0x12; //同时为单次自接收请求设置command_reg.srr和command_reg.at
}
 ...
 *
 * @note 必须在重置模式下调用
 * @note ESP32S2 brp可以是2到32768之间的任意偶数
 */
 ...
 * @return REC值
 *
 * @note REC在复位模式下未冻结。仅听模式将使其冻结。BUS OFF（总线断开）条件自动将REC设置为0。
 */
