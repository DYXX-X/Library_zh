...
/**
 * @brief 启用TX PDM模式。
 * @note  ESP32-S2不支持pdm此功能用于与支持pdm的功能兼容
 *
 * @param hw 外围I2S硬件实例地址（忽略）
 * @param pdm_enable 设置为真以启用TX PDM模式（忽略）
 */
 ...
 *
 * @param hw 外围I2S硬件实例地址（忽略）
 * @param pdm_enable 将真设置为RX启用PDM模式（忽略）
 */
...
/**
 * @brief 配置ESP32-S2上的TX通道位和音频数据位，sample_bit应等于data_bit
 *
 * @param hw 外围I2S硬件实例地址。
...
/**
 * @brief 配置ESP32-S2上的RX通道位和音频数据位，sample_bit应等于data_bit
 *
 * @param hw 外围I2S硬件实例地址。
 * @param chan_bit chan位宽度
 * @param data_bit 音频数据位宽
...
/**
 * @brief 将I2S tx dma设置为相等
 *
 * @param hw 外围I2S硬件实例地址。
 * @param val 设置tx-dma等于的值
 */
