...
#define RMT_LL_MAX_LOOP_COUNT           (1023)/*!< 硬件支持的最大循环计数*/

#define RMT_LL_HW_BASE  (&RMT)
#define RMT_LL_MEM_BASE (&RMTMEM)

// 注：在LL驱动程序中，TX和RX信道编号都是从零开始的索引
// i、 e.tx_channel属于[0,2]，rx_channel是[0,2]

static inline void rmt_ll_enable_drive_clock(rmt_dev_t *dev, bool enable)
{
    dev->sys_conf.clk_en = enable; // 寄存器时钟门控
    dev->sys_conf.mem_clk_force_on = enable; // 存储器时钟门控
}
...
static inline bool rmt_ll_is_mem_power_down(rmt_dev_t *dev)
{
    // RTC域也可以关闭RMT内存
    // 所以它可能不足以检测它是否断电
    // mem_force_pd的优先级高于mem_force_pru
...
{
    // 公式：rmt_sclk=module_clock_src/（1+div_num+div_a/div_b）
...
{
    // 如果编译器将32位指令（例如s32i）优化为两个16位指令（如s16i，不允许访问寄存器）
    // 我们自己负责“读-修改-写”过程。
...
// 设置为真，使载波处于所有RMT状态（空闲、读取、发送）
// 设置为false，仅在发送状态下启用载波（即RAM中有有效数据要发送）
