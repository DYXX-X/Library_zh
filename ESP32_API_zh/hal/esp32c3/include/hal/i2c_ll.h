...
// I2C源时钟
#define I2C_LL_CLK_SRC_FREQ(src_clk)  (((src_clk) == I2C_SCLK_RTC) ? 20*1000*1000 : 40*1000*1000); // 另一个时钟是XTAL时钟
// rtc_clk打开后的延迟时间
...
static inline void i2c_ll_cal_bus_clk(uint32_t source_clk, uint32_t bus_freq, i2c_clk_cal_t *clk_cal)
{
    ...
    int scl_wait_high = (bus_freq <= 50000) ? 0 : (half_cycle / 8); // 补偿频率>50K时的时间
 ...
 *
 * @param  hw 外围寄存器的起始地址
 * @param  hight_period I2C SCL高周期（在核心时钟周期中，high_period>2）
...
static inline void i2c_ll_master_clr_bus(i2c_dev_t *hw)
{
    ...
    hw->ctr.conf_upgate = 1;
    // hardhard将在发送scl脉冲之后清除scl_ rst_，
    // 我们应该设置confupgate位来同步寄存器值。
