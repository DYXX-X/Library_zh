...
/*
 * 当引脚18和引脚19被设置为用作GPIO时，以下定义用于禁用USB JTAG。请参见下面的gpio_pad_select_gpio（）。
 *
 * TODO:在合并USB设备寄存器定义后删除这些定义。
 */
...
{
    if (core_id == 0) {
        GPIO.pin[gpio_num].int_ena = GPIO_LL_PRO_CPU_INTR_ENA;     //启用pro cpu intr
    } else {
        // GPIO引脚[GPIO_num]。int_ena=GPIO_APP_CPU_INTR_ena//启用pro cpu intr
    }
}
