...
/* PTP寄存器位*/
#define EMAC_LL_DMAPTPRXDESC_PTPMT_SYNC 0x00000100U                      /* SYNC消息（所有时钟类型）*/
#define EMAC_LL_DMAPTPRXDESC_PTPMT_FOLLOWUP 0x00000200U                  /* 后续消息（所有时钟类型）*/
#define EMAC_LL_DMAPTPRXDESC_PTPMT_DELAYREQ 0x00000300U                  /* 延迟请求消息（所有时钟类型）*/
#define EMAC_LL_DMAPTPRXDESC_PTPMT_DELAYRESP 0x00000400U                 /* 延迟响应消息（所有时钟类型）*/
#define EMAC_LL_DMAPTPRXDESC_PTPMT_PDELAYREQ_ANNOUNCE 0x00000500U        /* PdelayReq消息（对等透明时钟）或公告消息（普通或边界时钟）*/
#define EMAC_LL_DMAPTPRXDESC_PTPMT_PDELAYRESP_MANAG 0x00000600U          /* PdelayResp消息（对等透明时钟）或管理消息（普通或边界时钟）*/
#define EMAC_LL_DMAPTPRXDESC_PTPMT_PDELAYRESPFOLLOWUP_SIGNAL 0x00000700U /* PdelayRespFollowUp消息（对等透明时钟）或信令消息（普通或边界时钟）*/

#define EMAC_LL_DMAPTPRXDESC_IPPT_UDP 0x00000001U  /* 封装在IP数据报中的UDP负载*/
#define EMAC_LL_DMAPTPRXDESC_IPPT_TCP 0x00000002U  /* 封装在IP数据报中的TCP有效负载*/
#define EMAC_LL_DMAPTPRXDESC_IPPT_ICMP 0x00000003U /* 封装在IP数据报中的ICMP有效载荷*/

#define EMAC_LL_DMADESC_OWNER_CPU  (0)
#define EMAC_LL_DMADESC_OWNER_DMA  (1)

/* 中断标志（指emac_dma_struct.h中的dmastatus寄存器）*/
...
/* 中断启用（指emac_dma_struct.h中的dmain_en寄存器）*/
...
/* 启用所需的中断（必须启用recv/recv_buf_unavailable/normal才能使eth工作）*/
...
static inline void emac_ll_clock_enable_rmii_input(emac_ext_dev_t *ext_regs)
{
    ...
    /* phy的ref clk以rmii模式输入，clk可以由mac层或外部晶体提供。配置pin作为输出，通过esp32 mac层生成ref clk或输入，从外部晶体获得时钟*/
