...
/**
 * 一些错误的解决方案需要对外围设备进行硬件重置。因此，某些寄存器必须在复位前保存，然后在复位后恢复。此结构用于保存其中一些寄存器。
 */
typedef struct {
    uint8_t mode_reg;
...
{
    hw->command_reg.val = 0x03;     //同时写入TR和AT
}
...
static inline void twai_ll_set_enabled_intrs(twai_dev_t *hw, uint32_t intr_mask)
{
    ...
    //ESP32 Rev 2或更高版本具有brp div字段。需要掩盖它
 ...
 *
 * @note 必须在重置模式下调用
 * @note ESP32 rev 2或更高版本可以通过设置brp_div位来支持x2 brp，允许brp从最大值128变为256。
 */
                                             ...
                                             uint8_t *data, uint32_t *flags)
{
    ...
    *flags = flags_temp;
    ...
    if (rx_frame->frame_format) {
        ...
        id_temp = HAL_SWAP32(id_temp) >> 3;  //（（字节[i]<<8*（3-i））>>3）
        *id = id_temp & TWAI_EXTD_ID_MASK;
        ...
        id_temp = HAL_SWAP16(id_temp) >> 5;  //（（字节[i]<<8*（1-i））>>5）
 ...
 *
 * 配置CLKOUT。CLKOUT是APB CLK的预缩放版本。除法器可以是1，也可以是2到14之间的任何偶数。将除法器设置为0以禁用CLKOUT。
 *
 * @param hw TWAI寄存器的起始地址
 * @param divider CLKOUT分频器。设置为0以禁用CLKOUT
 */
...
/**
 * @brief   将寄存器地址映射设置为扩展模式
 *
 * 扩展模式寄存器地址映射由更多寄存器和额外功能组成。
 *
 * @param hw TWAI寄存器的起始地址
 *
 * @note 必须在设置任何配置之前调用
...
#if defined(CONFIG_TWAI_ERRATA_FIX_RX_FRAME_INVALID) || defined(CONFIG_TWAI_ERRATA_FIX_RX_FIFO_CORRUPT)
/**
 * @brief   保存TWAI控制器寄存器的当前值
 *
 * 此功能保存TWAI控制器的一些寄存器的当前值，为控制器的硬件重置做准备。
 *
 * @param hw TWAI寄存器的起始地址
 * @param reg_save 指向存储寄存器值的结构的指针
 * @note 必须在重置模式下调用，以便可以访问配置寄存器。
 * @note 某些寄存器在进入重置模式时被清除，因此必须与此功能分开保存。
 */
...
/**
 * @brief   恢复TWAI控制器寄存器的先前值
 *
 * 此功能在控制器硬件复位后恢复一些TWAI控制器寄存器的先前值。
 *
 * @param hw TWAI寄存器的起始地址
 * @param reg_save 指向存储要还原的寄存器值的结构的指针
 * @note 必须在重置模式下调用，以便可以访问配置寄存器
 * @note 某些寄存器是只读的，因此无法恢复
 */
