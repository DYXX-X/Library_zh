...
//支持的时钟寄存器值
#define SPI_FLASH_LL_CLKREG_VAL_5MHZ    ((spi_flash_ll_clock_reg_t){.val=0x0000F1CF})   ///<时钟设置为5 MHz
#define SPI_FLASH_LL_CLKREG_VAL_10MHZ   ((spi_flash_ll_clock_reg_t){.val=0x000070C7})   ///<时钟设置为10 MHz
#define SPI_FLASH_LL_CLKREG_VAL_20MHZ   ((spi_flash_ll_clock_reg_t){.val=0x00003043})   ///<时钟设置为20 MHz
#define SPI_FLASH_LL_CLKREG_VAL_26MHZ   ((spi_flash_ll_clock_reg_t){.val=0x00002002})   ///<时钟设置为26 MHz
#define SPI_FLASH_LL_CLKREG_VAL_40MHZ   ((spi_flash_ll_clock_reg_t){.val=0x00001001})   ///<时钟设置为40 MHz
#define SPI_FLASH_LL_CLKREG_VAL_80MHZ   ((spi_flash_ll_clock_reg_t){.val=0x80000000})   ///<时钟设置为80 MHz

///通过主机ID获取SPI外围寄存器的起始地址
...
/**
 * 擦除扇区，地址应由spi_flash_ll_set_address设置。
 *
 * @param dev 外围寄存器的起始地址。
 */
...
/**
 * 擦除块，地址应设置为spi_flash_ll_set_address。
 *
 * @param dev 外围寄存器的起始地址。
 */
...
/**
 * 完成“spi_flash_ll_read”后，从缓冲区获取读取数据。
 *
 * @param dev 外围寄存器的起始地址。
...
/**
 * 编程闪存芯片的一页。在此之前调用``spi_flash_ll_set_address``，将地址设置为程序。
 *
 * @param dev 外围寄存器的起始地址。
...
/**
 * 设置要在用户命令模式下发送的地址。应在需要地址的命令之前调用，例如擦除扇区、读取、写入。。。
 *
 * @param dev 外围寄存器的起始地址。
 * @param addr 要发送的地址
 */
