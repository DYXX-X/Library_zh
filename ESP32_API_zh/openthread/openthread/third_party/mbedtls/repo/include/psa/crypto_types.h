 ...
 * -位8-31（#PSA_KEY_LEIFETIME_GET_LOCATION（\c生存期））：位置指示符。该值指示密钥的存储位置以及对密钥执行操作的位置。有关详细信息，请参阅：：psa_key_location_t。
 *
 * 当应用程序实例终止或设备电源重置时，易失性密钥会自动销毁。持久密钥将被保留，直到应用程序显式销毁它们或发生特定于实现的设备管理事件（例如，工厂重置）。
 *
 * 持久密钥的密钥标识符类型为#mbedtls_svc_key_id_t。即使创建密钥的应用程序实例终止，该标识符在密钥的整个生命周期内仍然有效。应用程序可以调用psa_open_key（）来打开之前创建的持久密钥。
 *
 * 本规范定义了两个基本寿命值：
 * -寿命为#PSA_KEY_lifetime_VOLATILE的密钥是易失性的。所有实现都应该支持此生命周期。
 * -具有生存期#PSA_KEY_lifetime_PERSISTENT的密钥是持久的。所有能够访问具有适当安全保证的持久性存储的实施都应该支持此生命周期。
 */
 ...
 * -\c 1=#PSA_KEY_PERSISTENCE_DEFAULT：具有默认生存期的持久密钥。如果实现完全支持持久密钥，则应支持此值。如果应用程序没有只有特定于实现的功能才能满足的特定需求，则应使用此值。
 * -\c 2-127:具有PSA指定生存期的持久密钥。PSA加密规范没有定义这些值的含义，但其他PSA规范可能会这样做。
 * -\c 128-254:具有供应商指定生存期的持久密钥。没有PSA规范将定义这些值的含义，因此实现可以自由选择其含义。作为一个指导原则，较高的持久性级别应该会导致密钥比较低级别的密钥在更多的管理事件中存活。
 * -\c 255=#PSA_KEY_PERSISTENCE_READ_ONLY：只读或一次写入密钥。无法销毁具有此持久性级别的密钥。支持这种密钥的实现可以允许通过PSA Cryptography API创建密钥，最好只向具有适当权限的应用程序创建，或者只公开通过特定于实现的方式（例如工厂ROM雕刻过程）创建的密钥。请注意，由于策略限制而非物理限制而只读的密钥不应具有此持久性级别。
 *
...
/** 关键位置指示器的编码。
 *
 * 如果此API的实现可以调用外部密码处理器（如安全元素），则密钥的位置指示哪个安全元素对密钥执行操作。如果一个实现为持久性存储提供了多个物理位置，则位置指示符将反映密钥存储在哪个物理位置。
 *
 * 本规范定义了以下位置指示器值：
 * -\c 0：主本地存储。所有实现都应支持此值。主本地存储通常是包含密钥元数据的同一存储区域。
 * -\c 1：主要安全元素。如果操作环境附加了安全元素，则实现应支持此值。作为指导原则，安全元件可以提供比主本地存储更高的抗侧信道和物理攻击的能力，但可能对支持的密钥类型、大小、策略和操作有限制，并且可能具有不同的性能特征。
 * -\c 2-0x7fffff：PSA规范定义的其他位置。PSA Cryptography API目前没有为这些位置指定任何含义，但本规范或其他PSA规范的未来版本可能会这样做。
 ...
 * -实现可以在#PSA_key_ID_VENDOR_MIN到#PSA_key _ID_VENDOR_MAX的范围内定义额外的密钥标识符。
 * -0被保留为无效的密钥标识符。
 * -这些范围之外的密钥标识符保留供将来使用。
 */
 ...
 *
 * 属性结构可以是简单的数据结构，其中每个函数“psa_set_key_xxx”设置一个字段，而相应的函数“psaquet_key_xx”检索相应字段的值。然而，实现可能会报告与原始值等效但具有不同编码的值。例如，实现可以对许多位模式无效或不受支持的类型使用更紧凑的表示，并将其不支持的所有值存储为特殊标记值。在这种实现中，在设置无效值之后，相应的get函数返回一个无效值，该值可能不是最初存储的值。
 *
