 ...
 *
 * \retval#PSA_SUCCESS\retval#PSA_ERROR_INSUFFIE_MEMORY\retval#PSA_ERROR_COMMUNICATION_FAILURE\retval#PSA_ERROR_HARDWARE_FAILURE \retval#PSA错误_CORRUPTION_DETECTED \retval#1PSA错误_INSUFFICE_ENTROPY
 */
 ...
 *
 * \param[out]attributes要写入的属性结构。\param owner_id密钥所有者标识符。
 */
 ...
 *
 * \retval#PSA_SUCCESS\retval#PSTA_ERROR_INVALID_HANDLE\retval#PSA_ERROR_INSUFFIE_MEMORY\retval#PSA_ERROR_COMMUNICATION_FAILURE\retval#PSA_ERROR_CORRUPTION_DETECTED\retval#1PSA_ERROR_STORAGE_FAILURE \retval#8PSA_ERROR_BAD_STATE该库以前未由PSA_crypto_init（）初始化。初始化失败是否导致此错误代码取决于实现。
 */
 ...
 *
 * \retval#PSA_SUCCESS\p密钥是一个有效标识符，它所指的密钥材料已被擦除。或者，\p键是\c 0。\retval#PSA_ERROR_NOT_PERMITTED由于策略或物理限制，无法擦除该键，因为它是只读的。\retval#PSA_ERROR_INVALID_HANDLE\p密钥不是有效标识符，也不是0。\retval#PSA _ERROR_COMMUNICATION_FAILURE与密码处理器通信失败。密钥材料可能仍然存在于密码处理器中。\retval#PSA_ERROR_STORAGE_FAILURE存储已损坏。即使在这个阶段，实施也应尽最大努力擦除关键材料，但应用程序应意识到，在这种情况下，可能无法保证关键材料不可恢复。\retval#PSA_ERROR_CORRUPTION_DETECTED发生了非存储损坏或通信故障的意外情况。密码处理器可能已被破坏。\retval#PSA_ERROR_BAD_STATE该库以前未由PSA_crypto_init（）初始化。初始化失败是否导致此错误代码取决于实现。
 */
 ...
 * -对于椭圆曲线公钥（#PSA_key_TYPE_IS_ECC_public_key为真的密钥类型），格式为SEC1&sect；2.3.3作为ECPoint的内容。设“m”是与曲线相关联的比特大小，即“F_q”上的曲线的“q”的比特大小。该陈述包括：
 *      -字节0x04；
 ...
 *
 * \retval#PSA_SUCCESS\retval#PSA错误_INVALID_HANDLE\retval#PSA_ERROR_INVALID_ARGUMENT密钥既不是公钥也不是密钥对。\retval#PSA_ERROR_NOT_SUPPORTED\retval#PSA_ERROR_BUFFER_TOO_SMALL数据缓冲区的大小太小。您可以通过调用#PSA_KEY_EXPORT_MAX_size（#PSA_KEY_TYPE_PUBLIC_KEY_OF_KEY_PAIR（\c类型），\c位）来确定足够的缓冲区大小，其中\c类型是密钥类型，\c位数是以位为单位的密钥大小。\retval#PSA_ERROR_COMMUNICATION_FAILURE\retval#PSA_ERROR_HARDWARE_FAILUE\retval#PSA _ERROR_CORRUPTION_DETECTED \retval#PSA_ERROR_STORAGE_FAILURE \retval#1PSA_ERROR_INSUFFIE_MEMORY\retval#PSA_RROR_BAD_STATE该库以前未由PSA_crypto_init（）初始化。初始化失败是否导致此错误代码取决于实现。
 */
 ...
 *
 * \param alg要计算的哈希算法（\c PSA_alg_XXX值，使得#PSA_alg_IS_hash（\p alg）为真）。\param[in]输入缓冲区，包含要哈希的消息。\param input_length输入缓冲区的大小（字节）。\param[out]hash要写入哈希的缓冲区。\param hash_size \p哈希缓冲区的大小（字节）。\param[out]hash_length成功时，构成哈希值的字节数。这始终是#PSA_HAH_SIZE（\p alg）。
 *
 ...
 *
 * \param[in，out]操作活动哈希操作。\param[out]hash要写入哈希的缓冲区。\param hash_size \p哈希缓冲区的大小（字节）。\param[out]hash_length成功时，构成哈希值的字节数。这始终是#PSA_HAH_SIZE（\c alg），其中\c alg是计算的哈希算法。
 *
 * \retval#PSA_SUCCESS成功。\retval#PSA_ERROR_BAD_STATE操作状态无效（必须为活动状态）。\retval#PSA_ERROR_BUFFER_TOO_SMALL哈希缓冲区的大小太小。您可以通过调用#PSA_HAH_size（\c alg）来确定足够的缓冲区大小，其中\c alg是计算的哈希算法。\retval#PSA_ERROR_INSUFFIE_MEMORY\retval#PSTA_ERROR_COMMUNICATION_FAILURE\retval#PSA_ERROR_HARDWARE_FAILURE \retval#PSA_ERROR_CORRUPTION_DETECTED \retval#PSA_RROR_BAD_STATE该库以前未由PSA_crypto_init（）初始化。初始化失败是否导致此错误代码取决于实现。
 */
 ...
 *
 * \param[in，out]操作活动MAC操作。\param[out]mac要写入mac值的缓冲区。\param mac_size \p mac缓冲区的大小（字节）。\param[out]mac_length成功时，构成mac值的字节数。这始终是#PSA_MAC_FINAL_SIZE（\c key_type，\c key_bits，\c alg），其中\c keytype和\c key_bits分别是密钥的类型和比特大小，\c alg是计算的MAC算法。
 *
 * \retval#PSA_SUCCESS成功。\retval#PSA_ERROR_BAD_STATE操作状态无效（必须是活动的mac符号操作）。\retval#PSA_ERROR_BUFFER_TOO_SMALL mac缓冲区的大小太小。您可以通过调用PSA_MAC_FINAL_size（）来确定足够的缓冲区大小。\retval#PSA_ERROR_INSUFFIE_MEMORY\retval#PSTA_ERROR_COMMUNICATION_FAILURE\retval#PSA_ERROR_HARDWARE_FAILURE \retval#1PSA_ERROR_CORRUPTION_DETECTED \retval#2PSA_ERROR_STORAGE_FAILUE\retval#8PSA_ERROR_BAD_STATE库以前未由PSA_crypto_init（）初始化。初始化失败是否导致此错误代码取决于实现。
 */
...
/** 处理经过身份验证的加密操作。
 *
 * \param key用于操作的密钥的标识符。它必须允许使用#PSA_KEY_usage_ENCRYPT。\param alg要计算的AEAD算法（\c PSA_alg_XXX值，使得#PSA_alg_IS_AEAD（\p alg）为真）。\param[in]要使用的nonce或IV。\param nonce_length nonce缓冲区的大小（字节）。\param[in]additional_data将进行身份验证但未加密的其他数据。\param additional_data_length additional_data的大小（字节）。\param[in]明文将被验证和加密的数据。\param plaintext_length明文的大小（字节）。\param[out]密文已验证和加密数据的输出缓冲区。附加数据不是此输出的一部分。对于加密数据和认证标签被定义为单独输出的算法，认证标签被附加到加密数据。\param ciphertext_size密文缓冲区的大小（字节）。此值必须至少为#PSA_EAD_ENCRYPT_OUTPUT_SIZE（\p alg，\p明文长度）。\param[out]密文_length成功时，密文缓冲区中输出的大小。
 *
 * \retval#PSA_SUCCESS成功。\retval#PSA_ERROR_INVALID_HANDLE\retval#PSA_ERROR_NOT_PERMITTED\retval#PSA_ERROR_INVALID_ARGUMENT\p密钥与\p alg不兼容。\retval#PSA_ERROR_NOT_SUPPORTED\p alg不受支持或不是AEAD算法。\retval#PSA_ERROR_INSUFFIE_MEMORY\retval#PSTA_ERROR_BUFFER_TOO_SMALL\p密文_大小太小\retval#PSA_ERROR_COMMUNICATION_FAILURE\retval#PSA_ERROR_HARDWARE_FAILUE\retval#PSA_RROR_CORRUPTION_DETECTED\retval#1PSA_ERROR_STORAGE_FAILURE \retval#2PSA_ERROR_BAD_STATE库以前未由PSA_crypto_init（）初始化。初始化失败是否导致此错误代码取决于实现。
 */
...
/** 处理经过身份验证的解密操作。
 *
 * \param key用于操作的密钥的标识符。它必须允许使用#PSA_KEY_usage_DECRYPT。\param alg要计算的AEAD算法（\c PSA_alg_XXX值，使得#PSA_alg_IS_AEAD（\p alg）为真）。\param[in]要使用的nonce或IV。\param nonce_length nonce缓冲区的大小（字节）。\param[in]additional_data已验证但未加密的其他数据。\param additional_data_length additional_data的大小（字节）。\param[in]密文已验证和加密的数据。对于加密数据和认证标签被定义为单独输入的算法，缓冲区必须包含加密数据，然后是认证标签。\param ciphertext_length密文的大小（字节）。\param[out]明文解密数据的输出缓冲区。\param plaintext_size明文缓冲区的大小（字节）。这必须至少为#PSA_EAD_DECRYPT_OUTPUT_SIZE（\p alg，\p密文长度）。\param[out]plaintext_length成功时，\p明文缓冲区中输出的大小。
 *
 * \retval#PSA_SUCCESS成功。\retval#PSA_ERROR_INVALID_HANDLE\retval#PSA_ERROR_INVALID_SIGNATURE密文不可信。\retval#PSA_ERROR_NOT_PERMITTED\retval#PSTA_ERROR_INVALID_ARGUMENT\p密钥与\p alg不兼容。\retval#PSA_ERROR_NOT_SUPPORTED\p alg不受支持或不是AEAD算法。\retval#PSA_ERROR_INSUFFIE_MEMORY\retval#PSA_ERROR_BUFFER_TOO_SMALL\p明文_大小或\p nonce_length太小\retval#PSA_ERROR_COMMUNICATION_FAILURE\retval#PSA _ERROR_HARDWARE_FAILURE \retval#PSA_RROR_CORRUPTION_DETECTED\retval#1PSA_ERROR_STORAGE_FAILUE\retval#2PSA_ERROR_BAD_STATE此库以前未由PSA_crypto_init（）初始化。初始化失败是否导致此错误代码取决于实现。
 */
 ...
 *
 * \param[in，out]操作激活AEAD操作。\param[in]输入缓冲区，包含要加密或解密的消息片段。\param input_length输入缓冲区的大小（字节）。\param[out]输出要写入输出的缓冲区。\param output_size输出缓冲区的大小（字节）。这必须至少是#PSA_EAD_UPDATE_OUTPUT_SIZE（\c alg，\p input_length），其中\c ald是正在计算的算法。\param[out]output_length成功时，构成返回输出的字节数。
 *
 * \retval#PSA_SUCCESS成功。\retval#PSA_ERROR_BAD_STATE操作状态无效（它必须是活动的，必须设置随机数，如果算法需要，还必须设置长度）。\retval#PSA_ERROR_BUFFER_TOO_SMALL输出缓冲区的大小太小。您可以通过调用#PSA_EAD_UPDATE_OUTPUT_size（\c alg，\p input_length）来确定足够的缓冲区大小，其中\c ald是正在计算的算法。\retval#PSA_ERROR_INVALID_ARGUMENT到目前为止，PSA_aead_update_ad（）的输入总长度小于先前使用PSA_aeaad_set_length（）指定的附加数据长度。\retval#PSA_ERROR_INVALID_ARGUMENT总输入长度溢出先前使用PSA_aead_set_length（）指定的明文长度。\retval#PSA_ERROR_INSUFFIE_MEMORY\retval#PSTA_ERROR_COMMUNICATION_FAILURE\retval#PSA_ERROR_HARDWARE_FAILURE \retval#1PSA_ERROR_CORRUPTION_DETECTED \retval#2PSA_ERROR_STORAGE_FAILUE\retval#8PSA_ERROR_BAD_STATE库以前未由PSA_crypto_init（）初始化。初始化失败是否导致此错误代码取决于实现。
 */
 ...
 * -\p标记包含身份验证标记。其长度始终为#PSA_EAD_TAG_length（\c alg），其中\c alg是运算执行的AEAD算法。
 *
 * 当此函数成功返回时，操作将变为非活动状态。如果此函数返回错误状态，则操作将进入错误状态，必须通过调用psa_aead_abort（）中止。
 *
 * \param[in，out]操作激活AEAD操作。\param[out]密文缓冲区，其中要写入密文的最后一部分。\param ciphertext_size密文缓冲区的大小（字节）。该值必须至少为#PSA_EAD_FINISH_OUTPUT_SIZE（\c alg），其中\c alh是正在计算的算法。\param[out]密文_length成功时，返回密文的字节数。\param[out]tag要写入身份验证标记的缓冲区。\param tag_size \p标记缓冲区的大小（字节）。这必须至少是#PSA_EAD_TAG_LENGTH（\c alg），其中\c alh是正在计算的算法。\param[out]tag_length成功时，构成返回标记的字节数。
 *
 * \retval#PSA_SUCCESS成功。\retval#PSA_ERROR_BAD_STATE操作状态无效（它必须是设置了随机数的活动加密操作）。\retval#PSA_ERROR_BUFFER_TOO_SMALL密文或标记缓冲区的大小太小。您可以通过调用#PSA_EAD_FISH_OUTPUT_size（\c alg）为密文确定足够的缓冲区大小，其中\c alh是正在计算的算法。您可以通过调用#PSA_EAD_tag_LENGTH（\c alg）来确定\p标记的足够缓冲区大小。\retval#PSA_ERROR_INVALID_ARGUMENT到目前为止，PSA_aead_update_ad（）的输入总长度小于先前使用PSA_aeaad_set_length（）指定的附加数据长度。\retval#PSA_ERROR_INVALID_ARGUMENT到目前为止，PSA_aed_update（）的输入总长度小于之前使用PSA_aad_set_length（）指定的明文长度。\retval#PSA_ERROR_INSUFFIE_MEMORY\retval#PSTA_ERROR_COMMUNICATION_FAILURE\retval#PSA_ERROR_HARDWARE_FAILURE \retval#1PSA_ERROR_CORRUPTION_DETECTED \retval#2PSA_ERROR_STORAGE_FAILUE\retval#8PSA_ERROR_BAD_STATE库以前未由PSA_crypto_init（）初始化。初始化失败是否导致此错误代码取决于实现。
 */
 ...
 *
 * \param[in，out]操作激活AEAD操作。\param[out]明文缓冲区，其中将写入明文的最后一部分。这是以前对psa_aead_update（）调用的剩余数据，在输入结束之前无法处理这些数据。\param plaintext_size明文缓冲区的大小（字节）。这必须至少是#PSA_EAD_VERIFY_OUTPUT_SIZE（\c alg），其中\c alj是正在计算的算法。\param[out]plaintext_length成功时，返回的明文的字节数。\param[in]tag包含身份验证标记的缓冲区。\param tag_length标记缓冲区的大小（字节）。
 *
 * \retval#PSA_SUCCESS成功。\retval#PSA_ERROR_INVALID_SIGNATURE计算成功，但身份验证标记不正确。\retval#PSA_ERROR_BAD_STATE操作状态无效（它必须是设置了随机数的活动解密操作）。\retval#PSA_ERROR_BUFFER_TOO_SMALL明文缓冲区的大小太小。您可以通过调用#PSA_EAD_VERIFY_OUTPUT_size（\c alg）来为明文确定足够的缓冲区大小，其中\c alj是正在计算的算法。\retval#PSA_ERROR_INVALID_ARGUMENT到目前为止，PSA_aead_update_ad（）的输入总长度小于先前使用PSA_aeaad_set_length（）指定的附加数据长度。\retval#PSA_ERROR_INVALID_ARGUMENT到目前为止，PSA_aed_update（）的输入总长度小于之前使用PSA_aad_set_length（）指定的明文长度。\retval#PSA_ERROR_INSUFFIE_MEMORY\retval#PSTA_ERROR_COMMUNICATION_FAILURE\retval#PSA_ERROR_HARDWARE_FAILURE \retval#1PSA_ERROR_CORRUPTION_DETECTED \retval#2PSA_ERROR_STORAGE_FAILUE\retval#8PSA_ERROR_BAD_STATE库以前未由PSA_crypto_init（）初始化。初始化失败是否导致此错误代码取决于实现。
 */
 ...
 * \brief使用私钥签名哈希或短消息。
 *
 * 注意，要执行哈希和签名算法，必须首先通过调用psa_hash_setup（）、psa_hhash_update（）和psa_has_finish（）来计算哈希。然后将生成的哈希作为\p哈希参数传递给此函数。您可以使用#PSA_ALG_SIG_GET_HASH（\p ALG）来确定要使用的哈希算法。
 *
 * \param key用于操作的密钥的标识符。它必须是非对称密钥对。密钥必须允许使用#PSA_key_usage_SIGN_HASH。\param alg与密钥类型兼容的签名算法。\param[in]hash要签名的哈希或消息。\param hash_length哈希缓冲区的大小（字节）。\param[out]signature要写入签名的缓冲区。\param signature_size签名缓冲区的大小（字节）。\param[out]signature_length成功时，构成返回签名值的字节数。
 *
 ...
 * 注意，要执行哈希和签名算法，必须首先通过调用psa_hash_setup（）、psa_hhash_update（）和psa_has_finish（）来计算哈希。然后将生成的哈希作为\p哈希参数传递给此函数。您可以使用#PSA_ALG_SIG_GET_HASH（\p ALG）来确定要使用的哈希算法。
 *
 * \param key用于操作的密钥的标识符。它必须是公钥或非对称密钥对。密钥必须允许使用#PSA_key_usage_VERIFY_HASH。\param alg与密钥类型兼容的签名算法。\param[in]hash要验证其签名的哈希或消息。\param hash_length哈希缓冲区的大小（字节）。\param[in]签名缓冲区，包含要验证的签名。\param signature_length签名缓冲区的大小（字节）。
 *
 ...
 * 如果此函数返回的错误状态不是#PSA_error_INSUFFIED_DATA，则操作将进入错误状态，必须通过调用PSA_key_derivation_abort（）中止。
 *
 * 操作产生和消耗多少输出，以及如何派生密钥，取决于密钥类型：
 *
 * -对于密钥是给定大小的任意字节序列的密钥类型，此函数在功能上等同于调用#psa_key_derivation_output_bytes并将结果输出传递给#psa_import_key。然而，该函数具有安全性优势：如果实现提供了隔离边界，那么密钥材料不会暴露在隔离边界之外。因此，对于这些密钥类型，此函数总是从操作中精确消耗（\pbits/8）字节。本规范中定义的以下密钥类型遵循此方案：
 *
 *     - #PSA_KEY_TYPE_AES;
 *     - #PSA_KEY_TYPE_ARC4;
 ...
 *
 * -对于密钥由单个位序列表示的密钥类型，该位序列具有可接受的限制，此函数绘制长度为（\p bits/8）字节的字节字符串，四舍五入到最接近的整数字节。如果生成的字节字符串是可接受的，则它将成为关键字，否则将丢弃绘制的字节。重复此过程，直到绘制出可接受的字节字符串。从操作中提取的字节字符串被解释为为psa_export_key（）生成的输出指定的字符串。本规范中定义的以下密钥类型遵循此方案：
 *
 ...
 *
 * \retval#PSA_SUCCESS成功。如果密钥是持久的，则密钥材料和密钥的元数据已保存到持久存储中。\retval#PSA_ERROR_ALREADY_EXISTS这是一次创建持久密钥的尝试，并且已经存在具有给定标识符的持久密钥。\retval#PSA_ERROR_NOT_SUPPORTED\retval#PSTA_ERROR_INVALID_ARGUMENT\retval#PSA_ERROR_INSUFFIE_MEMORY\retval#PSA_ERROR_INSUFFICE_ENTROPY\retval#1PSA_ERROR_COMMUNICATION_FAILURE\retval#2PSA_ERROR_HARDWARE_FAILUE\retval#8PSA_ERROR_CORRUPTION_DETECTED \retval#3PSA_ERROR_inSUFFIE_STORAGE\retval#xPSA_ERROR_STORAGE_FAILURE \retval#5PSA_ERROR_BAD_STATE库尚未之前由psa_crypto_init（）初始化。初始化失败是否导致此错误代码取决于实现。
 */
