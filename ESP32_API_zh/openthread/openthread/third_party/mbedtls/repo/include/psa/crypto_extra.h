 ...
 *
 * \retval#PSA_SUCCESS密钥已成功注册。注意，根据驱动程序的设计，这可能会也可能不会保证密钥实际上存在于指定的插槽中，并且与指定的属性兼容。\retval#PSA_ERROR_ALREADY_EXISTS已存在一个标识符在\p属性中指定的密钥。\retval#PSA_ERROR_NOT_SUPPORTED指定生存期的安全元素驱动程序不支持注册密钥。\retval#PSA_ERROR_INVALID_ARGUMENT\p属性指定不在安全元素中的生存期。\retval#PSA_ERROR_INVALID_ARGUMENT没有在\p属性中指定插槽号，或者指定的插槽号无效。\retval#PSA_ERROR_NOT_PERMITTED呼叫者无权注册指定的密钥槽。\retval#PSA_ERROR_INSUFFIE_MEMORY\retval#PSTA_ERROR_COMMUNICATION_FAILURE\retval#PSA_ERROR_HARDWARE_FAILURE \retval#PSA_ERROR_CORRUPTION_DETECTED \retval#PSA_RROR_BAD_STATE该库以前未由PSA_crypto_init（）初始化。初始化失败是否导致此错误代码取决于实现。
 */
 ...
 * \注意：此功能仅为方便Mbed TLS而提供，可随时删除，恕不另行通知。
 *
 * \param curve A PSA椭圆曲线标识符（`PSA_ECC_FAMILY_xxx`）。\param byte_length曲线上私钥的字节长度。
 *
 * \return对应的Mbed TLS椭圆曲线标识符（`MBEDTLS_EP_DP_xxx`）。\如果未识别\c曲线，则返回#MBEDTLS_ECP_DP_NONE。\如果\p byte_length对于\p曲线不正确，则返回#MBEDTLS_ECP_DP_NONE。
 */
