...
#define MBEDTLS_ERR_MPI_FILE_IO_ERROR                     -0x0002  /**<读取或写入文件时出错。*/
#define MBEDTLS_ERR_MPI_BAD_INPUT_DATA                    -0x0004  /**<功能输入参数错误。*/
#define MBEDTLS_ERR_MPI_INVALID_CHARACTER                 -0x0006  /**<数字字符串中有一个无效字符。*/
#define MBEDTLS_ERR_MPI_BUFFER_TOO_SMALL                  -0x0008  /**<缓冲区太小，无法写入。*/
#define MBEDTLS_ERR_MPI_NEGATIVE_VALUE                    -0x000A  /**<输入参数为负或导致非法输出。*/
#define MBEDTLS_ERR_MPI_DIVISION_BY_ZERO                  -0x000C  /**<除法的输入参数为零，这是不允许的。*/
#define MBEDTLS_ERR_MPI_NOT_ACCEPTABLE                    -0x000E  /**<输入参数不可接受。*/
#define MBEDTLS_ERR_MPI_ALLOC_FAILED                      -0x0010  /**<内存分配失败。*/
 ...
 *
 * \param X目标MPI。这必须指向已初始化的MPI。\param A求幂的基数。这必须指向已初始化的MPI。\param E指数MPI。这必须指向已初始化的MPI。\param N模块化约简的基础。这必须指向已初始化的MPI。\param _RR一个仅依赖于\N N的帮助器MPI，它可用于为\N N的相同值加速多个模幂运算。这可能是空的。如果不是空，则必须指向已初始化的MPI。如果在调用mbedtls_mpi_init（）后未使用它，则此函数将计算助手值并将其存储在\p_RR中，以便在后续调用此函数时重用。否则，该函数将假定\p_RR保存由先前对mbedtls_mpi_exp_mod（）的调用设置的帮助器值，并重用它。
 *
