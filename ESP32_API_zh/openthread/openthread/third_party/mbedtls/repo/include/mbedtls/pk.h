...
#define MBEDTLS_ERR_PK_ALLOC_FAILED        -0x3F80  /**<内存分配失败。*/
#define MBEDTLS_ERR_PK_TYPE_MISMATCH       -0x3F00  /**<类型不匹配，例如尝试使用ECDSA密钥加密*/
#define MBEDTLS_ERR_PK_BAD_INPUT_DATA      -0x3E80  /**<功能输入参数错误。*/
#define MBEDTLS_ERR_PK_FILE_IO_ERROR       -0x3E00  /**<读取/写入文件失败。*/
#define MBEDTLS_ERR_PK_KEY_INVALID_VERSION -0x3D80  /**<不支持的密钥版本*/
#define MBEDTLS_ERR_PK_KEY_INVALID_FORMAT  -0x3D00  /**<无效的键标记或值。*/
#define MBEDTLS_ERR_PK_UNKNOWN_PK_ALG      -0x3C80  /**<密钥算法不受支持（仅支持RSA和EC）。*/
#define MBEDTLS_ERR_PK_PASSWORD_REQUIRED   -0x3C00  /**<私钥密码不能为空。*/
#define MBEDTLS_ERR_PK_PASSWORD_MISMATCH   -0x3B80  /**<给定的私钥密码不允许正确解密。*/
#define MBEDTLS_ERR_PK_INVALID_PUBKEY      -0x3B00  /**<公钥标记或值无效（仅支持RSA和EC）。*/
#define MBEDTLS_ERR_PK_INVALID_ALG         -0x3A80  /**<算法标记或值无效。*/
#define MBEDTLS_ERR_PK_UNKNOWN_NAMED_CURVE -0x3A00  /**<不支持椭圆曲线（仅支持NIST曲线）。*/
#define MBEDTLS_ERR_PK_FEATURE_UNAVAILABLE -0x3980  /**<不可用功能，例如RSA密钥禁用RSA。*/
#define MBEDTLS_ERR_PK_SIG_LEN_MISMATCH    -0x3900  /**<缓冲区包含一个有效签名，后面跟着更多数据。*/

/* MBEDTLS_ERROR_PK_HW_ACCEL_FAILED已弃用，不应使用。*/
#define MBEDTLS_ERR_PK_HW_ACCEL_FAILED     -0x3880  /**<PK硬件加速器失败。*/
 ...
 *
 * \param p ASN.1数据中的位置\param缓冲区的结束\param pk要填充的pk上下文。它必须已初始化但未设置。
 *
...
#if defined(MBEDTLS_PK_WRITE_C)
/**
 * \brief将subjectPublicKey写入ASN.1数据注意：函数在数据缓冲区中向后工作
 *
