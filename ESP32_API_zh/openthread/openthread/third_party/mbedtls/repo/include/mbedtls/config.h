 ...
 *
 * 通过use_SRTP扩展支持DTLS-SRTP（RFC 5764）的反向旋转。
 *
 ...
 *
 * 请参见`docs/architecture/testing/mbed加密侵入测试。md`获取更多信息。
 *
 * 取消注释以启用侵入性测试。
 */
//#定义MBEDTLS_TEST_HOOKS
 ...
 *
 * \警告PSA Crypto API处于测试阶段。虽然欢迎您尝试使用它，但不兼容的API更改仍然是可能的，而且某些部分可能尚未达到与Mbed TLS其他部分相同的质量。
 *
 * \警告此选项启用依赖于PSA Crypto API的新的Mbed TLS API，因此不能与其他Mbed TLS APIs提供相同的稳定性保证。欢迎您尝试使用它们，但目前，对这些API的访问是可选的（通过启用当前选项），以便明确区分它们与稳定的Mbed TLS API。
 *
 * 需要：MBEDTLS_PSA_CRYPTO_C。
 *
 ...
 *
 * \注意：要使用CTR_DRBG实现256位安全强度，必须使用AES-256并使用足够的熵。请参见ctr_drbg。h了解更多详情。
 *
 ...
 * 启用平台安全体系结构加密API。
 *
 * \警告PSA Crypto API仍处于测试状态。虽然欢迎您尝试使用它，但不兼容的API更改仍然是可能的，而且某些部分可能尚未达到与Mbed TLS其他部分相同的质量。
 *
