 ...
 * RSA错误代码
 */
#define MBEDTLS_ERR_RSA_BAD_INPUT_DATA                    -0x4080  /**<功能输入参数错误。*/
#define MBEDTLS_ERR_RSA_INVALID_PADDING                   -0x4100  /**<输入数据包含无效填充并被拒绝。*/
#define MBEDTLS_ERR_RSA_KEY_GEN_FAILED                    -0x4180  /**<生成密钥时出现问题。*/
#define MBEDTLS_ERR_RSA_KEY_CHECK_FAILED                  -0x4200  /**<密钥未能通过库的有效性检查。*/
#define MBEDTLS_ERR_RSA_PUBLIC_FAILED                     -0x4280  /**<公钥操作失败。*/
#define MBEDTLS_ERR_RSA_PRIVATE_FAILED                    -0x4300  /**<私钥操作失败。*/
#define MBEDTLS_ERR_RSA_VERIFY_FAILED                     -0x4380  /**<PKCS#1验证失败。*/
#define MBEDTLS_ERR_RSA_OUTPUT_TOO_LARGE                  -0x4400  /**<解密的输出缓冲区不够大。*/
#define MBEDTLS_ERR_RSA_RNG_FAILED                        -0x4480  /**<随机生成器无法生成非零。*/

/* MBEDTLS_ERROR_RSA_UNSUPPORTED_OPERATION已弃用，不应使用。
 */
#define MBEDTLS_ERR_RSA_UNSUPPORTED_OPERATION             -0x4500  /**<例如，由于安全违规或缺少功能，实现不提供请求的操作。*/

/* MBEDTLS_ERROR_RSA_HW_ACCEL_FAILED已弃用，不应使用。*/
#define MBEDTLS_ERR_RSA_HW_ACCEL_FAILED                   -0x4580  /**<RSA硬件加速器失败。*/
 ...
 *
 * \注意RSA上下文中的\p hash_id是用于编码的hash_id。\函数调用中的p mdalg是编码的哈希类型。根据<em>RFC-3447:公钥密码标准（PKCS）#1 v2.1:RSA密码规范</em>，建议保持两个哈希相同。
 *
 ...
 *
 *                 MGF掩码生成函数的哈希函数是RSA上下文中指定的哈希函数。
 *
 * \注意RSA上下文中的\p hash_id是用于验证的散列。\函数调用中的p mdalg是要验证的哈希类型。根据<em>RFC-3447:公钥密码标准（PKCS）#1 v2.1:RSA密码规范</em>，建议保持两个哈希相同。如果RSA上下文中的\p hash_id未设置，则使用函数调用的\p md_alg。
 *
 * \不推荐在#MBEDTLS_RSA_PRIVATE模式下调用此函数。库的未来版本可能会删除\p mode参数，并将其隐式设置为#MBEDTLS_RSA_PUBLIC。
 *
 * \注意：RSA的替代实现不需要支持将模式设置为#MBEDTLS_RSA_PRIVATE，而是可以返回#MBEDLLS_ERROR_PLATFORM_FEATURE_UNSUPPORTED。
