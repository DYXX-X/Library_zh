...
/*
 * 我们假设CHAR_BIT在很多地方都是8。实际上，这在我们的目标平台上是正确的，所以这不是问题，但让我们更加确定。
 */
#include <limits.h>
#if CHAR_BIT != 8
...
/* 在此处修复配置。在配置中放入#ifdef_WIN32不方便。因为它会混淆config.py。*/
...
/*
 * 避免迂腐的警告。这是一个方便的解决方法，因为在#if defined（MBEDTLS_xx_C）之前的每个文件中都包含这一点，这会导致空的转换单元。
 */
