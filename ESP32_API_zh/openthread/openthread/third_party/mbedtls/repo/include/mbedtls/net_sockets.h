...
#define MBEDTLS_ERR_NET_SOCKET_FAILED                     -0x0042  /**<无法打开套接字。*/
#define MBEDTLS_ERR_NET_CONNECT_FAILED                    -0x0044  /**<与给定服务器/端口的连接失败。*/
#define MBEDTLS_ERR_NET_BIND_FAILED                       -0x0046  /**<套接字绑定失败。*/
#define MBEDTLS_ERR_NET_LISTEN_FAILED                     -0x0048  /**<无法在套接字上侦听。*/
#define MBEDTLS_ERR_NET_ACCEPT_FAILED                     -0x004A  /**<无法接受传入连接。*/
#define MBEDTLS_ERR_NET_RECV_FAILED                       -0x004C  /**<从套接字读取信息失败。*/
#define MBEDTLS_ERR_NET_SEND_FAILED                       -0x004E  /**<通过套接字发送信息失败。*/
#define MBEDTLS_ERR_NET_CONN_RESET                        -0x0050  /**<连接已由对等方重置。*/
#define MBEDTLS_ERR_NET_UNKNOWN_HOST                      -0x0052  /**<无法获取给定主机名的IP地址。*/
#define MBEDTLS_ERR_NET_BUFFER_TOO_SMALL                  -0x0043  /**<缓冲区太小，无法容纳数据。*/
#define MBEDTLS_ERR_NET_INVALID_CONTEXT                   -0x0045  /**<上下文无效，例如因为它是free（）ed。*/
#define MBEDTLS_ERR_NET_POLL_FAILED                       -0x0047  /**<轮询网络上下文失败。*/
#define MBEDTLS_ERR_NET_BAD_INPUT_DATA                    -0x0049  /**<输入无效。*/
 ...
 *
 * \如果成功，则返回0，或以下之一：MBEDTLS_ERROR_NET_SOCKET_FAILED、MBEDTLS_ERROR_NET_BIND_FAILED、MBEDTLS_ERR_NET_LISTEN_FAILED
 *
 * \注意：无论协议是什么，都会打开套接字并绑定它。此外，如果协议是TCP，则使套接字侦听。
 */
 ...
 *
 * \如果成功，则返回0；如果buf_size太小，则返回MBEDTLS_ERROR_NET_ACCEPT_FAILED；如果bind_fd设置为非阻塞，则返回EMBEDTLS_ERR_NET_BUFFER_TOO_SMALL。
 */
 ...
 *
 * \返回接收的字节数或非零错误代码：如果操作超时，返回MBEDTLS_error_SSL_TIMEOUT；如果被信号中断，返回MBEDTLS_ERR_SSL_WANT_READ。
 *
