 ...
 * ECP错误代码
 */
#define MBEDTLS_ERR_ECP_BAD_INPUT_DATA                    -0x4F80  /**<功能输入参数错误。*/
#define MBEDTLS_ERR_ECP_BUFFER_TOO_SMALL                  -0x4F00  /**<缓冲区太小，无法写入。*/
#define MBEDTLS_ERR_ECP_FEATURE_UNAVAILABLE               -0x4E80  /**<请求的功能不可用，例如，不支持请求的曲线。*/
#define MBEDTLS_ERR_ECP_VERIFY_FAILED                     -0x4E00  /**<签名无效。*/
#define MBEDTLS_ERR_ECP_ALLOC_FAILED                      -0x4D80  /**<内存分配失败。*/
#define MBEDTLS_ERR_ECP_RANDOM_FAILED                     -0x4D00  /**<生成临时密钥等随机值失败。*/
#define MBEDTLS_ERR_ECP_INVALID_KEY                       -0x4C80  /**<私钥或公钥无效。*/
#define MBEDTLS_ERR_ECP_SIG_LEN_MISMATCH                  -0x4C00  /**<缓冲区包含一个有效签名，后面跟着更多数据。*/

/* MBEDTLS_ERROR_ECP_HW_ACCEL_FAILED已弃用，不应使用。*/
#define MBEDTLS_ERR_ECP_HW_ACCEL_FAILED                   -0x4B80  /**<ECP硬件加速器失败。*/

#define MBEDTLS_ERR_ECP_IN_PROGRESS                       -0x4B00  /**<操作正在进行，请使用相同的参数再次调用以继续。*/
 ...
 * 组的最大大小，即\c N和\c P。
 */
#define MBEDTLS_ECP_MAX_BITS     521   /**<组的最大大小，以位为单位。*/
#endif
...
/*
 * 用于点乘法的最大“窗口”大小。默认值：6。最小值：2。最大值：7。
 *
...
/**
 * \brief此函数检索mbedtls_ecp_curve_info（）中为所有支持的曲线定义的信息（按首选项顺序）。
 *
