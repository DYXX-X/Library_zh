...
#define MBEDTLS_ERR_CTR_DRBG_ENTROPY_SOURCE_FAILED        -0x0034  /**<熵源失败。*/
#define MBEDTLS_ERR_CTR_DRBG_REQUEST_TOO_BIG              -0x0036  /**<请求的随机缓冲区长度太大。*/
#define MBEDTLS_ERR_CTR_DRBG_INPUT_TOO_BIG                -0x0038  /**<输入（熵+附加数据）太大。*/
#define MBEDTLS_ERR_CTR_DRBG_FILE_IO_ERROR                -0x003A  /**<文件中的读取或写入错误。*/
 ...
 * \name SECTION:模块设置
 *
 * 您可以为此模块设置的配置选项在本节中。要么在config.h中更改它们，要么使用编译器命令行定义它们\{
 */
