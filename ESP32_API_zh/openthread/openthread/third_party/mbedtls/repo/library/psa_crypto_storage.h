 ...
 * -实际上只使用从1到#PSA_key_SLOT_COUNT的密钥ID。
 *
 ...
 *
 * \retval#PSA_SUCCESS\retval#PSA_ERROR_INSUFFIE_MEMORY\retval#PSA_ERROR_inSUFFIE_STORAGE\retval#PSA_ERROR_STORAGE_FAILURE\retval*PSA_ERROR_ALREADY_EXISTS
 */
 ...
 *
 * 注意：此函数分配缓冲区，并通过数据参数返回指向它的指针。必须在该函数之后调用psa_free_persistent_key_data（），以使该缓冲区归零和释放，无论该函数是否成功。
 *
 ...
 *
 * \retval#PSA_SUCCESS\retval#PSTA_ERROR_INSUFFIE_MEMORY\retval#PSA_ERROR_STORAGE_FAILURE\retval#PSA_ERROR_DOES_NOT_EXIST
 */
 ...
 *
 * \retval#PSA_SUCCESS密钥已成功删除，或者密钥不存在。\retval#PSA_ERROR_STORAGE_FAILURE
 */
 ...
 *
 * \retval#PSA_SUCCESS\retval#PSTA_ERROR_INSUFFIE_STORAGE\retval#PSA_ERROR_INNUFFIE_MEMORY\retval#PSA_ERROR_STORAGE_FAILURE
 */
 ...
 *
 * \retval#PSA_SUCCESS\retval#PSTA_ERROR_INSUFFIE_STORAGE\retval#PSA_ERROR_STORAGE_FAILURE
 */
 ...
 *
 * \retval#PSA_SUCCESS有关正在进行的事务的数据已加载到#PSA_crypto_transaction。\retval#PSA_ERROR_DOES_NOT_EXIST没有正在进行的事务。\retval#PSA_ERROR_STORAGE_FAILURE
 */
