 ...
 * \return指示操作成功/失败的状态
 *
 * \retval#PSA_SUCCESS操作成功完成\retval#PSTA_ERROR_DOES_NOT_EXIST操作失败，因为在存储中找不到提供的“uid”值\retval#PSA_ERROR_INVALID_SIZE操作失败，原因是与提供的uid相关联的数据大于“data_SIZE”\retval#PSA_ERROR_storage_FAILURE存储失败（致命错误）\retval#PSA_error_INVALID_ARGUMENT操作失败，因为提供的指针之一（“p_data”，“p_data_length”）无效。例如，“NULL”或引用调用方无法访问的内存。此外，如果提供了无效的偏移量，也可能发生这种情况。
 */
 ...
 * \return指示操作成功/失败的状态
 *
 * \retval#PSA_SUCCESS操作成功完成\retval#PSA _ERROR_DOES_NOT_EXIST操作失败，因为在存储中找不到提供的uid值\retval#PSA_ERROR_storage_FAILURE操作失败，原因是物理存储失败（致命错误）\retval#1PSA_ERROR_INVALID_ARGUMENT操作失败，是因为提供的指针之一（“p_info”）无效，例如为“NULL”或引用调用方无法访问的内存
 */
