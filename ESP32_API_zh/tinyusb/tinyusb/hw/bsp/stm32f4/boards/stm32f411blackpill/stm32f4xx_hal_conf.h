/**
  ******************************************************************************
  * @file    stm32f4x_hal_conf_template.h
  * @author  MCD应用团队
  * @brief   HAL配置文件
  ******************************************************************************
  * @attention
...
/**
  ...
  */
  ...
  #define HSE_VALUE    (25000000U) /*!< 外部振荡器的值（Hz）*/
#endif /* HSE_VALUE */
...
/**
  ...
  */
  ...
  #define EXTERNAL_CLOCK_VALUE    (12288000U) /*!< 外部振荡器的值（Hz）*/
...
#define  TICK_INT_PRIORITY            (0x0FU) /*!< 滴答中断优先级*/           
#define  USE_RTOS                     0U
...
#define  USE_HAL_FMPI2C_REGISTER_CALLBACKS      0U /* FMPI2C寄存器回调已禁用*/
...
#define  USE_HAL_PCCARD_REGISTER_CALLBACKS      0U /* PCCARD寄存器回调已禁用*/
...
#define ETH_RXBUFNB                    4U                  /* 4个大小为ETH_Rx_BUF_size的Rx缓冲区*/
#define ETH_TXBUFNB                    4U                  /* 4个大小为ETH_Tx_BUF_size的Tx缓冲区*/
...
#define PHY_BCR                         ((uint16_t)0x0000)  /*!< 收发器基本控制寄存器*/
#define PHY_BSR                         ((uint16_t)0x0001)  /*!< 收发器基本状态寄存器*/
...
#define PHY_SR                          ((uint16_t)0x0010)  /*!< PHY状态寄存器偏移*/
#define PHY_MICR                        ((uint16_t)0x0011)  /*!< MII中断控制寄存器*/
#define PHY_MISR                        ((uint16_t)0x0012)  /*!< MII中断状态和其他。控制寄存器*/
 
#define PHY_LINK_STATUS                 ((uint16_t)0x0001)  /*!< PHY链路掩码*/
#define PHY_SPEED_STATUS                ((uint16_t)0x0002)  /*!< PHY速度掩码*/
#define PHY_DUPLEX_STATUS               ((uint16_t)0x0004)  /*!< PHY双工掩码*/

#define PHY_MICR_INT_EN                 ((uint16_t)0x0002)  /*!< PHY启用中断*/
#define PHY_MICR_INT_OE                 ((uint16_t)0x0001)  /*!< PHY启用输出中断事件*/

#define PHY_MISR_LINK_INT_EN            ((uint16_t)0x0020)  /*!< 启用链路状态更改时的中断*/
#define PHY_LINK_INTERRUPT              ((uint16_t)0x2000)  /*!< PHY链路状态中断掩码*/
