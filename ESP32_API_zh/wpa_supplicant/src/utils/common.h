/*
 * wpa_supplicant/hostapd/common-helper函数等。版权所有（c）2002-2007，Jouni Malinen<j@w1.fi>
 *
...
#define ETH_P_80211_ENCAP 0x890d /* TDLS属于此类*/
#endif
#ifndef ETH_P_PAE
...
/* snprintf-用于多个位置；由于缓冲区可能溢出，sprintf（）不是很好的替代品；参见例如。，http://www.ijs.si/software/snprintf/用于snprintf的可移植实现。*/
int snprintf(char *str, size_t size, const char *format, ...);

/* vsnprintf-仅用于wpa_supplicant中的wpa\msg（）。c*/
int vsnprintf(char *str, size_t size, const char *format, va_list ap);
#endif /* !定义（_MSC_VER）||_MSC_VER<1400*/
...
/* inline-如果需要，定义为__inline或将其定义为空*/
...
/*
 * 用于MAC地址字符串表示的紧凑形式，例如，用于构建P2P设备的dbus路径
 */
...
/*
 * 稀疏验证的定义(http://kernel.org/pub/linux/kernel/people/josh/sparse/)
 */
#ifdef __CHECKER__
...
/*
 * gcc4.4最终生成了一些非常常见的网络套接字使用的严格别名警告，这些警告不会真正导致真正的问题，并且由于结构定义在系统头文件中包含另一个结构，因此基于联合的类型punning无法轻易避免。为了避免必须完全禁用严格的别名警告，请提供一种暂时隐藏类型转换以避免别名的机制。希望将来能找到一种更清洁的解决方案来处理这些案件。
 */
