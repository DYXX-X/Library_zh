/*
 * Wi-Fi保护设置-内部定义版权所有（c）2008-2012，Jouni Malinen<j@w1.fi>
 *
...
#endif
/**
 * 结构wps_data-wps注册协议数据
 *
 * 该数据存储在EAP-WSC服务器/对等方方法中，并为单个注册协议运行保留。
 */
struct wps_data {
	/**
	 * wps-指向长期wps上下文的指针
	...
	/**
	 * request_type-来自（Re）AssocReq的请求类型属性
	 */
	u8 request_type;
	...
	int use_psk_key;
	u8 p2p_dev_addr[ETH_ALEN]; /* 客户端的P2P设备地址，如果不是P2P客户端，则为00:00:00:00:00*/
	int pbc_in_m1;
#ifdef CONFIG_WPS_NFC
