/*
 * hostapd和WPA_supplicant之间共享的WPA定义版权所有（c）2002-2008，Jouni Malinen<j@w1.fi>
 *
...
/* EAPOL密钥密钥数据封装GroupKey和PeerKey需要加密，否则，加密是可选的。
 */
...
/* IEEE 802.11，8.5.2 EAPOL关键帧*/
#define WPA_KEY_INFO_TYPE_MASK ((u16) (BIT(0) | BIT(1) | BIT(2)))
#define WPA_KEY_INFO_TYPE_AKM_DEFINED 0
#define WPA_KEY_INFO_TYPE_HMAC_MD5_RC4 BIT(0)
#define WPA_KEY_INFO_TYPE_HMAC_SHA1_AES BIT(1)
#define WPA_KEY_INFO_TYPE_AES_128_CMAC 3
#define WPA_KEY_INFO_KEY_TYPE BIT(3) /* 1=成对，0=组密钥*/
/* bit4..5在WPA中使用，但在IEEE 802.11i/RSN中保留*/
#define WPA_KEY_INFO_KEY_INDEX_MASK (BIT(4) | BIT(5))
#define WPA_KEY_INFO_KEY_INDEX_SHIFT 4
#define WPA_KEY_INFO_INSTALL BIT(6) /* 成对的，成对的*/
#define WPA_KEY_INFO_TXRX BIT(6) /* 组*/
#define WPA_KEY_INFO_ACK BIT(7)
#define WPA_KEY_INFO_MIC BIT(8)
#define WPA_KEY_INFO_SECURE BIT(9)
#define WPA_KEY_INFO_ERROR BIT(10)
#define WPA_KEY_INFO_REQUEST BIT(11)
#define WPA_KEY_INFO_ENCR_KEY_DATA BIT(12) /* 仅限IEEE 802.11i/RSN*/
#define WPA_KEY_INFO_SMK_MESSAGE BIT(13)


struct wpa_eapol_key {
	u8 type;
	/* 注意：key_info、key_length和key_data_length未对齐*/
	u8 key_info[2]; /* 大端序*/
	u8 key_length[2]; /* 大端序*/
	...
	u8 key_id[8]; /* 保留在IEEE 802.11i/RSN中*/
	u8 key_mic[16];
	u8 key_data_length[2]; /* 大端序*/
	/* 后跟key_data的key_data_length字节*/
} STRUCT_PACKED;
...
/**
 * 结构wpa_tk-wpa成对瞬态密钥IEEE Std 802.11i-2004-8.5.1.2成对密钥层次结构
 */
struct wpa_ptk {
	u8 kck[WPA_KCK_MAX_LEN]; /* EAPOL密钥确认密钥（KCK）*/
	u8 kek[WPA_KEK_MAX_LEN]; /* EAPOL密钥加密密钥（KEK）*/
	u8 tk[WPA_TK_MAX_LEN]; /* 临时密钥（TK）*/
	size_t kck_len;
	size_t kek_len;
	size_t tk_len;
};
...
/* WPA IE版本1 00-50-f2:1（OUI:OUI类型）0x01 0x00（版本；小端）（以下所有字段都是可选的：）组套件选择器（4个八位字节）（默认值：TKIP）成对套件计数（2个八位位组，小端）（4 n个八位字节）（默认值：未用802.1X）WPA功能（2个八位位组，小端）（默认：0）
 */

struct wpa_ie_hdr {
	u8 elem_id;
	u8 len;
	u8 oui[4]; /* 24位OUI后跟8位OUI类型*/
	u8 version[2]; /* 小端序*/
} STRUCT_PACKED;


/* 1/4:PMKID 2/4:RSN IE 3/4:一个或两个RSN IE+GTK IE（加密）4/4:空1/2:GTK IE
 */

/* RSN IE版本1 0x01 0x00（版本；小端）（以下所有字段都是可选的：）组套件选择器（4个八位字节）（默认值：CCMP）成对套件计数（2个八位位组，小端）（默认：未用802.1X）RSN功能（2个八位字节，小端）（默认：0）PMKID计数（2个8位字节）（默认值：0）PMTID列表（16 n个八位）管理组密码套件（4个八位比特）（默认为：AES-128-CMAC）
 */
