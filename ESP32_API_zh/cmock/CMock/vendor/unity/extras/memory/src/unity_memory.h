...
/* 定义此宏以删除stdlib的使用。h、 malloc和免费。许多嵌入式系统默认没有堆或malloc/free。这个内部unity_malloc（）仅从数组末尾确定地提供分配的内存，unity_free（）只从数组末尾释放，块不合并，未按LIFO顺序释放的内存被搁浅。*/
    #ifndef UNITY_INTERNAL_HEAP_SIZE_BYTES
    #define UNITY_INTERNAL_HEAP_SIZE_BYTES 256
    #endif
#endif

/* Unity使用这些函数来分配和释放堆上的内存，并且可以使用特定于平台的实现来覆盖这些函数。例如，当使用FreeRTOS时，UNITY_MALLOC变为pvPortMalloc（），UNITY_FREE变为vPortFree（）。*/
