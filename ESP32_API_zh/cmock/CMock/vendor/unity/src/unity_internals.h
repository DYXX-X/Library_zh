...
/* Unity尝试自动检测整数类型尝试1:UINT_MAX，ULONG_MAX在<limits中。h> ，或默认为32位尝试2:UINTPTR_MAX（在<stdint中）。h> ，或默认为相同的长度用户可以覆盖以下任何派生常量：UNITY_INT_WIDTH、UNITY_long_WIDTH、UNITY_POINTER_WIDTH*/
...
/* 如果尚未指定，请确定int的大小。我们不能使用sizeof（int），因为它在C程序的翻译阶段还没有定义。同样，sizeof（int）也会在所有平台上以可寻址的单位返回大小，而不一定是以字节为单位的大小。因此，如果可能，请从UINT_MAX推断它。*/
...
/* 64位支持相关配置*/
    ...
    #define UNITY_MAX_NIBBLES (8)  /* UNITY_（U）INT中的最大半字节数*/
#else
  /* 64位支持*/
    ...
    #define UNITY_MAX_NIBBLES (16) /* UNITY_（U）INT中的最大半字节数*/
#endif
...
#define UNITY_EXCLUDE_DOUBLE /* 当不包括浮动支架时，拆下两个*/
#endif
...
#ifndef isnan
/* NaN是唯一不等于自身的浮点值。因此，如果n！=n、 那么它是NaN。*/
...
/* 与float不同，我们默认不包含*/
  ...
  /* 对于UnityPrintFloat（UNITY_DOUBLE）中的参数，其别名为DOUBLE或float*/
  #endif
 ...
 *-------------------------------------------------------*/
  ...
  /* 默认为使用putchar，它在stdio中定义。小时*/
  #include <stdio.h>
  #define UNITY_OUTPUT_CHAR(a) (void)putchar(a)
#else
  /* 如果定义为其他内容，请确保我们在此处声明它，以便它可以使用*/
    ...
    /* 我们希望使用stdout flush实用程序*/
    #include <stdio.h>
    #define UNITY_OUTPUT_FLUSH() (void)fflush(stdout)
  #else
    /* 我们没有指定任何内容，因此应该忽略flush*/
...
#define UNITY_OUTPUT_COMPLETE()
      ...
      /* 如果未定义任何这些宏，请尝试提供默认实现*/

    #if defined(UNITY_CLOCK_MS)
      /* 这是在支持获取毫秒计数器的平台上获取默认实现的简单方法*/
...
/*-------------------------------------------------------
 * 测试断言函数
 *------------------------------------------------------- 使用本节下面的宏，而不是直接调用这些宏。宏具有一致的命名约定，并将为您引入文件和行信息。*/
...
/* 这一系列棘手的宏为我们提供了一个可选的行参数，将其视为RUN_TEST（func，num=__line_）*/
#ifndef RUN_TEST
...
/* 如果我们不能完成这个棘手的版本，我们只需要要求他们始终包含行号*/
#ifndef RUN_TEST
#ifdef CMOCK
