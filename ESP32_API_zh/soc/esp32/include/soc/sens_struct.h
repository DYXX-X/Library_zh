...
typedef volatile struct sens_dev_s {
    union {
        struct {
            ...
            uint32_t sar1_dig_force:    1;                /*1： ADC1由数字控制器0:RTC控制器控制*/
    ...
    union {
        struct {
            ...
            uint32_t meas1_start_force: 1;                /*1： ADC1由数字或RTC控制器0:Ulp协处理器控制*/
            uint32_t sar1_en_pad:      12;
            uint32_t sar1_en_pad_force: 1;                /*1： 数据端口由数字或RTC控制器0:Ulp协处理器控制*/
        };
        uint32_t val;
    } sar_meas_start1;
    union {
        struct {
            ...
            uint32_t xpd_hall_force:   1;                 /*1： 霍尔传感器的电源由数字或RTC控制器0:Ulp协处理器控制*/
            uint32_t hall_phase_force: 1;                 /*1： 霍尔传感器的相位由数字或RTC控制器0:Ulp协处理器控制*/
            uint32_t reserved28:       4;
        };
        uint32_t val;
    } sar_touch_ctrl1;
    ...
    union {
        struct {
            ...
            uint32_t sar2_pwdet_force:  1;                /*1： ADC2由PWDET 0控制：数字或RTC控制器*/
            uint32_t sar2_dig_force:    1;                /*1： ADC2由数字控制器0:RTC控制器控制*/
    ...
    union {
        struct {
            ...
            uint32_t meas2_start_force: 1;                /*1： ADC2由数字或RTC控制器0:Ulp协处理器控制*/
            uint32_t sar2_en_pad:      12;
            uint32_t sar2_en_pad_force: 1;                /*1： 数据端口由数字或RTC控制器0:Ulp协处理器控制*/
        };
        uint32_t val;
    } sar_meas_start2;
    union {
