 ...
 */
typedef enum {
    RTC_XTAL_FREQ_AUTO = 0,     //!< 自动XTAL频率检测
    RTC_XTAL_FREQ_40M = 40,     //!< 40 MHz XTAL
    RTC_XTAL_FREQ_26M = 26,     //!< 26兆赫XTAL
...
typedef struct rtc_clk_config_s {
    ...
    uint32_t clk_8m_div : 3;            //!< RTC 8M时钟分频器（分频为clk_8M_div+1，即0表示8MHz频率）
 ...
 * 初始化时钟并设置CPU频率
 *
 * 如果cfg.xtal_freq设置为RTC_xtal_freq_AUTO，此函数将尝试自动检测xtal频率。通过将XTAL频率与内部8MHz振荡器的频率进行比较来执行自动检测。注意，在高温下，内部8MHz振荡器的频率可能漂移到足以使自动检测不可靠。自动检测代码将尝试区分26MHz和40MHz晶体。自动检测代码不支持24 MHz晶体。如果无法自动检测XTAL频率，将使用此26MHz频率。
 *
 ...
 * @brief 获取主XTAL频率
 *
 * 这是引导加载程序存储在RTC寄存器RTC_XTAL_FREQ_REG中的值。当传递给rtc_clk_init函数时，或者如果值为rtc_XTAL_FREQ_AUTO，则检测到的XTAL频率。
 *
...
/**
 * @brief 获取与rtc_CPU_freq_t值对应的CPU频率配置
 * @param cpu_freq CPU频率枚举值
...
typedef struct rtc_sleep_config_s {
    ...
    uint32_t rtc_mem_inf_fpu : 1;       //!< 强制睡眠模式下的正常电压（RTC存储器）
    ...
    uint32_t rom_mem_pd_en : 1;         //!< 关闭主RAM和ROM
    ...
    uint32_t lslp_meminf_pd : 1;        //!< 删除所有外围设备强制加电标志
...
#define RTC_SLEEP_PD_XTAL               BIT(6)  //!< 关闭主XTAL电源
...
#define RTC_MAC_TRIG_EN     BIT(5)  //!< MAC唤醒（仅限轻度睡眠）
