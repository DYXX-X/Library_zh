...
/* TWAI外设的寄存器是8位，但是ESP32只能每32位访问一次外设寄存器。因此，每个TWAI寄存器被映射到每32位的最低有效字节。
 */
...
typedef volatile struct twai_dev_s {
    ...
    union {
        struct {
            uint32_t rm: 1;                     /* 国防部。0重置模式*/
            uint32_t lom: 1;                    /* 国防部。1只听模式*/
            uint32_t stm: 1;                    /* 国防部。2自检模式*/
            uint32_t afm: 1;                    /* 国防部。3验收过滤器模式*/
            uint32_t reserved4: 28;             /* 内部保留。国防部。4不支持睡眠模式*/
        };
        uint32_t val;
    } mode_reg;                                 /* 地址0x0000*/
    union {
        struct {
            uint32_t tr: 1;                     /* 厘米。0传输请求*/
            uint32_t at: 1;                     /* 厘米。1中止传输*/
            uint32_t rrb: 1;                    /* 厘米。2释放接收缓冲区*/
            uint32_t cdo: 1;                    /* 厘米。3清除数据溢出*/
            uint32_t srr: 1;                    /* 厘米。4自我接待请求*/
    ...
    union {
        struct {
            uint32_t ri: 1;                     /* 红外。0接收中断*/
            uint32_t ti: 1;                     /* 红外。1传输中断*/
            uint32_t ei: 1;                     /* 红外。2错误中断*/
            uint32_t doi: 1;                    /* 红外。3数据溢出中断*/
            uint32_t reserved4: 1;              /* 内部保留（不支持唤醒）*/
            uint32_t epi: 1;                    /* 红外。5错误被动中断*/
            uint32_t ali: 1;                    /* 红外。6仲裁中断中断*/
            uint32_t bei: 1;                    /* 红外。7总线错误中断*/
            uint32_t reserved8: 24;             /* 内部保留*/
        };
        uint32_t val;
    ...
    union {
        struct {
            uint32_t rie: 1;                    /* 更好。0接收中断启用*/
            uint32_t tie: 1;                    /* 更好。1传输中断启用*/
            uint32_t eie: 1;                    /* 更好。2错误中断启用*/
            uint32_t doie: 1;                   /* 更好。3数据超限中断启用*/
            uint32_t brp_div: 1;                /* 这不是中断。brp_div将brp预缩放2。仅适用于ESP32修订版2或更高版本。否则保留*/
            uint32_t epie: 1;                   /* 更好。5错误被动中断启用*/
            uint32_t alie: 1;                   /* 更好。6仲裁丢失中断启用*/
            uint32_t beie: 1;                   /* 更好。7总线错误中断启用*/
            uint32_t reserved8: 24;             /* 内部保留*/
        };
        uint32_t val;
    ...
    union {
        struct {
            uint32_t brp: 6;                    /* BTR0[5:0]波特率预分频器*/
            uint32_t sjw: 2;                    /* BTR0[7:6]同步跳转宽度*/
            uint32_t reserved8: 24;             /* 内部保留*/
        };
        uint32_t val;
    ...
    union {
        struct {
            ...
            uint32_t dir: 1;                    /* ECC公司。5错误方向（TX/RX）*/
    ...
    union {
        struct {
            uint32_t cd: 3;                     /* 基于fOSC的CDR[2:0]CLKOUT频率选择器*/
            uint32_t co: 1;                     /* CDR。3 CLKOUT启用/禁用*/
            uint32_t reserved4: 3;              /* 内部保留。不支持RXINTEN和CBP*/
            uint32_t cm: 1;                     /* CDR。7寄存器布局。基本：0扩展：1*/
            uint32_t reserved6: 24;             /* 内部保留*/
        };
        uint32_t val;
