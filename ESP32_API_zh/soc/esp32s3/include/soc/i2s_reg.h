...
/* I2S_TX_HUNG_INT_RAW:RO/WTC/SS；位位置：[3]；默认值：1’b0；*/
...
/* I2S_RX-HUNG_INT_RAW:RO/WTC/SS；位位置：[2]；默认值：1’b0；*/
...
/* I2S_TX_DONE_INT_RAW:RO/WTC/SS；位位置：[1]；默认值：1’b0；*/
...
/* I2S_RX_DONE_INT_RAW:RO/WTC/SS；位位置：[0]；默认值：1’b0；*/
...
/* I2S_TX_HUNG_INT_CLR:WT；位位置：[3]；默认值：1’b0；*/
...
/* I2S_RX-HUNG_INT_CLR:WT；位位置：[2]；默认值：1’b0；*/
...
/* I2S_TX_DONE_INT_CLR:WT；位位置：[1]；默认值：1’b0；*/
...
/* I2S_RX_完成_时间：WT；位位置：[0]；默认值：1’b0；*/
...
/* I2S_RX_PDM_SINC_DSR_16_EN:R/W；位位置：[22]；默认值：1'0；*/
/*description:配置PDM RX滤波器组1模块的下采样率。1： 下采样率是128.0：下采样率为64。。*/
...
/* I2S_RX-PDM2PCM_EN:R/W；位位置：[21]；默认值：1'0；*/
/*description:1：启用PDM2PCM RX模式。0:D禁用。。*/
...
/*description:I2S Rx位尾数。1： 首先接收LSB。0:big endian，首先接收MSB。。*/
...
/*描述：0:接收左声道数据时WS应为0，右声道中WS为1。1： 接收左声道数据时WS应为1，右声道中WS应为0。*/
...
/*描述：1：存储24通道位到32位。0：将24通道位存储到24位。。*/
...
/*描述：0:I2S Rx仅在清除reg_Rx_start时停止。1： 当reg_rx_start为0或in_suc_eof为1时停止。2：当reg_rx.start为0或者rx FIFO满时停止I2S rx。。*/
...
/*描述：I2S RX压缩/解压缩配置位。&0（环礁）：A-Law解压缩，1（ltoa）：A-Law压缩，2（utol）：u-Law解压缩；3（ltou）：u-Law压缩。&。*/
...
/* I2S_RX更新：R/W/SC；位位置：[8]；默认值：1’b0；*/
/*description:设置1以将I2S RX寄存器从APB时钟域更新到I2S RX时钟域。更新寄存器完成后，硬件将清除此位。。*/
...
/*描述：I2S Rx字节端，1：低地址值到高地址。0：地址值低的低地址。。*/
...
/* I2S_RX_FIFO_RESET:WT；位位置：[1]；默认值：1’b0；*/
...
/*description:I2S Tx位端。1： 首先发送LSB。0:big endian，MSB首先发送。。*/
...
/*描述：I2S TX压缩/解压缩配置位。&0（环礁）：A-Law解压缩，1（ltoa）：A-Law压缩，2（utol）：u-Law解压缩；3（ltou）：u-Law压缩。&。*/
...
/* I2S_TX_UPDATE:R/W/SC；位位置：[8]；默认值：1’b0；*/
/*description:设置1以将I2S TX寄存器从APB时钟域更新到I2S TX时钟域。更新寄存器完成后，硬件将清除此位。。*/
...
/*描述：I2S Tx字节端，1：低地址值到高地址。0：地址值低的低地址。。*/
...
/* I2S_TX_RESET:WT；位位置：[0]；默认值：1’b0；*/
/*description:设置此位以重置变送器。*/
...
/*description:设置位以配置I2S接收器信道的有效数据位长度。7： 所有有效信道数据都是8位模式。15： 所有有效信道数据都处于16位模式。23：所有有效信道数据均为24位模式。31：所有有效信道数据均为32位模式。。*/
...
/*description:TDM模式下rx_ws_out的宽度为（I2S_rx_TDM_ws_width[6:0]+1）*T_bck。*/
...
/* I2S_TX_BCK_NO_DLY:R/W；位位置：[30]；默认值：1’b1；*/
/*描述：1:BCK在主模式下不延迟以生成正/负边缘。0:BCK被延迟以在主模式下生成正/负边缘。。*/
#define I2S_TX_BCK_NO_DLY    (BIT(30))
#define I2S_TX_BCK_NO_DLY_M  (BIT(30))
#define I2S_TX_BCK_NO_DLY_V  0x1
#define I2S_TX_BCK_NO_DLY_S  30
...
/*description:设置位以配置I2S发射机信道的有效数据位长度。7： 所有有效信道数据都是8位模式。15： 所有有效信道数据都处于16位模式。23：所有有效信道数据均为24位模式。31：所有有效信道数据均为32位模式。。*/
...
/*description:TDM模式下tx_ws_out的宽度为（I2S_tx_TDM_ws_width[6:0]+1）*T_bck。*/
...
#define I2S_RX_CLKM_CONF_REG(i)          (REG_I2S_BASE(i) + 0x30)
/* I2S_MCLK_SEL:R/W；位位置：[29]；默认值：1’b0；*/
/*description:0：将I2S Tx模块时钟用作I2S_MCLK_OUT。1： 将I2S Rx模块时钟用作I2S_MCLK_OUT。*/
...
#define I2S_TX_CLKM_CONF_REG(i)          (REG_I2S_BASE(i) + 0x34)
...
/*description:积分I2S TX时钟分频器值。f_ I2S_CLK＝f_。将有（a-b）n-div和b（n+1）-div。因此，平均组合将是：对于b<=a/2，z[x-n-div+（n+1）-div]+y-n-div。对于b>a/2，z[n-div+x（n+1）-div]+y（n+1）-div。*/
...
#define I2S_RX_CLKM_DIV_CONF_REG(i)          (REG_I2S_BASE(i) + 0x38)
...
/*description:对于b<=a/2，I2S_RX_CLKM_DIV_X的值为（a/b）-1。对于b>a/2，I_2S_RX_CLKM_DIV_X的值是（a/（a-b））-1。*/
...
/*description:对于b<=a/2，I2S_RX_CLKM_DIV_Y的值为（a%b）。对于b>a/2，I_2S_RX_CLKM_DIV_Y的值是（a%（a-b））。*/
...
/*description:对于b<=a/2，I2S_RX_CLKM_DIV_Z的值为b。对于b>a/2，I_2S_RX_CLKM_DIV_Z的值是（a-b）。*/
...
#define I2S_TX_CLKM_DIV_CONF_REG(i)          (REG_I2S_BASE(i) + 0x3C)
/* I2S_TX_CLKM_DIV_YN1:R/W；位位置：[27]；默认值：1’b0；*/
/*description:对于b<=a/2，I2S_TX_CLKM_DIV_YN1的值为0。对于b>a/2，I2S_TX_CCLKM_DIV_YN1的数值为1。*/
...
/*description:对于b<=a/2，I2S_TX_CLKM_DIV_X的值为（a/b）-1。对于b>a/2，则I2S_TX-CLKM_DIV_X的值是（a/（a-b））-1。*/
...
/*description:对于b<=a/2，I2S_TX_CLKM_DIV_Z的值为b。对于b>a/2，12S_TX_CLKM_DIV_Z的值为（a-b）。*/
...
/*description:1：启用I2S RX TDM信道$n的有效数据输入。0：禁用，只需在此通道中输入0。。*/
...
/*description:1：启用I2S RX TDM或PDM信道$n的有效数据输入。0：禁用，只需在此通道中输入0。。*/
...
/*description:1：启用I2S TX TDM信道$n的有效数据输出。0：禁用，仅在此通道中输出0。。*/
...
/* I2S_RX_SD3_IN_DM:R/W；比特位置：[13:12]；默认值：2'h0；*/
/*描述：I2S Rx SD3输入信号的延迟模式。0：旁路。1： pos边缘延迟。2： 负边缘延迟。3： 未使用。。*/
...
/* I2S_RX_SD2_IN_DM:R/W；比特位置：[9:8]；默认值：2'h0；*/
/*描述：I2S Rx SD2输入信号的延迟模式。0：旁路。1： pos边缘延迟。2： 负边缘延迟。3： 未使用。。*/
...
/* I2S_RX_SD1_IN_DM:R/W；比特位置：[5:4]；默认值：2'h0；*/
/*描述：I2S Rx SD1输入信号的延迟模式。0：旁路。1： pos边缘延迟。2： 负边缘延迟。3： 未使用。。*/
...
/*描述：接收数据位长度为（I2S_RX_BITS_MOD[4:0]+1）*（REG_RX_EOF_NUM[11:0]+1）。它将在配置的DMA RX信道中触发in_suc_EOF中断。。*/
...
/* I2S_DATE:R/W；位位置：[27:0]；默认值：28'h2009070；*/
/*description:I2S版本控制寄存器。*/
