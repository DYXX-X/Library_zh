...
typedef volatile struct spi_dev_s {
    ...
    union {
        struct {
            ...
            uint32_t dummy_out                     :    1;  /*0：在虚拟阶段，不输出FSPI总线信号。1： 在虚拟阶段，输出FSPI总线信号。可在形态状态下配置。*/
            ...
            uint32_t reserved22                    :    1;  /*保留*/
            uint32_t rd_bit_order                  :    2;  /*在读取数据（MISO）阶段1:LSB优先0:MSB优先。可在形态状态下配置。*/
            uint32_t wr_bit_order                  :    2;  /*在命令地址写入数据（MOSI）阶段1:LSB首先为0:MSB。可在形态状态下配置。*/
    ...
    union {
        struct {
            ...
            uint32_t clk_data_dtr_en               :    1;  /*1： SPI主DTR模式应用于SPI clk、数据和SPI_dqs。0:SPI主DTR模式仅适用于SPI_dqs。该钻头应与钻头17/18/19一起使用。*/
            uint32_t data_dtr_en                   :    1;  /*1： SPI clk以及SPI_DOUT和SPI_DIN状态的数据处于DTR模式，包括主控1/2/4/8-bm。0:SPI clk以及SPI_DOUT和SPI_DIN状态的数据处于STR模式。可在形态状态下配置。*/
            uint32_t addr_dtr_en                   :    1;  /*1： SPI clk和SPI_SEND_ADDR状态的数据处于DTR模式，包括主控1/2/4/8-bm。0:SPI clk和SPI_SEND_ADDR状态的数据处于STR模式。可在形态状态下配置。*/
            uint32_t cmd_dtr_en                    :    1;  /*1： SPI clk和SPI_SEND_CMD状态的数据处于DTR模式，包括主控1/2/4/8-bm。0:SPI clk和SPI_SEND_CMD状态的数据处于STR模式。可在形态状态下配置。*/
            uint32_t reserved20                    :    3;  /*保留*/
            uint32_t slave_cs_pol                  :    1;  /*spi从输入cs极性选择。1： 发票0：未更改。可在形态状态下配置。*/
            uint32_t dqs_idle_edge                 :    1;  /*spi_dqs的默认值。可在形态状态下配置。*/
            uint32_t reserved25                    :    4;  /*保留*/
            uint32_t ck_idle_edge                  :    1;  /*1： 当空闲0时spi-clk线为高：当空闲时spi-clk线为低。可在形态状态下配置。*/
            uint32_t cs_keep_active                :    1;  /*当位置位时，spi-cs线保持低电平。可在形态状态下配置。*/
            uint32_t quad_din_pin_swap             :    1;  /*1： SPI四输入交换启用，将FSPID与FSPIQ交换，将FSPIWP与FSPIHD交换。0:spi四输入交换禁用。可在形态状态下配置。*/
        };
        uint32_t val;
    } misc;
    union {
        struct {
            ...
            uint32_t din4_mode                     :    2;  /*输入信号被SPI模块时钟周期延迟，0：无延迟输入，1：具有clk_apb的偏旁输入，2：具有clk _apb的负边输入，3：具有SPI_clk的输入。可在形态状态下配置。*/
            uint32_t din5_mode                     :    2;  /*输入信号被SPI模块时钟周期延迟，0：无延迟输入，1：具有clk_apb的偏旁输入，2：具有clk _apb的负边输入，3：具有SPI_clk的输入。可在形态状态下配置。*/
            uint32_t din6_mode                     :    2;  /*输入信号被SPI模块时钟周期延迟，0：无延迟输入，1：具有clk_apb的偏旁输入，2：具有clk _apb的负边输入，3：具有SPI_clk的输入。可在形态状态下配置。*/
            uint32_t din7_mode                     :    2;  /*输入信号被SPI模块时钟周期延迟，0：无延迟输入，1：具有clk_apb的偏旁输入，2：具有clk _apb的负边输入，3：具有SPI_clk的输入。可在形态状态下配置。*/
    ...
    union {
        struct {
            ...
            uint32_t din4_num                      :    2;  /*输入信号延迟SPI模块时钟周期，0：延迟1个周期，1：延迟2个周期，……可配置为CONF状态。*/
            uint32_t din5_num                      :    2;  /*输入信号延迟SPI模块时钟周期，0：延迟1个周期，1：延迟2个周期，……可配置为CONF状态。*/
            uint32_t din6_num                      :    2;  /*输入信号延迟SPI模块时钟周期，0：延迟1个周期，1：延迟2个周期，……可配置为CONF状态。*/
            uint32_t din7_num                      :    2;  /*输入信号延迟SPI模块时钟周期，0：延迟1个周期，1：延迟2个周期，……可配置为CONF状态。*/
    ...
    union {
        struct {
            ...
            uint32_t dout4_mode                    :    1;  /*输出信号$n被SPI模块时钟延迟，0：无延迟输出，1：在其负边缘的SPI模块时钟周期的输出延迟。可在形态状态下配置。*/
            uint32_t dout5_mode                    :    1;  /*输出信号$n被SPI模块时钟延迟，0：无延迟输出，1：在其负边缘的SPI模块时钟周期的输出延迟。可在形态状态下配置。*/
            uint32_t dout6_mode                    :    1;  /*输出信号$n被SPI模块时钟延迟，0：无延迟输出，1：在其负边缘的SPI模块时钟周期的输出延迟。可在形态状态下配置。*/
            uint32_t dout7_mode                    :    1;  /*输出信号$n被SPI模块时钟延迟，0：无延迟输出，1：在其负边缘的SPI模块时钟周期的输出延迟。可在形态状态下配置。*/
            uint32_t d_dqs_mode                    :    1;  /*输出信号SPI_DQS被SPI模块时钟延迟，0：无延迟输出，1：负边缘SPI模块时钟周期的输出延迟。可在形态状态下配置。*/
    ...
    union {
        struct {
            uint32_t outfifo_empty                 :    1;  /*记录DMA TX FIFO的状态。1： DMA TX FIFO未准备好发送数据。0:DMA TX FIFO已准备好发送数据。*/
            uint32_t infifo_full                   :    1;  /*记录DMA RX FIFO的状态。1： DMA RX FIFO未准备好接收数据。0:DMA RX FIFO已准备好接收数据。*/
            uint32_t reserved2                     :    16;  /*保留*/
    ...
    union {
        struct {
            ...
            uint32_t app2                          :    1;  /*SPI_APP2_INT中断的原始位。该值仅由软件控制。*/
            uint32_t app1                          :    1;  /*SPI_APP1_INT中断的原始位。该值仅由软件控制。*/
            uint32_t reserved21                    :    11;  /*保留*/
        };
        uint32_t val;
    } dma_int_raw;
    ...
    union {
        struct {
            uint32_t infifo_full_err_int_set       :    1;  /*SPI_DMA_INFIFO_FULL_ERROR_INT中断的软件设置位。*/
            uint32_t outfifo_empty_err_int_set     :    1;  /*SPI_DMA_OUTFIFO_EMPTY_ERR_INT中断的软件设置位。*/
            uint32_t ex_qpi_int_set                :    1;  /*SPI从设备Ex_QPI中断的软件设置位。*/
            uint32_t en_qpi_int_set                :    1;  /*SPI从属En_QPI中断的软件设置位。*/
            uint32_t cmd7_int_set                  :    1;  /*SPI从属CMD7中断的软件设置位。*/
            uint32_t cmd8_int_set                  :    1;  /*SPI从属CMD8中断的软件设置位。*/
            uint32_t cmd9_int_set                  :    1;  /*SPI从属CMD9中断的软件设置位。*/
            uint32_t cmda_int_set                  :    1;  /*SPI从属CMDA中断的软件设置位。*/
            uint32_t rd_dma_done_int_set           :    1;  /*SPI_SLV_RD_DMA_DONE_INT中断的软件设置位。*/
            uint32_t wr_dma_done_int_set           :    1;  /*SPI_SLV_WR_DMA_DONE_INT中断的软件设置位。*/
            uint32_t rd_buf_done_int_set           :    1;  /*SPI_SLV_RD_BUF_DON_INT中断的软件设置位。*/
            uint32_t wr_buf_done_int_set           :    1;  /*SPI_SLV_WR_BUF_DON_INT中断的软件设置位。*/
            uint32_t trans_done_int_set            :    1;  /*SPI_TRANS_DONE_INT中断的软件设置位。*/
            uint32_t dma_seg_trans_done_int_set    :    1;  /*SPI_DMA_SEG_TRANS_DONE_INT中断的软件设置位。*/
            uint32_t seg_magic_err_int_set         :    1;  /*SPI_SEG_MAGIC_ERR_INT中断的软件设置位。*/
            uint32_t buf_addr_err_int_set          :    1;  /*SPI_SLV_BUF_ADDR_ERR_INT中断的软件设置位。*/
            uint32_t cmd_err_int_set               :    1;  /*SPI_SLV_CMD_ERR_INT中断的软件设置位。*/
            uint32_t mst_rx_afifo_wfull_err_int_set:    1;  /*SPI_MST_RX_AFIFO_WFULL_ERROR_INT中断的软件设置位。*/
            uint32_t mst_tx_afifo_rempty_err_int_set:    1;  /*SPI_MST_TX_AFIFO_REMPTY_ERR_INT中断的软件设置位。*/
            uint32_t app2_int_set                  :    1;  /*SPI_APP2_INT中断的软件设置位。*/
            uint32_t app1_int_set                  :    1;  /*SPI_APP1_INT中断的软件设置位。*/
            uint32_t reserved21                    :    11;  /*保留*/
        };
        uint32_t val;
    } dma_int_set;
    ...
    uint32_t data_buf[16];                                               /*SPI CPU控制的缓冲器0*/
