...
/** int_st寄存器系统中断状态寄存器的类型
 */
typedef union {
    struct {
        /** target0_int_st:RO；位位置：[0]；默认值：0；中断0状态
         */
        uint32_t target0_int_st: 1;
        /** target1_int_st:RO；位位置：[1]；默认值：0；中断1状态
         */
        uint32_t target1_int_st: 1;
        /** target2_int_st:RO；位位置：[2]；默认值：0；interupt2状态
         */
...
/** SYSTEM TIMER COMP STATUS REGISTER系统定时器补偿实际目标值低寄存器
 */
typedef struct {
    union {
        struct {
            /** target_lo_ro:ro；位位置：[31:0]；默认值：0；实际目标值低32位
             */
    ...
    union {
        struct {
            /** target_hi_ro:ro；比特位置：[19:0]；默认值：0；实际目标值高20位
             */
...
typedef union {
    struct {
        /** 日期：R/W；位位置：[31:0]；默认值：33628753；systimer寄存器版本
         */
        uint32_t date: 32;
    };
    uint32_t val;
