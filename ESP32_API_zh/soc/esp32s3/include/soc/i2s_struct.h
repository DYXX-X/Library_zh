...
typedef volatile struct i2s_dev_s {
    ...
    union {
        struct {
            ...
            uint32_t reserved6                     :    1;  /*储备*/
            uint32_t rx_big_endian                 :    1;  /*I2S Rx字节尾数，1：低地址值到高地址。0：地址值低的低地址。*/
            ...
            uint32_t rx_pcm_conf                   :    2;  /*I2S RX压缩/解压缩配置位。&0（atol）：A-Law解压缩，1（ltoa）：A-Law压缩，2（utol）：u-Law解压，3（ltou）：u-Law压缩&*/
            ...
            uint32_t rx_ws_idle_pol                :    1;  /*0:WS在接收左通道数据时应为0，WS在右通道中为1。1： 接收左声道数据时，WS应为1，右声道中WS应为0。*/
            uint32_t rx_bit_order                  :    1;  /*I2S Rx位尾数。1： 首先接收LSB。0:big endian，首先接收MSB。*/
            uint32_t rx_tdm_en                     :    1;  /*1： 启用I2S TDM Rx模式。0：禁用。*/
            uint32_t rx_pdm_en                     :    1;  /*1： 启用I2S PDM Rx模式。0：禁用。*/
            uint32_t rx_pdm2pcm_en                 :    1;  /*1： 启用PDM2PCM RX模式。0:D禁用。*/
            uint32_t rx_pdm_sinc_dsr_16_en         :    1;  /*配置PDM RX滤波器组1模块的下采样率。1： 下采样率是128.0：下采样率为64。*/
            uint32_t reserved23                    :    9;  /*储备*/
        };
        uint32_t val;
    } rx_conf;
    union {
        struct {
            ...
            uint32_t tx_big_endian                 :    1;  /*I2S Tx字节尾数，1：低地址值到高地址。0：地址值低的低地址。*/
            ...
            uint32_t tx_pcm_conf                   :    2;  /*I2S TX压缩/解压缩配置位。&0（atol）：A-Law解压缩，1（ltoa）：A-Law压缩，2（utol）：u-Law解压，3（ltou）：u-Law压缩&*/
            ...
            uint32_t tx_bit_order                  :    1;  /*I2S Tx位尾数。1： 首先发送LSB。0:big endian，首先发送MSB。*/
            ...
            uint32_t reserved28                    :    4;  /*储备*/
        };
        uint32_t val;
    } tx_conf;
    union {
        struct {
            uint32_t rx_tdm_ws_width               :    7;  /* TDM模式下rx_ws_out的宽度为（I2S_rx_TDM_ws_width[6:0]+1）*T_bck*/
            uint32_t rx_bck_div_num                :    6;  /*接收器模式中的位时钟配置位。*/
            uint32_t rx_bits_mod                   :    5;  /*设置位以配置I2S接收器信道的有效数据位长度。7： 所有有效信道数据都是8位模式。15： 所有有效信道数据都处于16位模式。23：所有有效信道数据均为24位模式。31：所有有效信道数据均为32位模式。*/
    ...
    union {
        struct {
            uint32_t tx_tdm_ws_width               :    7;  /* TDM模式下tx_ws_out的宽度为（I2S_tx_TDM_ws_width[6:0]+1）*T_bck*/
            uint32_t tx_bck_div_num                :    6;  /*发射机模式中的位时钟配置位。*/
            uint32_t tx_bits_mod                   :    5;  /*设置位以配置I2S发射机信道的有效数据位长度。7： 所有有效信道数据都是8位模式。15： 所有有效信道数据都处于16位模式。23：所有有效信道数据均为24位模式。31：所有有效信道数据均为32位模式。*/
    ...
    union {
        struct {
            uint32_t tx_clkm_div_num               :    8;  /*积分I2S TX时钟分频器值。f_ I2S_CLK＝f_。将有（a-b）*n-div和b*（n+1）-div。因此，平均组合将是：对于b<=a/2，z*[x*n-div+（n+1）-div]+y*n-div。对于b>a/2，z*[n-div+x*（n+1）-div]+y*（n+2）-div。*/
            uint32_t reserved8                     :    18;  /* 保留*/
    ...
    union {
        struct {
            uint32_t tx_clkm_div_z                 :    9;  /*对于b<=a/2，I2S_TX_CLKM_DIV_Z的值为b。对于b>a/2，I1S_TX_CLKM_DIV_Z的值为（a-b）。*/
            uint32_t tx_clkm_div_y                 :    9;  /*对于b<=a/2，I2S_TX_CLKM_DIV_Y的值为（a%b）。对于b>a/2，则I2S_TX-CLKM_DIV_Y的值是（a%（a-b））。*/
            uint32_t tx_clkm_div_x                 :    9;  /*对于b<=a/2，I2S_TX_CLKM_DIV_X的值为（a/b）-1。对于b>a/2，则I2S_TX-CLKM_DIV_X的值是（a/（a-b））-1。*/
    ...
    union {
        struct {
            uint32_t tx_hp_bypass                  :    1;  /*I2S TX PDM是否旁通高压过滤器。该选项已删除。*/
            uint32_t tx_sinc_osr2                  :    4;  /*I2S TX PDM OSR2值*/
            uint32_t tx_prescale                   :    8;  /*用于sigmadelta的I2S TX PDM预缩放*/
            uint32_t tx_hp_in_shift                :    2;  /*I2S TX PDM sigmadelta刻度移位数：0:/2，1:x1，2:x2，3:x4*/
            uint32_t tx_lp_in_shift                :    2;  /*I2S TX PDM sigmadelta刻度移位数：0:/2，1:x1，2:x2，3:x4*/
            uint32_t tx_sinc_in_shift              :    2;  /*I2S TX PDM sigmadelta刻度移位数：0:/2，1:x1，2:x2，3:x4*/
            uint32_t tx_sigmadelta_in_shift        :    2;  /*I2S TX PDM sigmadelta刻度移位数：0:/2，1:x1，2:x2，3:x4*/
            uint32_t tx_sigmadelta_dither2         :    1;  /*I2S TX PDM sigmadelta抖动2值*/
            uint32_t tx_sigmadelta_dither          :    1;  /*I2S TX PDM sigmadelta抖动值*/
            uint32_t tx_dac_2out_en                :    1;  /*I2S TX PDM dac模式启用*/
            uint32_t tx_dac_mode_en                :    1;  /*I2S TX PDM dac 2信道启用*/
    ...
    union {
        struct {
            uint32_t rx_tdm_chan0_en               :    1;  /*1： 启用I2S RX TDM或PDM信道$n的有效数据输入。0：禁用，只需在此通道中输入0。*/
            uint32_t rx_tdm_chan1_en               :    1;  /*1： 启用I2S RX TDM或PDM信道$n的有效数据输入。0：禁用，只需在此通道中输入0。*/
            uint32_t rx_tdm_chan2_en               :    1;  /*1： 启用I2S RX TDM或PDM信道$n的有效数据输入。0：禁用，只需在此通道中输入0。*/
            uint32_t rx_tdm_chan3_en               :    1;  /*1： 启用I2S RX TDM或PDM信道$n的有效数据输入。0：禁用，只需在此通道中输入0。*/
            uint32_t rx_tdm_chan4_en               :    1;  /*1： 启用I2S RX TDM或PDM信道$n的有效数据输入。0：禁用，只需在此通道中输入0。*/
            uint32_t rx_tdm_chan5_en               :    1;  /*1： 启用I2S RX TDM或PDM信道$n的有效数据输入。0：禁用，只需在此通道中输入0。*/
            uint32_t rx_tdm_chan6_en               :    1;  /*1： 启用I2S RX TDM或PDM信道$n的有效数据输入。0：禁用，只需在此通道中输入0。*/
            uint32_t rx_tdm_chan7_en               :    1;  /*1： 启用I2S RX TDM或PDM信道$n的有效数据输入。0：禁用，只需在此通道中输入0。*/
    ...
    union {
        struct {
            uint32_t tx_tdm_chan0_en               :    1;  /*1： 启用I2S TX TDM信道$n的有效数据输出。0：禁用，仅在该通道中输出0。*/
            uint32_t tx_tdm_chan1_en               :    1;  /*1： 启用I2S TX TDM信道$n的有效数据输出。0：禁用，仅在该通道中输出0。*/
            uint32_t tx_tdm_chan2_en               :    1;  /*1： 启用I2S TX TDM信道$n的有效数据输出。0：禁用，仅在该通道中输出0。*/
            uint32_t tx_tdm_chan3_en               :    1;  /*1： 启用I2S TX TDM信道$n的有效数据输出。0：禁用，仅在该通道中输出0。*/
            uint32_t tx_tdm_chan4_en               :    1;  /*1： 启用I2S TX TDM信道$n的有效数据输出。0：禁用，仅在该通道中输出0。*/
            uint32_t tx_tdm_chan5_en               :    1;  /*1： 启用I2S TX TDM信道$n的有效数据输出。0：禁用，仅在该通道中输出0。*/
            uint32_t tx_tdm_chan6_en               :    1;  /*1： 启用I2S TX TDM信道$n的有效数据输出。0：禁用，仅在该通道中输出0。*/
            uint32_t tx_tdm_chan7_en               :    1;  /*1： 启用I2S TX TDM信道$n的有效数据输出。0：禁用，仅在该通道中输出0。*/
            uint32_t tx_tdm_chan8_en               :    1;  /*1： 启用I2S TX TDM信道$n的有效数据输出。0：禁用，仅在该通道中输出0。*/
            uint32_t tx_tdm_chan9_en               :    1;  /*1： 启用I2S TX TDM信道$n的有效数据输出。0：禁用，仅在该通道中输出0。*/
            uint32_t tx_tdm_chan10_en              :    1;  /*1： 启用I2S TX TDM信道$n的有效数据输出。0：禁用，仅在该通道中输出0。*/
            uint32_t tx_tdm_chan11_en              :    1;  /*1： 启用I2S TX TDM信道$n的有效数据输出。0：禁用，仅在该通道中输出0。*/
            uint32_t tx_tdm_chan12_en              :    1;  /*1： 启用I2S TX TDM信道$n的有效数据输出。0：禁用，仅在该通道中输出0。*/
            uint32_t tx_tdm_chan13_en              :    1;  /*1： 启用I2S TX TDM信道$n的有效数据输出。0：禁用，仅在该通道中输出0。*/
            uint32_t tx_tdm_chan14_en              :    1;  /*1： 启用I2S TX TDM信道$n的有效数据输出。0：禁用，仅在该通道中输出0。*/
            uint32_t tx_tdm_chan15_en              :    1;  /*1： 启用I2S TX TDM信道$n的有效数据输出。0：禁用，仅在该通道中输出0。*/
            uint32_t tx_tdm_tot_chan_num           :    4;  /*I2S TX TDM模式的总信道数。*/
            uint32_t tx_tdm_skip_msk_en            :    1;  /*当DMA TX缓冲区存储（REG_TX_TDM_TOT_CHAN_NUM+1）信道的数据，并且仅发送启用信道的数据时，应设置此位。当DMA TX缓冲区中存储的所有数据都用于启用的信道时，清除该选项。*/
    ...
    union {
        struct {
            ...
            uint32_t rx_sd1_in_dm                  :    2;  /*I2S Rx SD1输入信号的延迟模式。0：旁路。1： pos边缘延迟。2： 负边缘延迟。3： 未使用。*/
            uint32_t reserved6                     :    2;
            uint32_t rx_sd2_in_dm                  :    2;  /*I2S Rx SD2输入信号的延迟模式。0：旁路。1： pos边缘延迟。2： 负边缘延迟。3： 未使用。*/
            uint32_t reserved10                    :    2;
            uint32_t rx_sd3_in_dm                  :    2;  /*I2S Rx SD3输入信号的延迟模式。0：旁路。1： pos边缘延迟。2： 负边缘延迟。3： 未使用。*/
    ...
    union {
        struct {
            uint32_t rx_eof_num                    :    12;  /*接收数据位长度为（I2S_RX_BITS_MOD[4:0]+1）*（REG_RX_EOF_NUM[11:0]+1）。它将在配置的DMA RX信道中触发in_suc_EOF中断。*/
            uint32_t reserved12                    :    20;  /* 保留*/
        };
        uint32_t val;
    } rx_eof_num;
