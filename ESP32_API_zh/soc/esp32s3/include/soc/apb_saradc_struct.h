...
typedef volatile struct apb_saradc_dev_s {
    union {
        struct {
            ...
            uint32_t work_mode                     :    2;  /* 0：单模式，1：双模式，2：交替模式*/
            uint32_t sar_sel                       :    1;  /* 0:SAR1，1:SAR2，仅适用于单一SAR模式*/
            ...
            uint32_t data_sar_sel                  :    1;  /*1： sar_sel将由16位输出数据的MSB编码，在这种情况下，分辨率不应大于11位。*/
    ...
    union {
        struct {
            ...
            uint32_t sar1_inv                      :    1;  /*1： DIG ADC1 CTRL的数据被反转，否则不会*/
    ...
    union {
        struct {
            uint32_t sar1_patt_tab                 :    24;  /*模式表1的项目0~3（每个项目一个字节）*/
            uint32_t reserved24                    :    8;
        };
        uint32_t val;
    } sar1_patt_tab[4];
    union {
        struct {
            uint32_t sar2_patt_tab                 :    24;  /*模式表2的项目0~3（每个项目一个字节）*/
            uint32_t reserved24                    :    8;
        };
        uint32_t val;
    } sar2_patt_tab[4];
    ...
    union {
        struct {
            ...
            uint32_t filter_channel0               :    5;  /*apb_adc1_过滤器因子*/
            uint32_t reserved24                    :    7;
            uint32_t filter_reset                  :    1;  /*启用apb_adc1_filter*/
        };
        uint32_t val;
    } filter_ctrl0;
    union {
    ...
    union {
        struct {
            ...
            uint32_t thres0_high                   :    13;  /*saradc1的thres0监视器thres*/
            uint32_t thres0_low                    :    13;  /*saradc1的thres0监视器thres*/
    ...
    union {
        struct {
            ...
            uint32_t thres1_high                   :    13;  /*saradc1的thres0监视器thres*/
            uint32_t thres1_low                    :    13;  /*saradc1的thres0监视器thres*/
            uint32_t reserved31                    :    1;
        };
        uint32_t val;
    } thres1_ctrl;
