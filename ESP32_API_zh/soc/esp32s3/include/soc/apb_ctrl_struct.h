...
typedef volatile struct apb_ctrl_dev_s {
    ...
    union {
        struct {
            uint32_t writeback_bypass              :    1;  /*设置1以绕过对外部内存的缓存写回请求，以便spi不会检查其属性。*/
            uint32_t reserved1                     :    31;
        };
        uint32_t val;
    ...
    union {
        struct {
            ...
            uint32_t flash_page_size               :    2;  /*设置所用MSPI闪存的页面大小。0:256字节。1： 512字节。2： 1024字节。3： 2048字节。*/
            uint32_t sram_page_size                :    2;  /*设置所用MSPI外部RAM的页面大小。0:256字节。1： 512字节。2： 1024字节。3： 2048字节。*/
            uint32_t reserved22                    :    10;  /*保留*/
        };
        uint32_t val;
    } spi_mem_ecc_ctrl;
