...
/*description:0：在虚拟阶段，不输出FSPI总线信号。1： 在虚拟阶段，输出FSPI总线信号。可在形态状态下配置。。*/
...
/*描述：1:SPI四输入交换启用，将FSPID与FSPIQ交换，将FSPIWP与FSPIHD交换。0:spi四输入交换禁用。可在形态状态下配置。。*/
...
/*描述：1:SPI clk和SPI_SEND_CMD状态的数据处于DTR模式，包括主控1/2/4/8-bm。0:SPI clk和SPI_SEND_CMD状态的数据处于STR模式。可在形态状态下配置。。*/
...
/*描述：1:SPI clk和SPI_SEND_ADDR状态的数据处于DTR模式，包括主控1/2/4/8-bm。0:SPI clk和SPI_SEND_ADDR状态的数据处于STR模式。可在形态状态下配置。。*/
...
/*描述：1:SPI clk以及SPI_DOUT和SPI_DIN状态的数据处于DTR模式，包括主控1/2/4/8-bm。0:SPI clk以及SPI_DOUT和SPI_DIN状态的数据处于STR模式。可在形态状态下配置。。*/
...
/*描述：1:SPI主DTR模式应用于SPI clk、data和SPI_dqs。0:SPI主DTR模式仅适用于SPI_dqs。该钻头应与钻头17/18/19一起使用。*/
...
/* SPI_DIN7模式：R/W；比特位置：[15:14]；默认值：2’b0；*/
...
/* SPI_DIN6_MODE：R/W；比特位置：[13:12]；默认值：2’b0；*/
/*描述：输入信号被SPI模块时钟周期延迟，0：输入无延迟，1：带clk_apb的偏旁输入，2：带clk _apb的负边输入，3：带SPI_clk的输入。可在形态状态下配置。。*/
...
/* SPI_DIN5_MODE：R/W；比特位置：[11:10]；默认值：2’b0；*/
/*描述：输入信号被SPI模块时钟周期延迟，0：输入无延迟，1：带clk_apb的偏旁输入，2：带clk _apb的负边输入，3：带SPI_clk的输入。可在形态状态下配置。。*/
...
/* SPI_DIN4_MODE：R/W；比特位置：[9:8]；默认值：2’b0；*/
/*描述：输入信号被SPI模块时钟周期延迟，0：输入无延迟，1：带clk_apb的偏旁输入，2：带clk _apb的负边输入，3：带SPI_clk的输入。可在形态状态下配置。。*/
...
/* SPI_DIN7_NUM:R/W；比特位置：[15:14]；默认值：2’b0；*/
...
/* SPI_DIN6_NUM：随机；比特位置：[13:12]；默认值：2’b0；*/
/*描述：输入信号延迟SPI模块时钟周期，0：延迟1个周期，1：延迟2个周期，…可配置为CONF状态。。*/
...
/* SPI_DIN5_NUM:R/W；比特位置：[11:10]；默认值：2’b0；*/
/*描述：输入信号延迟SPI模块时钟周期，0：延迟1个周期，1：延迟2个周期，…可配置为CONF状态。。*/
...
/* SPI_DIN4_NUM:R/W；比特位置：[9:8]；默认值：2’b0；*/
/*描述：输入信号延迟SPI模块时钟周期，0：延迟1个周期，1：延迟2个周期，…可配置为CONF状态。。*/
...
/* SPI_D_DQS_MODE：R/W；位位置：[8]；默认值：1’b0；*/
/*描述：输出信号SPI_DQS被SPI模块时钟延迟，0：无延迟输出，1：负边缘SPI模块时钟周期的输出延迟。可以配置为CONF状态。。*/
#define SPI_D_DQS_MODE    (BIT(8))
#define SPI_D_DQS_MODE_M  (BIT(8))
#define SPI_D_DQS_MODE_V  0x1
#define SPI_D_DQS_MODE_S  8
/* SPI_DOUT7_模式：R/W；位位置：[7]；默认值：1’b0；*/
...
/* SPI_MST_RX_AFIFO_wull_ERR_INT_RAW:R/WTC/SS；位位置：[17]；默认值：1’b0；*/
...
/* SPI_SLV_CMD_ERR_INT_RAW:R/WTC/SS；位位置：[16]；默认值：1’b0；*/
...
/* SPI_SLV_BUF_ADDR_ERR_INT_RAW:R/WTC/SS；位位置：[15]；默认值：1’b0；*/
...
/* SPI_SEG_MAGIC_ERR_INT_RAW:R/WTC/SS；位位置：[14]；默认值：1’b0；*/
...
/* SPI_DMA_SEG_TRANS_DONE_INT_RAW:R/WTC/SS；位位置：[13]；默认值：1’b0；*/
...
/* SPI_TRANS_DONE_INT_RAW:R/WTC/SS；位位置：[12]；默认值：1’b0；*/
...
/* SPI_SLV_WR_BUF_DONE_INT_RAW:R/WTC/SS；位位置：[11]；默认值：1’b0；*/
...
/* SPI_SLV_RD_BUF_DONE_INT_RAW:R/WTC/SS；位位置：[10]；默认值：1’b0；*/
...
/* SPI_SLV_WR_DMA_DONE_INT_RAW:R/WTC/SS；位位置：[9]；默认值：1’b0；*/
...
/* SPI_SLV_RD_DMA_DONE_INT_RAW:R/WTC/SS；位位置：[8]；默认值：1’b0；*/
...
/* SPI_SLV_CMDA_INT_RAW:R/WTC/SS；位位置：[7]；默认值：1’b0；*/
...
/* SPI_SLV_CMD9_INT_RAW:R/WTC/SS；位位置：[6]；默认值：1’b0；*/
...
/* SPI_SLV_CMD8_INT_RAW:R/WTC/SS；位位置：[5]；默认值：1’b0；*/
...
/* SPI_SLV_CMD7_INT_RAW:R/WTC/SS；位位置：[4]；默认值：1’b0；*/
...
/* SPI_SLV_EN_QPI_INT_RAW:R/WTC/SS；位位置：[3]；默认值：1’b0；*/
...
/* SPI_SLV_EX_QPI_INT_RAW:R/WTC/SS；位位置：[2]；默认值：1’b0；*/
...
/* SPI_DMA_OUTFIFO_EMPTY_ERR_INT_RAW:R/WTC/SS；位位置：[1]；默认值：1’b0；*/
...
/* SPI_DMA_INFIFO_FULL_ERR_INT_RAW:R/WTC/SS；位位置：[0]；默认值：1’b0；*/
...
/*description:软件重置启用，重置spi时钟线cs线和数据线。可在形态状态下配置。。*/
...
/*描述：｛CPOL，CPHA｝，1：支持spi-clk模式1和3，第一边缘输出数据B[0]/B[7]。0：支持spi clk模式0和2，第一边缘输出数据B[1]/B[6]。。*/
...
/*description:此位用于在主模式下选择SPI模块时钟源。1： PLL_CLK_80M。0:XTAL命令。。*/
...
/* SPI_日期：R/W；位位置：[27:0]；默认值：28'h2101190；*/
