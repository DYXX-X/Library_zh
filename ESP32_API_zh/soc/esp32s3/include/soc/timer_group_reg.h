...
/** TIMG_T0HI_REG寄存器定时器0当前值，高22位
 */
#define TIMG_T0HI_REG(i) (REG_TIMG_BASE(i) + 0x8)
/** TIMG_T0_HI:RO；比特位置：[21:0]；默认值：0；在写入TIMG_T0UPDATE_REG之后，这里可以读取计时器0的时基计数器的高22位。
 */
...
/** TIMG_T0_UPDATE:R/W/SC；位位置：[31]；默认值：0；在将0或1写入TIMG_T0UPDATE_REG后，计数器值被锁存。
 */
...
/** TIMG_T0_ALARM_HI:R/W；比特位置：[21:0]；默认值：0；计时器0报警触发时基计数器值，高22位。
 */
...
/** TIMG_T0_LOAD_LO:R/W；位位置：[31:0]；默认值：0；重新加载将加载到计时器0时基计数器上的值的低32位。
 */
...
/** TIMG_T0LOADHI_REG寄存器定时器0重新加载值，高22位
 */
#define TIMG_T0LOADHI_REG(i) (REG_TIMG_BASE(i) + 0x1c)
/** TIMG_T0_LOAD_HI:R/W；比特位置：[21:0]；默认值：0；重新加载将加载到计时器0时基计数器上的值的高22位。
 */
...
/** TIMG_T1_USE_XTAL:R/W；位位置：[9]；默认值：0；1： 使用XTAL_CLK作为定时器组的源时钟。0：使用APB_CLK作为定时器组的源时钟。
 */
...
/** TIMG_T1_ALARM_EN:R/W/SC；位位置：[10]；默认值：0；设置后，警报启用。一旦发生报警，该位将自动清除。
 */
...
/** TIMG_T1_DIVIDER:R/W；比特位置：[28:13]；默认值：1；计时器1时钟（T1_clk）预分频器值。
 */
...
/** TIMG_T1_AUTORELOAD:R/W；位位置：[29]；默认值：1；设置后，计时器1在报警时自动重新加载。
 */
...
/** TIMG_T1_增加：R/W；位位置：[30]；默认值：1；设置后，计时器1时基计数器将在每个时钟周期递增。清除后，计时器1时基计数器将递减。
 */
...
/** TIMG_T1_EN:R/W；位位置：[31]；默认值：0；设置后，启用计时器1时基计数器。
 */
...
/** TIMG_T1_LO:RO；位位置：[31:0]；默认值：0；在写入TIMG_T1UPDATE_REG之后，这里可以读取计时器1的时基计数器的低32位。
 */
...
/** TIMG_T1HI_REG寄存器定时器1电流值，高22位
 */
#define TIMG_T1HI_REG(i) (REG_TIMG_BASE(i) + 0x2c)
/** TIMG_T1_HI:RO；比特位置：[21:0]；默认值：0；在写入TIMG_T1UPDATE_REG之后，这里可以读取计时器1的时基计数器的高22位。
 */
...
/** TIMG_T1_UPDATE:R/W/SC；位位置：[31]；默认值：0；在将0或1写入TIMG_T1UPDATE_REG后，锁存计数器值。
 */
...
/** TIMG_T1_ALARM_LO:R/W；位位置：[31:0]；默认值：0；计时器1报警触发时基计数器值，低32位。
 */
...
/** TIMG_T1_ALARM_HI:R/W；比特位置：[21:0]；默认值：0；计时器1报警触发时基计数器值，高22位。
 */
...
/** TIMG_T1_LOAD_LO:R/W；位位置：[31:0]；默认值：0；重新加载将加载到计时器1时基计数器上的值的低32位。
 */
...
/** TIMG_T1LOADHI_REG寄存器定时器1重新加载值，高22位
 */
#define TIMG_T1LOADHI_REG(i) (REG_TIMG_BASE(i) + 0x40)
/** TIMG_T1_LOAD_HI:R/W；比特位置：[21:0]；默认值：0；重新加载将加载到计时器1时基计数器上的值的高22位。
 */
...
/** TIMG_T1LOAD_REG寄存器从TIMG_T1_（LOADLOLOADHI）_REG写入重新加载计时器
 */
#define TIMG_T1LOAD_REG(i) (REG_TIMG_BASE(i) + 0x44)
/** TIMG_T1_LOAD:WT；位位置：[31:0]；默认值：0；
 *
...
/** TIMG_RTC_CALI_CLK_SEL:R/W；比特位置：[14:13]；默认值：1；0:rtc慢时钟。1： clk_80m。2： xtal_32k中。
 */
...
/** TIMG_T0_INT_RAW:R/WTC/SS；位位置：[0]；默认值：0；TIMG_T0_INT中断的原始中断状态位。
 */
...
/** TIMG_T1_INT_RAW:R/WTC/SS；位位置：[1]；默认值：0；TIMG_T1_INT中断的原始中断状态位。
 */
...
/** TIMG_WDT_INT_RAW:R/WTC/SS；位位置：[2]；默认值：0；TIMG_WDT_INT中断的原始中断状态位。
 */
...
/** TIMG_T0_INT_CLR:WT；位位置：[0]；默认值：0；设置该位以清除TIMG_T0_INT中断。
 */
...
/** TIMG_T1_INT_CLR:WT；位位置：[1]；默认值：0；设置该位以清除TIMG_T1_INT中断。
 */
...
/** TIMG_WDT_INT_CLR:WT；位位置：[2]；默认值：0；设置该位以清除TIMG_WDT_INT中断。
 */
...
/** TIMG_NTIMERS_DATE:R/W；位位置：[27:0]；默认值：33566833；定时器版本控制寄存器
 */
...
/** TIMG_CLK_EN:R/W；位位置：[31]；默认值：0；寄存器时钟门信号。1： 软件读取和写入寄存器的时钟始终打开。0：软件读取和写寄存器的时钟仅在操作发生时退出。
 */
