 ...
 */
#define RTC_CNTL_DBIAS_SLP          0 //睡眠dig_dbias和rtcdbias
...
typedef enum {
    ...
    RTC_CPU_FREQ_SRC_8M,    //!< 内部18M RTC振荡器
...
typedef enum {
    ...
    RTC_SLOW_FREQ_RC32K = 2,   //!< 内部32KHz RC振荡器
} rtc_slow_freq_t;
...
typedef enum {
    ...
    RTC_CAL_RC32K = 1,        //!< 内部32 kHz RC振荡器
...
typedef struct {
    ...
    uint32_t clk_8m_dfreq : 10;      //!< RTC 8m时钟调整参数（值越高，频率越高）
    uint32_t root_clk_slt : 2;      //!< 为esp32h2选择时钟根源（默认为0:xtal_32M）
} rtc_clk_config_t;
...
/**
 * @brief 将新APB频率值存储到RTC_APB_FREQ_REG中
 *
 * 此功能不会更改任何硬件时钟。
 *
 * 执行频率切换和更改APB频率的功能调用此功能以更新RTC_APB_FREQ_REG（RTC通用保留寄存器之一）中存储的APB频率值。这通常不应从应用程序代码中调用。
 *
...
typedef struct {
    ...
    uint32_t dig_ret_pd_en : 1;               //!< 断电dig_ret
...
#define RTC_SLEEP_PD_DIG_RET            BIT(6)  //!< 关闭WIFI
...
#define RTC_GPIO_TRIG_EN            BIT(2)  //!< GPIO唤醒
...
typedef struct {
    /* 数据*/
    uint32_t or_en_cont_cal : 1;                //!< 默认值：0 rtc_init:0 pvt可以通过此寄存器或数字--if_en_cont_cal启用
    uint32_t enx_rtc_dreg : 1;                  //!< 默认值：1 rtc_init:1使用i2c寄存器配置rtc调节器电压电平，而不是pvt结果--int_dreg
    uint32_t enx_dig_dreg : 1;                  //!< 默认值：1 rtc_init:1使用i2c寄存器配置dig稳压器电压电平，而不是pvt结果--int_dreg
    uint32_t en_i2c_rtc_dreg : 1;               //!< 默认值：1 rtc_init:0 1:i2c_rtc_dreg；0:if_rtc_dreg
    uint32_t en_i2c_dig_dreg : 1;               //!< 默认值：1 rtc_init:0 1:i2c_dig_dreg；0:if_dig_dreg
    uint32_t en_i2c_rtc_dreg_slp : 1;           //!< 默认值：1 rtc_init:0 1:i2c_rtc_dreg_slp；0:if_rtc_dreg_slp
    uint32_t en_i2c_dig_dreg_slp : 1;           //!< 默认值：1 rtc_init:0 1:i2c_dig_dreg_slp；0:if_dig_dreg_slp
    uint32_t or_xpd_rtc_slave_3p3 : 1;          //!< 默认值：1 rtc_init:0以关闭rtc slave，这仅在DCDC运行之前需要
    uint32_t or_xpd_rtc_reg : 1;                //!< 默认值：1 rtc_init:0切换控制到数字--if_xpd_rtc_reg
    uint32_t or_xpd_dig_reg : 1;                //!< 默认值：1 rtc_init:0切换控制到数字--if_xpd_dig_reg
    uint32_t or_pd_rtc_reg_slp : 1;             //!< 默认值：0 rtc_init:1将此i2c配置为控制rtc_sleep_regulator开/关，无内核绑定数字控制信号
    uint32_t or_pd_dig_reg_slp : 1;             //!< 默认值：0 rtc_init:0默认值0将dig_sleep_regulator置于数字控制下--if_xpd_dig_reg_slp
    uint32_t or_xpd_dcdc : 1;                   //!< 默认值：1 rtc_init:0切换控制到数字--if_xpd_dcdc
    uint32_t or_disalbe_deep_sleep_dcdc : 1;    //!< 默认值：1 rtc_init:0切换控制到数字--if_enable_deep_sleep_dcdc
    uint32_t or_disalbe_light_sleep_dcdc : 1;   //!< 默认值：1 rtc_init:0切换控制到数字--if_enable_light_sleep_dcdc
    uint32_t or_enalbe_trx_mode_dcdc : 1;       //!< 默认值：1 rtc_init:0切换控制到数字--if_enable_trx_mode_dcdc
    uint32_t or_enx_reg_dcdc : 1;               //!< 默认值：0 rtc_init:1将dcdc配置寄存器切换到数字控制信号，包括popenb、sstime、ccm、vset、fsw、dcmlevel、dcm2enb、ramp、ramp level
    uint32_t or_unlock_dcdc : 1;                //!< 默认值：0 rtc_init:0未用于此版本的硅，如果金属将if_vgood_lock_dcdc信号更改为高，则可以释放
    uint32_t or_force_lock_dcdc : 1;            //!< 默认值：如果此寄存器设置为1，则将锁定0 rtc_init:0 dcdc并关闭
    uint32_t or_enb_slow_clk : 1;               //!< 默认值：0 rtc_init:1将慢速时钟控制切换到数字--if_enb_slow_clk
    uint32_t or_xpd_trx : 1;                    //!< 默认值：1 rtc_init:0将trx控制切换到数字--if_xpd_trx
    uint32_t or_en_reset_chip : 1;              //!< 默认值：0 rtc_init:1切换重置芯片控制到数字--if_reset_chip
    uint32_t or_force_xpd_reg_slave : 1;        //!< 默认值：0 rtc_init:1在DCDC就绪后将此reg设置为1，以使rtc&dig从属控制独立于DCDC状态
} pmu_config_t;
...
typedef struct {
    uint32_t swt_idle: 1;       //!< 如果为1，则swt_idle为睡眠模式；如果为0，则swt_idle为活动模式
    uint32_t swt_monitor: 1;    //!< 如果为1，则swt_monitor为睡眠模式；如果为0，则swt_monitor为活动模式
    uint32_t swt_slp: 1;        //!< 如果为1，则swt_slp为睡眠模式；如果为0，则swt_slp为活动模式
} dbias_swt_cfg_t;
...
typedef struct {
    /* 数据*/
    uint32_t dig_regul0_en: 1;  //!< 如果为1，则dig_regulator0是fsm的ctl；如果为0，dig_regulator 0强制pd。
    uint32_t dig_regul1_en: 1;  //!< 如果为1，则dig_regulator1是fsm的ctl；如果为0，dig_regulator1强制pd。
    uint32_t rtc_regul0_en: 1;  //!< 如果1，则rtc_regulator0是fsm的ctl；如果为0，则rtc_regulator0强制pd。
} regulator_cfg_t;
...
/* 为esp32h2选择时钟根源。返回源clk freq_mhz
 */
