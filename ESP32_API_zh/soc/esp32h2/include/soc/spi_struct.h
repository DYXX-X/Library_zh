...
typedef volatile struct spi_dev_s {
    ...
    union {
        struct {
            ...
            uint32_t hold_pol:     1;                          /*SPI空闲时的SPI_HOLD输出值。1： 输出高0：输出低。可在形态状态下配置。*/
            uint32_t wp_pol:       1;                          /*SPI空闲时的写保护信号输出。1： 输出高0：输出低。可在形态状态下配置。*/
    ...
    union {
        struct {
            ...
            uint32_t rx_afifo_rst:            1;               /*设置此位以重置RX AFIFO，该AFIFO用于在SPI主模式和从模式传输中接收数据。*/
            uint32_t buf_afifo_rst:           1;               /*设置此位以重置BUF TX AFIFO，其用于在SPI从CPU控制模式传输和主模式传输中发送数据。*/
            uint32_t dma_afifo_rst:           1;               /*设置此位以重置DMA TX AFIFO，该AFIFO用于在SPI从属DMA控制模式传输中发送数据。*/
        };
        uint32_t val;
    } dma_conf;
    union {
