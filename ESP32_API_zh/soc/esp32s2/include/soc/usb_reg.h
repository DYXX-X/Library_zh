...
/** USB_SUPCNT4:R/W；位位置：[31:29]；默认值：0；SETUP数据包计数（SUPCnt）。此字段指定端点可以接收的背靠背SETUP数据数据包的数量2’b01：1包2’b10：2包2’b11：3包
 */
...
/** USB_DMABUFFERADDR4:R/W；位位置：[32:0]；默认值：0；保存当前缓冲区地址。当相应端点的数据传输正在进行时，此寄存器将被更新。此寄存器仅在分散/聚集DMA模式下存在。否则将保留此字段。
 */
...
/** USB_XFERSIZE5:R/W；位位置：[7:0]；默认值：0；传输大小。表示ENDPOINT5的传输大小（字节）
 */
...
/** USB_PKTCNT5:R/W；位位置：[19]；默认值：0；数据包计数（PktCnt）。数据包写入RxFIFO后，此字段将减为零。
 */
...
/** USB_SUPCNT5:R/W；位位置：[31:29]；默认值：0；SETUP数据包计数（SUPCnt）。此字段指定端点可以接收的背靠背SETUP数据数据包的数量2’b01：1包2’b10：2包2’b11：3包
 */
...
/** USB_DMABUFFERADDR5:R/W；位位置：[32:0]；默认值：0；保存当前缓冲区地址。当相应端点的数据传输正在进行时，此寄存器将被更新。此寄存器仅在分散/聚集DMA模式下存在。否则将保留此字段。
 */
...
/** USB_MODEMIS：R/W1C；位位置：[1]；默认值：0；模式不匹配中断。当应用程序试图访问时，内核设置此位：当控制器在设备模式下运行时，主机模式寄存器
 */
...
/** USB_OTGINT:RO；位位置：[2]；默认值：0；OTG中断。控制器设置该位以指示OTG协议事件。应用程序必须读取OTG中断状态（GOTGINT_REG）寄存器，以确定导致此中断的确切事件。应用程序必须清除GOTGINT_REG寄存器中的相应状态位才能清除此位。
 */
...
#define USB_SOF_S  3
/** USB_rxvlvi:RO；位位置：[4]；默认值：0；RxFIFO非空。指示至少有一个数据包等待从RxFIFO 1’b0:Rx Fifo为空1’b1：Rx Fivo不为空
 */
#define USB_RXFLVI    (BIT(4))
#define USB_RXFLVI_M  (USB_RXFLVI_V << USB_RXFLVI_S)
#define USB_RXFLVI_V  0x00000001
#define USB_RXFLVI_S  4
/** USB_NPTXFEMP:RO；位位置：[5]；默认值：0；非周期TxFIFO空。当非周期性TxFIFO为半空或完全空，并且有空间将至少一个条目写入非周期性传输请求队列时，该中断被断言。半空或完全空状态由核心AHB配置寄存器（GAHBCFG_REG.USB_NPTXFEMPLVL）中的非周期TxFIFO空电平位确定。
 */
...
/** USB_GINNAKEFF:RO；位位置：[6]；默认值：0；仅限设备。全局IN非周期NAK有效。指示应用程序设置的设备控制寄存器（DCTL.SGPINNak）中的设置全局非周期性IN NAK位已在核心中生效。也就是说，内核已经对应用程序的全局IN NAK位集进行了采样。可以通过清除设备控制寄存器（DCTL.CGNPNNak）中的清除全局非周期性IN NAK位来清除该位。此中断不一定意味着在USB上发送NAK握手。STALL位优先于NAK位。
 */
...
/** USB_GOUTNAKEFF:RO；位位置：[7]；默认值：0；仅限设备。全局输出NAK有效。表示由应用程序设置的设备控制寄存器（DCTL_REG.USB_SGOUTNAK）中的Set Global OUT NAK位已在内核中生效。通过将清除全局输出NAK位写入设备控制寄存器（DCTL_REG.REG_CGOUTNAK），可以清除该位。
 */
...
/** USB_ERLYSUSP:R/W1C；位位置：[10]；默认值：0；仅限设备。提前暂停。控制器设置该位以指示USB上已检测到空闲状态3毫秒。
 */
...
/** USB_USBSUSP:R/W1C；位位置：[11]；默认值：0；仅限设备。USB挂起。控制器设置该位以指示在USB上检测到挂起。当线路状态信号在延长的时间段内没有活动时，控制器进入暂停状态。
 */
...
/** USB_USBRST:R/W1C；位位置：[12]；默认值：0；仅限设备。USB重置。控制器设置该位以指示在USB上检测到重置
 */
...
/** USB_ENUMDONE:R/W1C；位位置：[13]；默认值：0；仅限设备。枚举完成。核心设置此位以指示速度枚举已完成。应用程序必须读取设备状态（DSTS_REG）寄存器以获得枚举速度。
 */
...
/** USB_ISOOUTDROP:R/W1C；位位置：[14]；默认值：0；仅限设备。等时OUT数据包丢失中断。当由于RxFIFO没有足够的空间来容纳同步OUT端点的最大分组大小分组而未能将同步OUT分组写入RxFIFO时，控制器设置该位。
 */
...
/** USB_D_intknepm为1:R/W1C；位位置：[5]；默认值：0；0x0：未接收到带EP不匹配中断的IN令牌0x1：接收到带IP不匹配中断
 */
...
/** USB_H_PKTCNT2:R/W；位：[29:19]；默认值：0；当处于非分散/聚集DMA模式时：数据包计数。此字段由应用程序使用预期发送（OUT）或接收（in）的数据包数进行编程。分散/聚集DMA模式：保留。
 */
...
/** USB_H_PKTCNT3:R/W；位：[29:19]；默认值：0；当处于非分散/聚集DMA模式时：数据包计数。此字段由应用程序使用预期发送（OUT）或接收（in）的数据包数进行编程。分散/聚集DMA模式：保留。
 */
...
/** USB_H_PKTCNT4:R/W；位：[29:19]；默认值：0；当处于非分散/聚集DMA模式时：数据包计数。此字段由应用程序使用预期发送（OUT）或接收（in）的数据包数进行编程。分散/聚集DMA模式：保留。
 */
...
/** USB_H_PKTCNT5:R/W；位：[29:19]；默认值：0；当处于非分散/聚集DMA模式时：数据包计数。此字段由应用程序使用预期发送（OUT）或接收（in）的数据包数进行编程。分散/聚集DMA模式：保留。
 */
...
/** USB_H_PKTCNT6:R/W；位：[29:19]；默认值：0；当处于非分散/聚集DMA模式时：数据包计数。此字段由应用程序使用预期发送（OUT）或接收（in）的数据包数进行编程。分散/聚集DMA模式：保留。
 */
...
/** USB_dfi深度：RO；位位置：[32:16]；默认值：256；DFIFO深度。此值以32位字表示。
 */
...
/** USB_FRREM:RO；位位置：[32:16]；默认值：0；剩余帧时间。以PHY时钟表示当前微帧（HS）或帧（FS/LS）中剩余的时间量。
 */
