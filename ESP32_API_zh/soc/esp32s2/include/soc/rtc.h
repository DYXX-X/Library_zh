...
/** @brief 此SoC的固定晶体频率
    ...
    在仅支持一个晶体频率的SoC上，使用此宏可以替代调用rtc_clk_xtal_freq_get（）
 */
 ...
 */
typedef enum {
    RTC_SLOW_FREQ_RTC = 0,      //!< 内部90 kHz RC振荡器
...
typedef struct {
    ...
    uint32_t slow_clk_dcap : 8;     //!< RTC 90k时钟调整参数（值越高，频率越低）
 ...
 * @brief 获取主XTAL频率
 *
 * 结果是一个常数，因为XTAL频率是固定的。
 *
 * @note 功能仅适用于ESP32兼容代码。仅需要支持此SoC的代码可以使用宏RTC_XTAL_FREQ作为此SoC固定晶体值。
 *
 * @return XTAL频率（MHz），RTC_XTAL_FREQ_40M
 */
 ...
 *
 * -如果选择RTC_SLOW_FREQ_RTC，则返回~90000
 ...
 */
typedef struct {
    uint32_t dig_fpu : 1;    //!< 设置为1以关闭休眠状态下的数字部件
    uint32_t rtc_fpu : 1;    //!< 设置为1以关闭睡眠中的RTC内存
    uint32_t cpu_fpu : 1;    //!< 设置为1可关闭休眠状态下的数字存储器和CPU
    uint32_t i2s_fpu : 1;    //!< 设置为1以关闭休眠状态下的I2S
    uint32_t bb_fpu : 1;     //!< 设置为1可在睡眠中关闭WiFi
    uint32_t nrx_fpu : 1;    //!< 设置为1可在睡眠中关闭WiFi
    uint32_t fe_fpu : 1;     //!< 设置为1可在睡眠中关闭WiFi
} rtc_sleep_pd_config_t;

/**
 * rtc_sleep_pd_config_t的初始化器，它将所有标志设置为相同的值
 */
...
typedef struct {
    ...
    uint32_t dig_dbias_wak : 3;         //!< 设置数字域的偏置，处于活动模式
    uint32_t dig_dbias_slp : 3;         //!< 在休眠模式下为数字域设置偏置
    uint32_t rtc_dbias_wak : 3;         //!< 设置RTC域的偏置，处于活动模式
    uint32_t rtc_dbias_slp : 3;         //!< 设置RTC域的偏置，处于睡眠模式
...
#define RTC_SLEEP_PD_INT_8M             BIT(7)  //!< 断电内部8M振荡器
#define RTC_SLEEP_PD_XTAL               BIT(8)  //!< 关闭主XTAL电源
...
/**
 * 使用有效的硬件校准值校准slowclk如果没有正在进行的硬件校准，请启动硬件校准并等待校准完成
 * @param cal_clk 待测时钟
 * @param slowclk_cycles 如果没有进行硬件校准，请使用此数量的慢周期来校准slowclk。
 */
