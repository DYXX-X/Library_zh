...
typedef volatile struct spi_mem_dev_s {
    union {
        struct {
            ...
            uint32_t flash_pe:       1;                 /*在用户模式下，设置为指示将触发编程/擦除操作。该位与spi_mem_usr位组合。一旦操作完成，该位将被清除。1：启用0：禁用。*/
            uint32_t usr:            1;                 /*用户定义命令启用。设置位时将触发操作。一旦操作完成，该位将被清除。1：启用0：禁用。*/
            uint32_t flash_hpm:      1;                 /*将Flash驱动到高性能模式。一旦操作完成，该位将被清除。1：启用0：禁用。*/
            uint32_t flash_res:      1;                 /*此位与reg_resandres位结合使用，将Flash从断电状态或高性能模式释放，并获取设备ID。一旦操作完成，该位将被清除。1:启用0:禁用。*/
            uint32_t flash_dp:       1;                 /*将闪存驱动器关闭。设置位时将触发操作。一旦操作完成，该位将被清除。1：启用0：禁用。*/
            uint32_t flash_ce:       1;                 /*芯片擦除启用。设置位时将触发芯片擦除操作。一旦操作完成，该位将被清除。1：启用0：禁用。*/
            uint32_t flash_be:       1;                 /*块擦除启用（32KB）。设置位时将触发块擦除操作。一旦操作完成，该位将被清除。1：启用0：禁用。*/
            uint32_t flash_se:       1;                 /*扇区擦除启用（4KB）。设置位时将触发扇区擦除操作。一旦操作完成，该位将被清除。1：启用0：禁用。*/
            uint32_t flash_pp:       1;                 /*页面编程启用（要编程的数据为1字节~256字节）。设置位时将触发页面程序操作。一旦操作完成，该位将被清除。1：启用0：禁用。*/
            uint32_t flash_wrsr:     1;                 /*写入状态寄存器启用。设置位时将触发写入状态操作。一旦操作完成，该位将被清除。1：启用0：禁用。*/
            uint32_t flash_rdsr:     1;                 /*读取状态寄存器-1。设置位时将触发读取状态操作。一旦操作完成，该位将被清除。1：启用0：禁用。*/
