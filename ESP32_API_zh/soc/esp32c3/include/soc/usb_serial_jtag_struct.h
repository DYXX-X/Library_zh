...
typedef volatile struct usb_serial_jtag_dev_s {
    union {
        struct {
            uint32_t rdwr_byte                     :    32;  /*虽然只有低8位有效，但将其更改为32位以避免没有读取/修改/写入行为*/ /*通过该字段向UART Tx/Rx FIFO写入和读取字节数据。当设置了USB_SERIAL_JTAG_SERIAL_IN_EMPTY_INT时，用户可以将数据（最多64字节）写入UART Tx FIFO。当设置了USB_SERIAL_JTAG_SERIAL_OUT_RECV_PKT_INT时，用户可以检查USB_SERIAL_JTAG_OUT_EP1_WR_ADDR和USB_SERIAL-JTAG_OUT_EP0_RD_ADDR，以了解接收了多少数据，然后从UART Rx FIFO读取该数据量。*/
        };
        uint32_t val;
    } ep1;
    union {
