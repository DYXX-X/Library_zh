...
typedef volatile struct apb_saradc_dev_s {
    union {
        struct {
            ...
            uint32_t start:                   1;
            uint32_t reserved2:               4;              /*0：单模式1：双模式2：交替模式*/
            uint32_t sar_clk_gated:           1;
            uint32_t sar_clk_div:             8;              /*SAR时钟分频器*/
            uint32_t sar_patt_len:            3;              /*0~15表示长度1~16*/
            uint32_t reserved18:              5;
            uint32_t sar_patt_p_clear:        1;              /*清除DIG ADC1 CTRL的模式表指针*/
    ...
    union {
        struct {
            ...
            uint32_t reserved11:            1;                /*1： 选择saradc计时器0:i2s_ws触发器*/
    ...
    union {
        struct {
            uint32_t sar_patt_tab1:       24;                 /*模式表1的项目0~3（每个项目一个字节）*/
            uint32_t reserved24:           8;
        };
        uint32_t val;
    } sar_patt_tab[2];
    ...
    union {
        struct {
            ...
            uint32_t filter_channel0: 4;                      /*apb_adc1_过滤器因子*/
