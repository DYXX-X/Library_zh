...
#ifdef ESP32C2  // ESP32C2 ROM使用mbedtls_md5
struct MD5Context {  // 在ROM中调用mbedtls_md5_context
...
#if ESP32S2_OR_LATER
uint32_t GetSecurityInfoProc(int* pMsg, int* pnErr, uint8_t *buf);  // ROM中未使用的pMsg和pnErr
SpiFlashOpResult SPI_read_status_high(esp_rom_spiflash_chip_t *spi, uint32_t *status);
#else
/* 注意：在ESP32上，这是一个静态函数，其第一个参数被编译器忽略。*/
