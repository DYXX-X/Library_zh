if (bytes >= 256) {
    ...
    /* _mm_shuffle_epi32的元素广播立即顺序为：0x00、0x55、0xaa、0xff*/
    ...
    while (bytes >= 256) {
        ...
        for (i = 0; i < ROUNDS; i += 2) {
            /* 内部循环是amd64-xmm6 ASM的直接转换（正则表达式搜索/替换）*/
...
/* 存储数据；该宏将shuffle+mov替换为直接提取；差别不大*/
...
/* 存储数据；这个宏首先在寄存器中调换数据，然后将它们存储在内存中。使用icc更快。*/
