...
/*
 h=f+g可以与f或g重叠。
 *
 前提条件：|f|以1.12^25,1.12^24,1.12^25,1.12 ^24等为界|g|以1.12^25,1.12^24,1.12^25,1.12 ^24等为界。
 *
 后置条件：|h|以1.12^26,1.12^25,1.12^26,1.12 ^25等为界。
 */
...
/*
 h=-f
 *
 前提条件：|f|以1.12^25,1.12^24,1.12^25,1.12 ^24等为界。
 *
 后置条件：|h|以1.12^25,1.12^24,1.12^25,1.12 ^24等为界。
 */
...
/*
 如果b==1，则将（f，g）替换为（g，g）；如果b==0，则将（f，g）替换为（f，g）。
 *
 前提条件：｛0,1｝中的b。
 */
...
/*
 如果f在｛1,3,5，…，q-2｝中，则返回1

 前提条件：|f|以1.12^26,1.12^25,1.12^26,1.12 ^25等为界。
 */
...
/*
 h = f * g可以与f或g重叠。
 *
 前提条件：|f|以1.652^26,1.652^25,1.652^ 26,1.652 ^25等为界|g |以1.652^26,1.652^25,1.652^2 6,1.652^25等为界。
 *
 后置条件：|h|以1.012^25,1.012^24,1.012^ 25,1.012*^24等为界。
 */

/*
 实施战略说明：
 *
 使用课本乘法。Karatsuba在一些成本模型中会节省一点。
 *
 大多数乘以2和19的乘法是32位预计算；比64位后计算更便宜。
 *
 在进位链中有一个剩余的乘以19；可以将一个19预计算合并到其中，但生成的数据流要少得多。
 *
 下面有12个进位。其中10个是双向并行和矢量化的。可以通过11次进位，但数据流要深得多。
 *
 如果对输入有更严格的约束，则可以将进位压缩到int32中。
 */
...
fe25519_mul(fe25519 h, const fe25519 f, const fe25519 g)
{
    ...
    int32_t g2_19 = 19 * g2; /* 1.959375*2^30; 还可以*/
    int32_t g3_19 = 19 * g3;
    int32_t g4_19 = 19 * g4;
    int32_t g5_19 = 19 * g5;
    int32_t g6_19 = 19 * g6;
    ...
    /*
     |h0| <= (1.65*1.65*2^52*(1+19+19+19+19)+1.65*1.65*2^50*(38+38+38+38+38)) i、 e.|h0|<=1.42^60；h2、h4、h6、h8的较窄范围|h1|<=（1.651.652^51（1+1+19+19+19+19/19+19+19）），即|h1|<1.72^59；h3、h5、h7、h9的范围更窄
     */
    ...
    /* |h2|<=2^25；从现在起，int32不变*/
    /* |h6|<=2^25；从现在起，int32不变*/
    /* |h3|<=1.71*2^59*/
    /* |h7 |<=1.71*2^59*/
    ...
    /* |h3|<=2^24；从现在起，int32不变*/
    /* |h7|<=2^24；从现在起，int32不变*/
    /* |h4|<=1.72*2^34*/
    /* |h8|<=1.41*2^60*/
    ...
    /* |h4|<=2^25；从现在起，int32不变*/
    /* |h8|<=2^25；从现在起，int32不变*/
    /* |h5|<=1.01*2^24*/
    /* |h9 |<=1.71*2^59*/
    ...
    /* |h9|<=2^24；从现在起，int32不变*/
    /* |h0|<=1.1*2^39*/

    carry0 = (h0 + (int64_t)(1L << 25)) >> 26;
    h1 += carry0;
    h0 -= carry0 * ((uint64_t) 1L << 26);
    /* |h0|<=2^25；从现在起，int32不变*/
    /* |h1|<=1.01*2^24*/
...
/*
 h = f * f可以与h重叠。
 *
 前提条件：|f|以1.652^26,1.652^25,1.652^ 26,1.652 ^25等为界。
 *
 后置条件：|h|以1.012^25,1.012^24,1.012^ 25,1.012*^24等为界。
 */
