...
/** mDNS严格模式：将此值设置为1，使mDNS库严格遵循RFC6762:严格功能：
 *   -不根据RFC6762第6节在响应包中设置原始问题
 *
 * 实际配置为0，即非严格模式，因为某些实现，如lwIP mdns解析器（由标准POSIX API使用，如getaddrinfo、gethostbyname）无法正确解析播发的名称。
 */
...
#if !MDNS_STRICT_MODE
/* mDNS响应者有时会在响应中重复查询，但根据RFC6762第6节：响应中不得包含任何问题字段中的项目*/
...
#define MDNS_SERVICE_PORT           5353                    // 服务器运行的UDP端口
...
#define MDNS_PACKET_QUEUE_LEN       16                      // 可排队进行分析的最大数据包数
#define MDNS_ACTION_QUEUE_LEN       16                      // 服务器挂起的最大操作数
#define MDNS_TXT_MAX_LEN            1024                    // TXT记录中文本数据的最大字符串长度
#define MDNS_NAME_MAX_LEN           64                      // 主机名、实例、服务和proto的最大字符串长度
#define MDNS_NAME_BUF_LEN           (MDNS_NAME_MAX_LEN+1)   // 容纳主机名、实例、服务或proto的最大字符缓冲区大小
#define MDNS_MAX_PACKET_SIZE        1460                    // mDNS传出数据包的最大大小
...
typedef struct {
    char host[MDNS_NAME_BUF_LEN]; // A/AAAA记录的主机名，SRV记录的实例名
...
typedef struct mdns_txt_linked_item_s {
    ...
    char * value;                           /*!< 项目值字符串*/
    uint8_t value_len;                      /*!< 项目值长度*/
    struct mdns_txt_linked_item_s * next;   /*!< 下一个结果，或列表中最后一个结果为NULL*/
...
/*
 * @brief  将mnds if转换为esp netif句柄
 *
 * @param  tcpip_if     mdns支持的接口作为内部枚举
 *
 * @return
 *     -成功论者
 *     -如果当前接口索引没有可用的netif，则为NULL
 */
