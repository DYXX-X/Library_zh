...
#define ESP32H2_ERR_HW_CRYPTO_DS_HMAC_FAIL           ESP_ERR_HW_CRYPTO_BASE + 0x1 /*!< HMAC外围问题*/
#define ESP32H2_ERR_HW_CRYPTO_DS_INVALID_KEY         ESP_ERR_HW_CRYPTO_BASE + 0x2 /*!< 给定HMAC密钥不正确，HMAC外围问题*/
#define ESP32H2_ERR_HW_CRYPTO_DS_INVALID_DIGEST      ESP_ERR_HW_CRYPTO_BASE + 0x4 /*!< 消息摘要检查失败，结果无效*/
#define ESP32H2_ERR_HW_CRYPTO_DS_INVALID_PADDING     ESP_ERR_HW_CRYPTO_BASE + 0x5 /*!< 填充检查失败，但仍会生成结果，并且可以读取*/
 ...
 * 数字签名使用的明文参数。
 *
 * 这仅用于通过调用esp_ds_encrypt_params（）加密RSA参数。然后，结果可以存储在闪存或其他持久存储器中。加密是进行任何签名操作之前的先决步骤。
 */
typedef struct {
    uint32_t  Y[ESP_DS_SIGNATURE_MAX_BIT_LEN / 32]; //!< RSA指数
    uint32_t  M[ESP_DS_SIGNATURE_MAX_BIT_LEN / 32]; //!< RSA模数
    uint32_t Rb[ESP_DS_SIGNATURE_MAX_BIT_LEN / 32]; //!< RSA r反操作数
    uint32_t M_prime;                               //!< RSA M素数操作数
    uint32_t length;                                //!< RSA字长度（32位）
} esp_ds_p_data_t;

/**
 * @brief 使用特定密钥槽中的硬件密钥对消息进行签名。
 *
 ...
 *      -如果消息摘要不匹配，则为ESP_ERR_HW_CRYPTO_DS_INVALID_DIGEST；签名无效。这意味着加密的RSA密钥参数无效，表明它们可能已被篡改或指示闪存错误等。
 *      -ESP_ERR_HW_CRYPTO_DS_INVALID_PADDING如果消息填充不正确，则可以读取签名，因为消息摘要匹配（有关详细信息，请参阅TRM）。
 */
 ...
 * @brief 加密私钥参数。
 *
 * 加密是进行任何签名操作之前的先决步骤。严格来说，不必使用此加密功能，加密也可以在外部设备上进行。
 *
