...
#define ESP_ERR_HW_CRYPTO_DS_HMAC_FAIL           ESP_ERR_HW_CRYPTO_BASE + 0x1 /*!< HMAC外围问题*/
#define ESP_ERR_HW_CRYPTO_DS_INVALID_KEY         ESP_ERR_HW_CRYPTO_BASE + 0x2 /*!< 给定HMAC密钥不正确，HMAC外围问题*/
#define ESP_ERR_HW_CRYPTO_DS_INVALID_DIGEST      ESP_ERR_HW_CRYPTO_BASE + 0x4 /*!< 消息摘要检查失败，结果无效*/
#define ESP_ERR_HW_CRYPTO_DS_INVALID_PADDING     ESP_ERR_HW_CRYPTO_BASE + 0x5 /*!< 填充检查失败，但仍会生成结果，并且可以读取*/
 ...
 */
typedef struct {
    uint32_t Y[4096/32];                    //!< RSA指数
    uint32_t M[4096/32];                    //!< RSA模数
    uint32_t Rb[4096/32];                   //!< RSA r反操作数
